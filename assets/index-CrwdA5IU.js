...
Respond in EXACTLY this JSON format (no markdown, no code blocks):
{
  ...
  `,document.body.appendChild(e),document.getElementById(`dt-skip`).addEventListener(`click`,rp),document.getElementById(`dt-next`).addEventListener(`click`,()=>{clearTimeout(Zf),clearInterval(qf),$f()})}function tp(){document.getElementById(`dt-card`)?.remove(),document.getElementById(`dt-flash`)?.remove()}function np(){Yf&&rp(),jf=window.location.hash||``,Mf=wf(localStorage),Nf=wf(sessionStorage),Pf=window.__retakeQuiz,localStorage.clear(),sessionStorage.clear(),delete window.__retakeQuiz,localStorage.removeItem(kf),Yf=!0,Xf=0,ep(),Qf(0)}function rp(){Yf=!1,clearTimeout(Zf),clearInterval(qf),Tf(localStorage,Mf),Tf(sessionStorage,Nf),Mf=null,Nf=null,Pf===void 0?delete window.__retakeQuiz:window.__retakeQuiz=Pf,Pf=void 0,(jf||``)!==(window.location.hash||``)&&(window.location.hash=jf||`#landing`),jf=``,tp()}function ip(){if(document.getElementById(`dt-styles`))return;let e=document.createElement(`style`);e.id=`dt-styles`,e.textContent=`
    #dt-card {
      position: fixed;
      z-index: 99995;
      top: 0;
  ...
  `,document.head.appendChild(e)}var ap=`modulepreload`,op=function(e){return`/mon_accord/`+e},sp={},cp=function(e,t,n){let r=Promise.resolve();if(t&&t.length>0){let e=document.getElementsByTagName(`link`),i=document.querySelector(`meta[property=csp-nonce]`),a=i?.nonce||i?.getAttribute(`nonce`);function o(e){return Promise.all(e.map(e=>Promise.resolve(e).then(e=>({status:`fulfilled`,value:e}),e=>({status:`rejected`,reason:e}))))}r=o(t.map(t=>{if(t=op(t,n),t in sp)return;sp[t]=!0;let r=t.endsWith(`.css`),i=r?`[rel="stylesheet"]`:``;if(n)for(let n=e.length-1;n>=0;n--){let i=e[n];if(i.href===t&&(!r||i.rel===`stylesheet`))return}else if(document.querySelector(`link[href="${t}"]${i}`))return;let o=document.createElement(`link`);if(o.rel=r?`stylesheet`:ap,r||(o.as=`script`),o.crossOrigin=``,o.href=t,a&&o.setAttribute(`nonce`,a),document.head.appendChild(o),r)return new Promise((e,n)=>{o.addEventListener(`load`,e),o.addEventListener(`error`,()=>n(Error(`Unable to preload CSS for ${t}`)))})}))}function i(e){let t=new Event(`vite:preloadError`,{cancelable:!0});if(t.payload=e,window.dispatchEvent(t),!t.defaultPrevented)throw e}return r.then(t=>{for(let e of t||[])e.status===`rejected`&&i(e.reason);return e().catch(i)})},lp=document.getElementById(`app`),up={"":b,"#landing":b,"#profile":Dd,"#lab":Yd,"#vault":nf,"#shop":yf,"#community":ff};function dp(){return window.location.hash||``}function fp(e){window.location.hash=e}function pp(){let e=dp(),t=up[e]||b;lp.innerHTML=``,lp.appendChild(f(fp,e));let n=document.createElement(`div`);if(n.className=`page page-enter`,n.id=`page-content`,t(n,fp),lp.appendChild(n),!document.querySelector(`.toast-container`)){let e=document.createElement(`div`);e.className=`toast-container`,e.id=`toast-container`,document.body.appendChild(e)}window.scrollTo({top:0,behavior:`auto`})}window.showToast=function(e,t=`success`){let n=document.getElementById(`toast-container`);if(!n)return;let r=document.createElement(`div`);r.className=`toast toast--${t}`,r.innerHTML=`<span class="toast__message">${e}</span>`,n.appendChild(r),setTimeout(()=>{r.style.animation=`fadeOut 0.3s var(--ease-out) forwards`,setTimeout(()=>r.remove(),300)},3e3)},window.showSettings=function(){let e=document.querySelector(`.modal-overlay`);e&&e.remove();let t=d.getApiKey(),n=document.createElement(`div`);n.className=`modal-overlay`,n.id=`settings-modal`,n.innerHTML=`
    <div class="modal">
      <div class="modal__header">
  ...
  `,document.body.appendChild(n),n.querySelector(`#close-settings`).onclick=()=>n.remove(),n.querySelector(`#cancel-settings`).onclick=()=>n.remove(),n.onclick=e=>{e.target===n&&n.remove()},n.querySelector(`#save-settings`).onclick=()=>{let e=n.querySelector(`#api-key-input`).value.trim();d.setApiKey(e),cp(()=>Promise.resolve().then(()=>md).then(e=>e.resetAI()),void 0),window.showToast(`Settings saved!`),n.remove()};let r=n.querySelector(`#reset-profile-btn`);r&&(r.onclick=()=>{[`profile`,`quiz_state`,`vault`,`interactions`,`likes`,`my_perfumes`,`pending_shop_cart`,`shop_cart`,`community_posts`,`post_likes`,`vault_folders`].forEach(e=>d.remove(e)),sessionStorage.clear(),window.showToast(`All data cleared. Starting fresh!`),n.remove(),pp()});let i=n.querySelector(`#settings-demo-btn`);i&&(i.onclick=()=>{n.remove(),typeof window.startDemoTour==`function`&&window.startDemoTour()})},window.startDemoTour=np,window.endDemoTour=rp,window.addEventListener(`hashchange`,pp),pp();