...
`).filter(e=>e.trim()).map(e=>{if(e.match(/^(OPENING|HEART|DRY DOWN|OVERALL|SILLAGE)/i)){let[t,...n]=e.split(`:`);return`<p><strong style="color: var(--accent);">${t.trim()}:</strong> ${n.join(`:`).trim()}</p>`}return`<p>${e}</p>`}).join(``):``}function Jd(){if(document.getElementById(`lab-styles`))return;let e=document.createElement(`style`);e.id=`lab-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e)}function Yd(e,t){let n=d.get(`vault_folders`,[{id:`default`,name:`All Formulas`,icon:`📁`}]),r=null,i=!1;function a(){d.set(`vault_folders`,n)}function o(e){let t=d.getVault();return e===`default`?t:t.filter(t=>t.folderId===e)}function s(){let t=d.getVault();e.innerHTML=`
      <div class="page__container">
        <div class="section-header">
          <p class="section-label">Your Collection</p>
          <h2 class="section-title">Vault</h2>
          <p class="section-subtitle">${t.length} formula${t.length===1?``:`s`} saved</p>
        </div>

        ${r?l(r,t):c(t)}
      </div>
    `,Xd(),u(t)}function c(e){return`
      ...
      <div class="vault-folders-grid">
        ${n.map(e=>{let t=o(e.id).length;return`
      ...
      `:``}
    `}function l(e,t){let r=o(e.id);return`
      ...
      `}
    `}function u(o){e.querySelectorAll(`.vault-folder-card`).forEach(e=>{e.addEventListener(`click`,t=>{if(t.target.closest(`.vault-folder-delete`))return;let i=e.dataset.folder;r=n.find(e=>e.id===i),s()})}),e.querySelectorAll(`.vault-folder-delete`).forEach(e=>{e.addEventListener(`click`,t=>{t.stopPropagation();let r=e.dataset.delete,i=d.getVault();i.forEach(e=>{e.folderId===r&&(e.folderId=void 0)}),d.set(`vault`,i),n=n.filter(e=>e.id!==r),a(),s()})});let c=e.querySelector(`#btn-create-folder`);c&&c.addEventListener(`click`,()=>{i=!0,s()});let l=e.querySelector(`#btn-save-folder`);l&&l.addEventListener(`click`,()=>{let t=e.querySelector(`#new-folder-name`).value.trim();t&&(n.push({id:`folder-`+Date.now(),name:t,icon:`📂`}),a(),i=!1,s())});let u=e.querySelector(`#btn-cancel-folder`);u&&u.addEventListener(`click`,()=>{i=!1,s()});let f=e.querySelector(`#btn-back-folders`);f&&f.addEventListener(`click`,()=>{r=null,s()});let p=e.querySelector(`#go-to-lab`);p&&p.addEventListener(`click`,()=>t(`#lab`)),e.querySelectorAll(`.vault-load-btn`).forEach(e=>{e.addEventListener(`click`,()=>{let n=o.find(t=>t.id===e.dataset.id);if(n?.layers){let e=n.layers.map(e=>e.perfumeId).filter(Boolean);sessionStorage.setItem(`labPending`,JSON.stringify(e)),t(`#lab`)}})}),e.querySelectorAll(`.vault-delete-btn`).forEach(e=>{e.addEventListener(`click`,()=>{d.removeFormula(e.dataset.id),s(),window.showToast(`Formula removed from vault.`)})}),e.querySelectorAll(`.vault-move-select`).forEach(e=>{e.addEventListener(`change`,()=>{let t=e.value;if(!t)return;let n=d.getVault(),r=n.find(t=>t.id===e.dataset.id);r&&(r.folderId=t,d.set(`vault`,n),s(),window.showToast(`Formula moved to folder.`))})})}s()}function Xd(){if(document.getElementById(`vault-styles`))return;let e=document.createElement(`style`);e.id=`vault-styles`,e.textContent=`
...
Respond in EXACTLY this JSON format (no markdown, no code blocks):
{
  ...
  "scentDescription": "Vivid 2-sentence sensory preview"
        ...
        <div class="community-section mt-2xl">
          <div class="community-section-header">
            ...
            <div class="community-sort">
              <button class="community-sort-btn ${i===`likes`?`community-sort-btn--active`:``}" data-sort="likes">♥ Most Liked</button>
              <button class="community-sort-btn ${i===`match`?`community-sort-btn--active`:``}" data-sort="match">✦ Best Match</button>
              <button class="community-sort-btn ${i===`newest`?`community-sort-btn--active`:``}" data-sort="newest">🕐 Newest</button>
            </div>
          </div>
          <div class="community-trending-list">
            ${n.map((e,t)=>`
              <div class="community-trending-item ${e.id===null?`community-trending-item--active`:``}" data-id="${e.id}" id="community-${e.id}">
                ...
                <div class="community-trending-content">
                  <div class="community-trending-top">
                    ...
                    <div class="community-trending-match" style="color: ${e.matchPercent>70?`#4CAF50`:e.matchPercent>50?`var(--accent)`:`var(--text-tertiary)`};">
                      ${e.matchPercent}% match
                    </div>
                  </div>
                  <div class="community-trending-layers">
                    ${(e.layers||[]).map(e=>{let t=y(e.perfumeId),n=t?p.find(e=>e.id===t.region):null;return`<span style="font-size: var(--text-xs); color: ${n?.color||`var(--text-tertiary)`};">${n?.icon||``} ${e.amount} ${e.unit} ${t?.name||``}</span>`}).join(` <span style="color: var(--text-tertiary);">+</span> `)}
                  </div>
                  ${e.description?`<p class="community-trending-desc">${e.description}</p>`:``}
                </div>
                <div class="community-trending-actions">
                  <span class="community-trending-likes">♥ ${e.likes||0}</span>
                  <button class="btn btn--ghost btn--sm community-interact-btn" data-id="${e.id}">💬</button>
                </div>
              </div>
            `).join(``)}
          </div>
    ...
    `,nf(),e.querySelectorAll(`.community-sort-btn`).forEach(e=>{e.addEventListener(`click`,()=>{i=e.dataset.sort,c()})}),e.querySelectorAll(`.community-interact-btn`).forEach(e=>{e.addEventListener(`click`,t=>{t.stopPropagation();let r=e.dataset.id,i=n.find(e=>e.id===r);i&&tf(i)})}),e.querySelectorAll(`.community-trending-item`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.id,r=n.find(e=>e.id===t);r&&tf(r)})});let r=e.querySelector(`#btn-generate-suggestion`);r&&r.addEventListener(`click`,async()=>{if(!hd()){window.showToast(`Set your Gemini API key in Settings.`,`error`),window.showSettings();return}let n=e.querySelector(`#suggestion-container`);n.innerHTML=`
        ...
        `;let i=await Qd();if(i.success&&i.remix){let e=i.remix;n.innerHTML=`
          ...
          `})}c()}function tf(e){let t=JSON.parse(localStorage.getItem(`monaccord_comments_`+e.id)||`[]`),n=d.isLiked(e.id),r=document.createElement(`div`);r.className=`modal-overlay`,r.innerHTML=`
    <div class="modal" style="max-width: 600px;">
      ...
      <div class="modal__body">
        ...
        <div id="comments-list" style="max-height: 200px; overflow-y: auto; margin-bottom: var(--space-md);">
          ${t.length===0?`<p style="font-size: var(--text-xs); color: var(--text-tertiary);">No comments yet. Be the first!</p>`:t.map(e=>`
              <div style="padding: var(--space-sm); background: var(--bg-secondary); border-radius: var(--radius-sm); margin-bottom: var(--space-xs);">
                ...
                <p style="font-size: var(--text-sm);">${e.text}</p>
                <span style="font-size: 10px; color: var(--text-tertiary);">${new Date(e.date).toLocaleDateString()}</span>
              </div>
            `).join(``)}
        </div>
  ...
  `,document.head.appendChild(e)}function rf(e,t){let n=[];function r(){e.innerHTML=`
      <div class="page__container">
        <div class="section-header">
  ...
  `,document.body.appendChild(r),r.querySelector(`#btn-go-profile`).addEventListener(`click`,()=>{r.remove(),n(`#profile`)}),r.onclick=e=>{e.target===r&&(r.remove(),n(`#profile`))}}function of(){if(document.getElementById(`shop-styles`))return;let e=document.createElement(`style`);e.id=`shop-styles`,e.textContent=`
    .shop-layout {
      display: grid;
  ...
  `,document.body.appendChild(n),n.querySelector(`#close-settings`).onclick=()=>n.remove(),n.querySelector(`#cancel-settings`).onclick=()=>n.remove(),n.onclick=e=>{e.target===n&&n.remove()},n.querySelector(`#save-settings`).onclick=()=>{let e=n.querySelector(`#api-key-input`).value.trim();d.setApiKey(e),uf(()=>Promise.resolve().then(()=>ud).then(e=>e.resetAI()),void 0),window.showToast(`Settings saved!`),n.remove()};let r=n.querySelector(`#reset-profile-btn`);r&&(r.onclick=()=>{d.remove(`monaccord_profile`),d.set(`profile`,null),window.showToast(`Profile reset.`),n.remove(),hf()})},window.addEventListener(`hashchange`,hf),hf();