(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var e=`monaccord_`,t={get(t,n=null){try{let r=localStorage.getItem(e+t);return r?JSON.parse(r):n}catch{return n}},set(t,n){try{localStorage.setItem(e+t,JSON.stringify(n))}catch(e){console.warn(`Storage write failed:`,e)}},remove(t){localStorage.removeItem(e+t)},getProfile(){return this.get(`profile`,null)},setProfile(e){this.set(`profile`,{...e,updatedAt:Date.now()})},getVault(){return this.get(`vault`,[])},saveFormula(e){let t=this.getVault(),n=t.findIndex(t=>t.id===e.id);return n>=0?t[n]={...e,savedAt:Date.now()}:t.push({...e,id:e.id||`f-`+Date.now(),savedAt:Date.now()}),this.set(`vault`,t),this.addInteraction({type:`save`,formulaId:e.id,timestamp:Date.now()}),t},removeFormula(e){let t=this.getVault().filter(t=>t.id!==e);return this.set(`vault`,t),t},getInteractions(){return this.get(`interactions`,[])},addInteraction(e){let t=this.getInteractions();t.push({...e,timestamp:e.timestamp||Date.now()}),t.length>500&&t.splice(0,t.length-500),this.set(`interactions`,t)},getLikes(){return this.get(`likes`,[])},toggleLike(e){let t=this.getLikes(),n=t.indexOf(e);return n>=0?(t.splice(n,1),this.addInteraction({type:`unlike`,formulaId:e,timestamp:Date.now()})):(t.push(e),this.addInteraction({type:`like`,formulaId:e,timestamp:Date.now()})),this.set(`likes`,t),t},isLiked(e){return this.getLikes().includes(e)},getOwnedPerfumes(){let e=this.get(`my_perfumes`,null);return Array.isArray(e)?{monAccord:e.map(e=>e.id),loreal:[]}:e||{monAccord:[],loreal:[]}},setOwnedPerfumes(e){this.set(`my_perfumes`,e)},getApiKey(){return this.get(`gemini_api_key`,``)},setApiKey(e){this.set(`gemini_api_key`,e)},getQuizState(){return this.get(`quiz_state`,null)},setQuizState(e){this.set(`quiz_state`,e)},clearQuizState(){this.remove(`quiz_state`)},setPendingShopCart(e){this.set(`pending_shop_cart`,e)},consumePendingShopCart(){let e=this.get(`pending_shop_cart`,[]);return this.remove(`pending_shop_cart`),Array.isArray(e)?e:[]},getShopCart(){let e=this.get(`shop_cart`,[]);return Array.isArray(e)?e:[]},setShopCart(e){this.set(`shop_cart`,e)},clearShopCart(){this.remove(`shop_cart`)},getUsername(){return this.getProfile()?.username||`Anonymous`},getPosts(){return this.get(`community_posts`,[])},setPosts(e){this.set(`community_posts`,e)},getPostLikes(){return this.get(`post_likes`,[])},togglePostLike(e){let t=this.getPostLikes(),n=t.indexOf(e);return n>=0?t.splice(n,1):t.push(e),this.set(`post_likes`,t),t},isPostLiked(e){return this.getPostLikes().includes(e)}};function n(e,n){let r=document.createElement(`nav`);r.className=`navbar`,r.id=`main-nav`,t.getProfile();let i=[{hash:`#landing`,label:`Home`,always:!0},{hash:`#profile`,label:`Profile`,always:!0},{hash:`#lab`,label:`Layering Lab`,always:!0},{hash:`#vault`,label:`Vault`,always:!0},{hash:`#shop`,label:`Shop`,always:!0},{hash:`#community`,label:`Community`,always:!0}],a=e=>n===e||n===``&&e===`#landing`;r.innerHTML=`
    ...
    <div class="navbar__links" id="nav-links">
      ${i.map(e=>`
        <a href="${e.hash}" class="navbar__link ${a(e.hash)?`active`:``}" id="nav-${e.hash.replace(`#`,``)}">${e.label}</a>
      `).join(``)}
    </div>
  ...
  `,p(),e.querySelector(`#scroll-hint`).addEventListener(`click`,()=>{document.getElementById(`origin-section`)?.scrollIntoView({behavior:`smooth`})}),e.querySelector(`#bottom-cta`).addEventListener(`click`,()=>{n(a?`#lab`:`#profile`)});let o=[0,1,2,3,4,5];function s(t){return e.querySelector(`.cf-card[data-index="${t}"]`)}function c(){let e=window.innerWidth;return e<400?170:e<600?220:e<900?270:320}function l(e,t,n=!1){let r=Math.abs(t),i=t*c(),a=r===0?1:r===1?.68:.5,o=r===0?1:r===1?.55:0,s=r===0?5:r===1?3:1;n&&(e.style.transition=`none`),e.style.transform=`translateX(calc(-50% + ${i}px)) translateY(-50%) scale(${a})`,e.style.opacity=o,e.style.zIndex=s,n&&(e.offsetHeight,e.style.transition=``)}function f(t=!1){o.forEach((e,n)=>{l(s(e),n-2,t),s(e).classList.toggle(`cf-card--active`,n===2)}),e.querySelectorAll(`.cf-dot`).forEach((e,t)=>{e.classList.toggle(`active`,t===o[2])}),m()}function m(){let t=r[o[2]],n=i.find(e=>e.region===t.id&&e.format===`spray`),a=i.find(e=>e.region===t.id&&e.format===`oil`),s=e.querySelector(`#cf-info`),c=(e,t)=>`
    ...
    `,s.querySelector(`.cf-info__inner`).animate([{opacity:0,transform:`translateX(16px)`},{opacity:1,transform:`translateX(0)`}],{duration:340,easing:`cubic-bezier(0.16,1,0.3,1)`,fill:`forwards`})}function h(){l(s(o[0]),3,!0),o=[...o.slice(1),o[0]],f()}function g(){l(s(o[5]),-3,!0),o=[o[5],...o.slice(0,5)],f()}function _(e){if(o.indexOf(e)-2==0)return;let t=o.indexOf(e);o=[...o.slice(t),...o.slice(0,t)],f(!0)}e.querySelector(`#cf-prev`).addEventListener(`click`,g),e.querySelector(`#cf-next`).addEventListener(`click`,h),e.querySelectorAll(`.cf-dot`).forEach((e,t)=>{e.addEventListener(`click`,()=>_(t))}),e.querySelectorAll(`.cf-card`).forEach(e=>{e.addEventListener(`click`,()=>{let t=o.indexOf(Number(e.dataset.index));t!==2&&(t>2?h():g())})}),e.addEventListener(`keydown`,e=>{e.key===`ArrowLeft`&&g(),e.key===`ArrowRight`&&h()});let v=e.querySelector(`#cf-stage`),y=0,b=0;v.addEventListener(`touchstart`,e=>{y=e.touches[0].clientX,b=e.touches[0].clientY},{passive:!0}),v.addEventListener(`touchend`,e=>{let t=y-e.changedTouches[0].clientX,n=Math.abs(b-e.changedTouches[0].clientY);Math.abs(t)>n&&Math.abs(t)>40&&(t>0?h():g())},{passive:!0}),f(!0);let x=new IntersectionObserver(e=>{e.forEach(e=>{e.isIntersecting&&e.target.classList.add(`visible`)})},{threshold:.1});e.querySelectorAll(`.how-step`).forEach(e=>x.observe(e))}function p(){if(document.getElementById(`landing-styles`))return;let e=document.createElement(`style`);e.id=`landing-styles`,e.textContent=`
    /* ── Hero ── */
    .hero {
    ...
    @media (max-width: 1100px) {
      ...
      .cf-info__name { font-size: 1.8rem; }
      .cf-info__desc { max-width: 100%; }
      .how-steps { grid-template-columns: 1fr; }
      .hero__title { font-size: 2.2rem; }
    }

    @media (max-width: 600px) {
      .hero { padding: var(--space-lg) var(--space-md); padding-bottom: 80px; }
      .hero__title { font-size: 1.9rem; }
      .hero__subtitle { font-size: var(--text-base); }

      .cf-showcase { padding: 0 var(--space-xs); }
      .cf-visual { height: 300px; }
      .cf-card { width: 220px; height: 280px; }
      .cf-card__img { height: 240px; }
      .cf-nav--prev { left: 6px; }
      .cf-nav--next { right: 6px; }
      .cf-nav { width: 38px; height: 38px; }

      .cf-info { padding: var(--space-md); }
      .cf-info__name { font-size: 1.5rem; }
      .cf-info__notes-cols { gap: var(--space-md); }

      .cta-card { padding: var(--space-2xl) var(--space-lg); }
      .landing-how { padding: var(--space-2xl) 0 var(--space-3xl); }
      .how-step { flex-direction: column; gap: var(--space-sm); padding: var(--space-md) var(--space-sm); }
      .how-step__text { width: auto; margin-top: var(--space-sm); }
      .how-step__number { font-size: var(--text-3xl); }
    }
...
Be poetic but concise. One vivid idea per section. No extra headings or commentary.`,2,300);if(t.success)return t;let n=e.map(e=>i.find(t=>t.id===e.perfumeId)).filter(Boolean),r=n[0]?.topNotes?.[0]||`bergamot`,a=n[1]?.middleNotes?.[0]||n[0]?.middleNotes?.[0]||`iris`,o=n[n.length-1]?.baseNotes?.[0]||`sandalwood`,s=n[n.length-1]?.name||`the base`;return{success:!0,text:[`OPENING (first 15 min): A vivid burst of ${r} announces the blend — bright, immediate, and self-assured.`,`HEART (30 min – 2 hrs): ${a} rises gently from beneath, adding warmth and complexity as the formula finds its shape on your skin.`,`DRY DOWN (2+ hrs): ${o} from ${s} anchors the composition — intimate, lasting, and quietly confident.`].join(`
`)}}var le=Object.fromEntries([`fresh`,`floral`,`woody`,`oriental`,`citrus`,`gourmand`,`green`,`aromatic`,`spicy`,`musky`].map(e=>[e,new URL(Object.assign({"../assets/quiz_scent_families_v2/aromatic.png":m,"../assets/quiz_scent_families_v2/citrus.png":h,"../assets/quiz_scent_families_v2/floral.png":g,"../assets/quiz_scent_families_v2/fresh.png":_,"../assets/quiz_scent_families_v2/gourmand.png":v,"../assets/quiz_scent_families_v2/green.png":y,"../assets/quiz_scent_families_v2/musky.png":b,"../assets/quiz_scent_families_v2/oriental.png":x,"../assets/quiz_scent_families_v2/spicy.png":S,"../assets/quiz_scent_families_v2/woody.png":ee})[`../assets/quiz_scent_families_v2/${e}.png`],import.meta.url).href])),ue={elegant:new URL(`/mon_accord/assets/elegant_classic-BiRAW1Ly.png`,``+import.meta.url).href,adventurous:new URL(`/mon_accord/assets/adventurous_bold-Bz6pRBer.png`,``+import.meta.url).href,romantic:new URL(`/mon_accord/assets/romantic_dreamy-CNzM-gWm.png`,``+import.meta.url).href,minimalist:new URL(`/mon_accord/assets/minimalist_clean-Dr7j3KT0.png`,``+import.meta.url).href,creative:new URL(`/mon_accord/assets/creative_expressive-D6akHWmL.png`,``+import.meta.url).href,confident:new URL(`/mon_accord/assets/confident_powerful-Bas-bCGv.png`,``+import.meta.url).href},de=5;function fe(e,n){let r=t.getProfile();if(r&&!window.__retakeQuiz){xe(e,r,n);return}window.__retakeQuiz=!1;let i=t.getQuizState(),a=i?.step||1,o=i?.answers||{username:``,scentFamilies:[],knownPerfumes:[],sillage:5,longevity:5,intensity:5,personality:``,notes:``};function s(){t.setQuizState({step:a,answers:o}),e.innerHTML=`
      <div class="page__container">
        <div class="quiz-container">
          <div class="quiz-stepper">
            ${Array.from({length:de},(e,t)=>{let n=t+1,r=n<a?`completed`:n===a?`active`:``,i=n<a?`done`:``;return`<div class="quiz-stepper__item ${r}"><div class="quiz-stepper__circle">${n<a?`✓`:String(n)}</div></div>${t<de-1?`<div class="quiz-stepper__line ${i}"></div>`:``}`}).join(``)}
          </div>
          ...
          <div class="quiz-actions">
            ...
            ${a<de?`<button class="btn btn--primary" id="quiz-next">Continue →</button>`:`<button class="btn btn--primary" id="quiz-finish">
                  ${A()?`✦ Generate My Profile`:`✦ Generate Profile`}
                </button>`}
          </div>
        </div>
      </div>
    `,Se(),he(a,o,e);let r=e.querySelector(`#quiz-back`),i=e.querySelector(`#quiz-next`),c=e.querySelector(`#quiz-finish`);r&&r.addEventListener(`click`,()=>{ge(a,o,e),a--,s()}),i&&i.addEventListener(`click`,()=>{ge(a,o,e),a++,s()}),c&&c.addEventListener(`click`,async()=>{if(ge(a,o,e),!A()){window.showToast(`Please set your Gemini API key in Settings first.`,`error`),window.showSettings();return}c.disabled=!0,c.innerHTML=`<span class="loading-spinner"></span> Generating...`;let r=e.querySelector(`#generation-status`);r||(r=document.createElement(`p`),r.id=`generation-status`,r.style.cssText=`font-size: var(--text-xs); color: var(--text-tertiary); text-align: center; margin-top: var(--space-sm);`,c.parentElement.appendChild(r)),r.textContent=`Analysing your preferences...`;let i=await j(o);i.success?(i.profile.username=o.username||`Anonymous`,t.setProfile(i.profile),t.clearQuizState(),_e(e,i.profile,n),window.showToast(`Your olfactory profile has been created! ✦`)):(c.disabled=!1,c.innerHTML=`✦ Generate My Profile`,r.textContent=``,window.showToast(i.error||`Failed to generate profile.`,`error`))})}s()}function pe(e,t){switch(e){case 1:return`
      ...
      `;case 5:return`
        ...
        <div class="quiz-grid quiz-grid--context">
          ${[{id:`elegant`,name:`Elegant & Classic`,desc:`Timeless sophistication`},{id:`adventurous`,name:`Adventurous & Bold`,desc:`Love discovering the new`},{id:`romantic`,name:`Romantic & Dreamy`,desc:`Soft, poetic, emotional`},{id:`minimalist`,name:`Minimalist & Clean`,desc:`Less is more`},{id:`creative`,name:`Creative & Expressive`,desc:`Unique, unconventional`},{id:`confident`,name:`Confident & Powerful`,desc:`Commands attention`}].map(e=>`
            <div class="quiz-option quiz-option--img-card quiz-option--family-card quiz-option--trait-card ${t.personality===e.id?`quiz-option--selected`:``}" data-value="${e.id}" id="ctx-${e.id}">
              <img class="quiz-family-img" src="${ue[e.id]}" alt="${e.name}" loading="lazy" decoding="async" />
              <div class="quiz-family-overlay">
                <span class="quiz-family-name">${e.name}</span>
              </div>
            </div>
      ...
      `}}function me(e,t){let n=t?o.filter(e=>e.name.toLowerCase().includes(t.toLowerCase())||e.brand.toLowerCase().includes(t.toLowerCase())):o,r={};return n.forEach(e=>{r[e.brand]||(r[e.brand]=[]),r[e.brand].push(e)}),Object.entries(r).map(([t,n])=>`
  ...
  `).join(``)}function he(e,t,n){if(e===2&&n.querySelectorAll(`.quiz-grid--families .quiz-option`).forEach(e=>{e.addEventListener(`click`,()=>{let n=e.dataset.value;t.scentFamilies.includes(n)?(t.scentFamilies=t.scentFamilies.filter(e=>e!==n),e.classList.remove(`quiz-option--selected`)):(t.scentFamilies.push(n),e.classList.add(`quiz-option--selected`))})}),e===3){let e=n.querySelector(`#perfume-search`),r=n.querySelector(`#perfume-list`),i=e=>{r.innerHTML=me(t.knownPerfumes,e),r.querySelectorAll(`.quiz-option--perfume`).forEach(e=>{e.addEventListener(`click`,()=>{let r=e.dataset.value;t.knownPerfumes.includes(r)?(t.knownPerfumes=t.knownPerfumes.filter(e=>e!==r),e.classList.remove(`quiz-option--selected`)):(t.knownPerfumes.push(r),e.classList.add(`quiz-option--selected`)),n.querySelector(`.quiz-hint`).textContent=`Selected: ${t.knownPerfumes.length} perfume${t.knownPerfumes.length===1?``:`s`}`})})};e.addEventListener(`input`,e=>i(e.target.value)),i(``)}e===4&&[`sillage`,`longevity`,`intensity`].forEach(e=>{let r=n.querySelector(`#slider-${e}`),i=n.querySelector(`#${e}-val`);r&&r.addEventListener(`input`,()=>{t[e]=parseInt(r.value),i.textContent=`${r.value}/10`,r.style.setProperty(`--pct`,`${(parseInt(r.value)-1)/9*100}%`)})}),e===5&&n.querySelectorAll(`.quiz-grid--context .quiz-option`).forEach(e=>{e.addEventListener(`click`,()=>{let r=e.classList.contains(`quiz-option--selected`);n.querySelectorAll(`.quiz-grid--context .quiz-option`).forEach(e=>e.classList.remove(`quiz-option--selected`)),r?t.personality=``:(e.classList.add(`quiz-option--selected`),t.personality=e.dataset.value)})})}function ge(e,t,n){if(e===1){let e=n.querySelector(`#quiz-username`);e&&(t.username=e.value.trim())}if(e===5){let e=n.querySelector(`#quiz-notes`);e&&(t.notes=e.value)}}function _e(e,n,r){window.scrollTo({top:0,behavior:`smooth`});let i=ye(n),a=be(n);e.innerHTML=`
    <div class="page__container">
      <div class="profile-result">
        ...
        <div class="mt-xl">
          ...
          <div class="recommendation-grid">
            ${i.map(e=>ve(e,{showBuyButton:!0})).join(``)}
          </div>
        </div>
...
Respond in EXACTLY this JSON format (no markdown, no code blocks):
{
...
}`,2,400);if(a.success)try{let n=a.text.trim();n.startsWith("```")&&(n=n.replace(/^```(?:json)?\n?/,``).replace(/\n?```$/,``));let r=JSON.parse(n);return t.addInteraction({type:`contextual-recommendation`,context:e,recommendation:r.formulaName}),{success:!0,recommendation:r}}catch(e){return console.error(`Failed to parse recommendation:`,e),{success:!0,recommendation:{formulaName:`Your Recommendation`,layers:[],reasoning:a.text,scentPreview:``,tips:``}}}return{success:!0,recommendation:ke(e)}}function ke(e){let t=e?.mood||``,n=e?.occasion||``,r=e?.season||``,i=[`romantic`,`confident`,`mysterious`].includes(t)||[`date-night`,`formal`].includes(n)||[`autumn`,`winter`].includes(r),a=[`energetic`,`playful`,`calm`].includes(t)||[`outdoor`,`casual`].includes(n)||[`spring`,`summer`].includes(r);return i?{formulaName:`Golden Dusk`,layers:[{perfumeId:`middleeast-spray`,amount:3,unit:`sprays`},{perfumeId:`mediterranean-spray`,amount:2,unit:`sprays`},{perfumeId:`southamerica-oil`,amount:2,unit:`drops`}],reasoning:`A warm, opulent formula built for evenings and intimate occasions. The Middle Eastern spray leads with oud and rose, anchored by a South American oil that adds lasting depth.`,scentPreview:`Opens with a confident burst of oud and saffron; dries down to warm tonka and vetiver — rich, memorable, unmistakably present.`,tips:`Apply the oil first to clean, moisturised skin for maximum longevity, then spray from 20 cm.`}:a?{formulaName:`Morning Clarity`,layers:[{perfumeId:`scandinavian-spray`,amount:3,unit:`sprays`},{perfumeId:`mediterranean-spray`,amount:2,unit:`sprays`},{perfumeId:`eastasia-oil`,amount:2,unit:`drops`}],reasoning:`A crisp, uplifting blend ideal for daytime and outdoor settings. Nordic freshness pairs with Mediterranean citrus, while the East Asian oil adds quiet complexity.`,scentPreview:`A burst of bergamot and cool pine opens the blend; white tea and green notes settle into a clean, skin-close finish.`,tips:`Best applied right after a shower — the warmth of your skin activates the citrus top notes immediately.`}:{formulaName:`Signature Accord`,layers:[{perfumeId:`scandinavian-spray`,amount:2,unit:`sprays`},{perfumeId:`middleeast-spray`,amount:2,unit:`sprays`},{perfumeId:`mediterranean-spray`,amount:1,unit:`sprays`},{perfumeId:`eastasia-oil`,amount:2,unit:`drops`}],reasoning:`A balanced, all-occasion formula that bridges freshness and depth. Each region contributes a distinct character while remaining harmonious as a whole.`,scentPreview:`Fresh Nordic air over a warm Eastern heart — versatile enough for day or night, understated enough to never overwhelm.`,tips:`Layer the oil on pulse points first, then apply sprays starting from the heaviest to lightest sillage.`}}function Ae(e,n={}){let{showNameInput:r=!0,onSaved:i}=n,a=t.get(`vault_folders`,[{id:`default`,name:`All Formulas`,icon:``}]),o=`default`,s=!1;function c(){let n=document.getElementById(`save-vault-overlay`);n&&n.remove();let l=document.createElement(`div`);l.className=`modal-overlay`,l.id=`save-vault-overlay`,l.innerHTML=`
  ...
  `,document.head.appendChild(e)}var Me={scandinavian:new URL(`/mon_accord/assets/scandinavian-BN4U9Git.webp`,``+import.meta.url).href,eastasia:new URL(`/mon_accord/assets/east_asia-CIRqjWhL.webp`,``+import.meta.url).href,southafrica:new URL(`/mon_accord/assets/south_africa-BeRUDnKj.webp`,``+import.meta.url).href,mediterranean:new URL(`/mon_accord/assets/mediterranean-v1NGrAPJ.webp`,``+import.meta.url).href,southamerica:new URL(`/mon_accord/assets/south_america-DEpvED0s.webp`,``+import.meta.url).href,middleeast:new URL(`/mon_accord/assets/middle_east-DE6yO7Qz.webp`,``+import.meta.url).href},Ne={scandinavian:new URL(`/mon_accord/assets/scandinavian-DvtLirp4.webp`,``+import.meta.url).href,eastasia:new URL(`/mon_accord/assets/east_asia-UzXNmAB4.webp`,``+import.meta.url).href,southafrica:new URL(`/mon_accord/assets/south_africa-PFpjigF9.webp`,``+import.meta.url).href,mediterranean:new URL(`/mon_accord/assets/mediterranean-CV0fuGZk.webp`,``+import.meta.url).href,southamerica:new URL(`/mon_accord/assets/south_america-DyzbduD3.webp`,``+import.meta.url).href,middleeast:new URL(`/mon_accord/assets/middle_east-Bmlf8LlT.webp`,``+import.meta.url).href},Pe={scandinavian:new URL(`/mon_accord/assets/scandinavian-BBoj4W5D.webp`,``+import.meta.url).href,eastasia:new URL(`/mon_accord/assets/east_asia-D93RVLet.webp`,``+import.meta.url).href,southafrica:new URL(`/mon_accord/assets/south_africa-hdHFYwXs.webp`,``+import.meta.url).href,mediterranean:new URL(`/mon_accord/assets/mediterranean-B_fMAvaC.webp`,``+import.meta.url).href,southamerica:new URL(`/mon_accord/assets/south_america-DcR51yl4.webp`,``+import.meta.url).href,middleeast:new URL(`/mon_accord/assets/middle_east-DMI8-64c.webp`,``+import.meta.url).href},Fe=`lab_session_state`;function Ie(e){try{sessionStorage.setItem(Fe,JSON.stringify(e))}catch{}}function Le(){try{return JSON.parse(sessionStorage.getItem(Fe)||`null`)}catch{return null}}function Re(e,t){let n=Le(),a=n?.layers||[],o=n?.scentSimulation||null,s=!1,c=n?.contextResult||null,u=!1,d=n?.selectedMood||null,f=n?.selectedOccasion||null,p=n?.selectedSeason||null,m=n?.selectedTime||null,h=n?.selectedIntensity||5;function g(){Ie({layers:a,scentSimulation:o,contextResult:c,selectedMood:d,selectedOccasion:f,selectedSeason:p,selectedTime:m,selectedIntensity:h})}let _=JSON.parse(sessionStorage.getItem(`labPending`)||`[]`);_.length>0&&(_.forEach(e=>{let t=l(e);t&&!a.find(t=>t.perfumeId===e)&&a.push({perfumeId:e,amount:t.format===`spray`?2:3,unit:t.format===`spray`?`sprays`:`drops`})}),sessionStorage.removeItem(`labPending`),g());function v(){e.innerHTML=`
      <div class="page__container">
              ...
              <div class="lab-advisor__form">
                <div class="input-group">
                  ...
                  <div class="lab-advisor__chips" id="mood-chips">
                    ${Ce.map(e=>`<button class="lab-chip ${d===e.id?`lab-chip--active`:``}" data-value="${e.id}">${e.icon} ${e.name}</button>`).join(``)}
                  </div>
                </div>
                ...
                <div class="input-group">
                  ...
                  <div class="lab-advisor__chips" id="occasion-chips">
                    ${we.map(e=>`<button class="lab-chip ${f===e.id?`lab-chip--active`:``}" data-value="${e.id}">${e.icon} ${e.name}</button>`).join(``)}
                  </div>
                </div>
                ...
                <div class="input-group">
                  ...
                  <div class="lab-advisor__chips" id="season-chips">
                    ${Te.map(e=>`<button class="lab-chip ${p===e.id?`lab-chip--active`:``}" data-value="${e.id}">${e.icon} ${e.name}</button>`).join(``)}
                  </div>
                </div>
                ...
                <div class="slider-container">
                  <div class="slider-header">
                    <span class="slider-label">Intensity</span>
                    <span class="slider-value" id="advisor-intensity-val">${(Ee.find(e=>e.value===h)||{}).name||`Moderate`}</span>
                  </div>
                ...
                <div class="lab-advisor__result mt-lg" id="advisor-result">
                  <div class="ai-response">
                    ...
                    <div class="ai-response__text ai-response__text--compact">
                      ${c.reasoning?`<p>${ze(c.reasoning,150)}</p>`:``}
                      ${c.scentPreview?`<p><em>${ze(c.scentPreview,100)}</em></p>`:``}
                      ${c.tips?`<p style="color: var(--accent); font-size: var(--text-xs);">◈ ${ze(c.tips,100)}</p>`:``}
                    </div>
                  </div>
                  ${c.layers?.length>0?`
                      ...
                      <div class="lab-bottle-row">
                        ${t?`
                          <button class="lab-bottle-btn ${r?`lab-bottle-btn--added`:``}" data-id="${t.id}" style="--region-color: ${e.color};" title="${e.name} Spray">
                            <img src="${Ne[e.id]}" alt="${e.name} Spray" class="lab-bottle-img" loading="lazy" decoding="async" />
                        ...
                        ${n?`
                          <button class="lab-bottle-btn ${o?`lab-bottle-btn--added`:``}" data-id="${n.id}" style="--region-color: ${e.color};" title="${e.name} Oil">
                            <img src="${Pe[e.id]}" alt="${e.name} Oil" class="lab-bottle-img" loading="lazy" decoding="async" />
            ...
            <div class="lab-layers" id="lab-layers">
              ...
              `:a.map((e,t)=>{let n=l(e.perfumeId),i=r.find(e=>e.id===n.region);return`
                  <div class="lab-layer" data-idx="${t}" style="--region-color: ${i.color};">
                    <div class="lab-layer__bg" style="background-image: url('${Me[i.id]}')"></div>
...
`).filter(e=>e.trim()).filter(e=>!e.match(/^(OVERALL|SILLAGE)/i)).map(e=>{if(e.match(/^(OPENING|HEART|DRY DOWN)/i)){let t=e.indexOf(`:`);return`<p class="sim-section"><span class="sim-label">${e.substring(0,t).trim()}</span><span class="sim-body">${e.substring(t+1).trim()}</span></p>`}return`<p>${e}</p>`}).join(``):``}function Ve(){if(document.getElementById(`lab-styles`))return;let e=document.createElement(`style`);e.id=`lab-styles`,e.textContent=`
    ...
    @media (max-width: 1024px) {
      ...
      .lab-canvas { position: static; max-height: none; overflow-y: visible; }
    }

    @media (max-width: 768px) {
      .lab-page-header { padding: var(--space-lg) 0; }
      .lab-page-header__title { font-size: 1.8rem; }

      .lab-perfume-selector { grid-template-columns: repeat(2, 1fr); }
      .lab-bottle-img { height: 88px; }

      .lab-chip { padding: 7px 14px; font-size: var(--text-sm); min-height: 36px; }

      .lab-actions { flex-direction: column; }
      .lab-actions .btn { width: 100%; justify-content: center; }

      .lab-advisor { padding: var(--space-lg); }
    }

    @media (max-width: 480px) {
      .lab-page-header { padding: var(--space-md) 0; }
      .lab-page-header__desc { font-size: var(--text-sm); }

      .lab-perfume-selector { grid-template-columns: repeat(2, 1fr); gap: var(--space-sm); }
      .lab-bottle-img { height: 72px; }
      .lab-bottle-btn { padding: 6px 2px 5px; }

      .lab-layer__content { padding: var(--space-sm) var(--space-md); }
      .lab-layer__amount-btn { width: 32px; height: 32px; }

      .lab-advisor { padding: var(--space-md); }
    }
  `,document.head.appendChild(e)}var He=`/mon_accord/assets/folder-DAYqs9Dj.webp`,Ue={scandinavian:new URL(`/mon_accord/assets/scandinavian-DvtLirp4.webp`,``+import.meta.url).href,eastasia:new URL(`/mon_accord/assets/east_asia-UzXNmAB4.webp`,``+import.meta.url).href,southafrica:new URL(`/mon_accord/assets/south_africa-PFpjigF9.webp`,``+import.meta.url).href,mediterranean:new URL(`/mon_accord/assets/mediterranean-CV0fuGZk.webp`,``+import.meta.url).href,southamerica:new URL(`/mon_accord/assets/south_america-DyzbduD3.webp`,``+import.meta.url).href,middleeast:new URL(`/mon_accord/assets/middle_east-Bmlf8LlT.webp`,``+import.meta.url).href},We={scandinavian:new URL(`/mon_accord/assets/scandinavian-BBoj4W5D.webp`,``+import.meta.url).href,eastasia:new URL(`/mon_accord/assets/east_asia-D93RVLet.webp`,``+import.meta.url).href,southafrica:new URL(`/mon_accord/assets/south_africa-hdHFYwXs.webp`,``+import.meta.url).href,mediterranean:new URL(`/mon_accord/assets/mediterranean-B_fMAvaC.webp`,``+import.meta.url).href,southamerica:new URL(`/mon_accord/assets/south_america-DcR51yl4.webp`,``+import.meta.url).href,middleeast:new URL(`/mon_accord/assets/middle_east-DMI8-64c.webp`,``+import.meta.url).href},Ge={"mugler-angel":new URL(`/mon_accord/assets/angel-JaKC4iyN.webp`,``+import.meta.url).href,"ysl-black-opium":new URL(`/mon_accord/assets/black_opium-CwBrp2qb.webp`,``+import.meta.url).href,"lancome-la-vie-est-belle":new URL(`/mon_accord/assets/la_vie_est_belle-CHGqLg72.webp`,``+import.meta.url).href},Ke=[`mugler-angel`,`ysl-black-opium`,`lancome-la-vie-est-belle`],qe=[{id:`evening`,name:`Evening Wear`},{id:`daytime`,name:`Daytime`},{id:`office`,name:`Office`},{id:`weekend`,name:`Weekend`},{id:`seasonal`,name:`Seasonal`}],Je=[{id:`default-ev-1`,name:`Velvet Dusk`,folderId:`evening`,layers:[{perfumeId:`middleeast-spray`,amount:3,unit:`sprays`},{perfumeId:`southafrica-oil`,amount:2,unit:`drops`},{perfumeId:`scandinavian-oil`,amount:1,unit:`drops`}]},{id:`default-ev-2`,name:`Golden Night`,folderId:`evening`,layers:[{perfumeId:`southamerica-spray`,amount:3,unit:`sprays`},{perfumeId:`middleeast-oil`,amount:2,unit:`drops`}]},{id:`default-ev-3`,name:`Night Bloom`,folderId:`evening`,layers:[{perfumeId:`mediterranean-spray`,amount:2,unit:`sprays`},{perfumeId:`eastasia-oil`,amount:2,unit:`drops`},{perfumeId:`middleeast-oil`,amount:1,unit:`drops`}]},{id:`default-da-1`,name:`Fresh Morning`,folderId:`daytime`,layers:[{perfumeId:`scandinavian-spray`,amount:3,unit:`sprays`},{perfumeId:`mediterranean-oil`,amount:1,unit:`drops`}]},{id:`default-da-2`,name:`Sun Garden`,folderId:`daytime`,layers:[{perfumeId:`mediterranean-spray`,amount:2,unit:`sprays`},{perfumeId:`eastasia-spray`,amount:2,unit:`sprays`},{perfumeId:`scandinavian-oil`,amount:1,unit:`drops`}]},{id:`default-da-3`,name:`Coastal Breeze`,folderId:`daytime`,layers:[{perfumeId:`scandinavian-spray`,amount:2,unit:`sprays`},{perfumeId:`southamerica-spray`,amount:2,unit:`sprays`}]},{id:`default-da-4`,name:`Island Citrus`,folderId:`daytime`,layers:[{perfumeId:`southamerica-spray`,amount:3,unit:`sprays`},{perfumeId:`mediterranean-oil`,amount:1,unit:`drops`}]},{id:`default-of-1`,name:`Clean Presence`,folderId:`office`,layers:[{perfumeId:`scandinavian-spray`,amount:2,unit:`sprays`},{perfumeId:`eastasia-oil`,amount:1,unit:`drops`}]},{id:`default-of-2`,name:`Polished Air`,folderId:`office`,layers:[{perfumeId:`mediterranean-spray`,amount:2,unit:`sprays`},{perfumeId:`scandinavian-oil`,amount:1,unit:`drops`}]},{id:`default-we-1`,name:`Lazy Sunday`,folderId:`weekend`,layers:[{perfumeId:`southamerica-spray`,amount:3,unit:`sprays`},{perfumeId:`eastasia-oil`,amount:2,unit:`drops`}]},{id:`default-se-1`,name:`Winter Warmth`,folderId:`seasonal`,layers:[{perfumeId:`middleeast-spray`,amount:3,unit:`sprays`},{perfumeId:`southafrica-oil`,amount:2,unit:`drops`}]},{id:`default-se-2`,name:`Spring Bloom`,folderId:`seasonal`,layers:[{perfumeId:`mediterranean-spray`,amount:2,unit:`sprays`},{perfumeId:`scandinavian-spray`,amount:2,unit:`sprays`},{perfumeId:`eastasia-oil`,amount:1,unit:`drops`}]},{id:`default-se-3`,name:`Autumn Spice`,folderId:`seasonal`,layers:[{perfumeId:`southafrica-spray`,amount:2,unit:`sprays`},{perfumeId:`middleeast-oil`,amount:2,unit:`drops`}]}];function Ye(e,n){if(!t.get(`vault_defaults_seeded`,!1)){let e=t.getVault(),n=new Set(e.map(e=>e.id)),r=Date.now();Je.forEach((t,i)=>{n.has(t.id)||e.push({...t,savedAt:r+i})}),t.set(`vault`,e),t.set(`vault_defaults_seeded`,!0)}let a=t.getOwnedPerfumes(),s=Ke.filter(e=>!a.loreal.includes(e));s.length&&(a.loreal=[...a.loreal,...s],t.setOwnedPerfumes(a));let c=t.get(`vault_folders`,null),u;if(!c)u=qe.map(e=>({...e})),t.set(`vault_folders`,u);else{let e=c.filter(e=>e.id!==`default`),n=new Set(e.map(e=>e.id)),r=qe.filter(e=>!n.has(e.id));u=[...e,...r].map(e=>({id:e.id,name:e.name})),t.set(`vault_folders`,u)}let d=null,f=!1,p=null,m=null;function h(){t.set(`vault_folders`,u)}function g(e){return t.getVault().filter(t=>t.folderId===e)}function _(){let n=t.getVault();e.innerHTML=`
      <div class="page__container">
        ${d?b(d,n):v(n)}
      </div>
      ${x()}
    `,Xe(),S(n)}function v(e){return`
          ...
          <div class="vault-folders-grid">
            ${u.map(e=>{let t=g(e.id).length;return`
                <div class="vault-folder-card card card--interactive" data-folder="${e.id}">
                  <img src="${He}" class="vault-folder-icon" alt="folder" loading="lazy" decoding="async" />
    ...
    `}function y(){let e=t.getOwnedPerfumes();function n(e){return e.length?`<div class="vault-bottle-grid">
        ${e.map(e=>{let t=l(e),n=t?r.find(e=>e.id===t.region):null,i=t?.format===`spray`?Ue[t.region]:We[t.region],a=t?.name||e;return`
        ...
        ${e.map(e=>{let t=o.find(t=>t.id===e),n=Ge[e],r=t?t.name:e,i=t?.brand||``;return`
                      ...
                      <div class="vault-modal-bottle-row">
                        ${n?`
                          <button class="vault-modal-bottle-btn vault-owned-add-btn ${a?`vault-modal-bottle-btn--owned`:``}"
                            ...
                            ${a?`disabled`:``}>
                            <img src="${Ue[t.id]}" alt="Spray" class="vault-modal-bottle-img" loading="lazy" decoding="async" />
                        ...
                        ${r?`
                          <button class="vault-modal-bottle-btn vault-owned-add-btn ${o?`vault-modal-bottle-btn--owned`:``}"
                            ...
                            ${o?`disabled`:``}>
                            <img src="${We[t.id]}" alt="Oil" class="vault-modal-bottle-img" loading="lazy" decoding="async" />
...
Respond in EXACTLY this JSON format (no markdown, no code blocks):
{
  ...
  "scentDescription": "Vivid 2-sentence sensory preview"
        ...
        <div class="community-choice-row">
          <div class="community-choice-card">
            <div class="community-choice-card__photo-wrap">
              <img src="${tt}" alt="Léna" class="community-choice-card__photo" loading="lazy" decoding="async" />
            </div>
            <p class="community-choice-card__name">Léna</p>
            <p class="community-choice-card__role">Author & Fashion Entrepreneur</p>
            <p class="community-choice-card__quote">"Luxury isn't just what you wear — it's what you leave in the room. Mediterranean florals with a hint of Middle Eastern warmth is my non-negotiable."</p>
            <div class="community-choice-card__combo">
              <div class="community-choice-card__layer">
                <span style="color:${r.find(e=>e.id===`mediterranean`)?.color};">Mediterranean — Spray</span>
                <span class="community-choice-card__amount">2 sprays</span>
              </div>
              <div class="community-choice-card__layer">
                <span style="color:${r.find(e=>e.id===`middleeast`)?.color};">Middle East — Oil</span>
                <span class="community-choice-card__amount">2 drops</span>
              </div>
              <div class="community-choice-card__layer">
                <span style="color:${r.find(e=>e.id===`scandinavian`)?.color};">Scandinavian — Oil</span>
          ...
          <div class="community-choice-card">
            <div class="community-choice-card__photo-wrap">
              <img src="${nt}" alt="Squeezie" class="community-choice-card__photo" loading="lazy" decoding="async" />
            </div>
            <p class="community-choice-card__name">Squeezie</p>
            <p class="community-choice-card__role">YouTuber & Streamer</p>
            <p class="community-choice-card__quote">"When I walk into an event I want my presence felt before I speak. Scandinavian cool layered over South African wild — that's the statement."</p>
            <div class="community-choice-card__combo">
              <div class="community-choice-card__layer">
                <span style="color:${r.find(e=>e.id===`scandinavian`)?.color};">Scandinavian — Spray</span>
                <span class="community-choice-card__amount">3 sprays</span>
              </div>
              <div class="community-choice-card__layer">
                <span style="color:${r.find(e=>e.id===`southafrica`)?.color};">South Africa — Oil</span>
                <span class="community-choice-card__amount">2 drops</span>
              </div>
            </div>
          </div>
          <div class="community-choice-card">
            <div class="community-choice-card__photo-wrap">
              <img src="${rt}" alt="Stéphanie" class="community-choice-card__photo" loading="lazy" decoding="async" />
            </div>
            <p class="community-choice-card__name">Stéphanie</p>
            <p class="community-choice-card__role">Content Creator & Founder</p>
            <p class="community-choice-card__quote">"I tested every combo for three weeks straight. East Asia cherry blossom with South American tonka is the most joyful, delicious thing I've ever worn — no notes."</p>
            <div class="community-choice-card__combo">
              <div class="community-choice-card__layer">
              ...
              <div class="community-choice-card__layer">
                <span style="color:${r.find(e=>e.id===`southamerica`)?.color};">South America — Oil</span>
                <span class="community-choice-card__amount">2 drops</span>
              </div>
            </div>
          </div>
    ...
    `,st(),k(n,i)}function te(e){let t=[...i.map(e=>({id:e.id,label:`${r.find(t=>t.id===e.region)?.icon||``} ${e.name} (${e.format})`,group:`Mon Accord`})),...o.map(e=>({id:e.id,label:`${e.brand} — ${e.name}`,group:`L'Oreal Luxe`}))];return`
  ...
  `:`<p style="font-size:var(--text-sm);color:var(--text-tertiary);padding:var(--space-lg);">No suggestion available.</p>`}function st(){if(document.getElementById(`community-styles`))return;let e=document.createElement(`style`);e.id=`community-styles`,e.textContent=`
    ...
    .community-panel-title { font-size: var(--text-2xl); font-weight: 600; }
  ...
  `,document.head.appendChild(e)}var ct=`/mon_accord/assets/Sephora-Logo-0PIDe1Np.png`,lt=`/mon_accord/assets/Boyner_Logo-CbMLtvDJ.webp`,ut=`/mon_accord/assets/Trendyol_logo-CfMQtlYm.png`,dt=`/mon_accord/assets/hepsiburada-logo-I_Ul_zcU.png`,ft={scandinavian:new URL(`/mon_accord/assets/scandinavian-DvtLirp4.webp`,``+import.meta.url).href,eastasia:new URL(`/mon_accord/assets/east_asia-UzXNmAB4.webp`,``+import.meta.url).href,southafrica:new URL(`/mon_accord/assets/south_africa-PFpjigF9.webp`,``+import.meta.url).href,mediterranean:new URL(`/mon_accord/assets/mediterranean-CV0fuGZk.webp`,``+import.meta.url).href,southamerica:new URL(`/mon_accord/assets/south_america-DyzbduD3.webp`,``+import.meta.url).href,middleeast:new URL(`/mon_accord/assets/middle_east-Bmlf8LlT.webp`,``+import.meta.url).href},pt={scandinavian:new URL(`/mon_accord/assets/scandinavian-BBoj4W5D.webp`,``+import.meta.url).href,eastasia:new URL(`/mon_accord/assets/east_asia-D93RVLet.webp`,``+import.meta.url).href,southafrica:new URL(`/mon_accord/assets/south_africa-hdHFYwXs.webp`,``+import.meta.url).href,mediterranean:new URL(`/mon_accord/assets/mediterranean-B_fMAvaC.webp`,``+import.meta.url).href,southamerica:new URL(`/mon_accord/assets/south_america-DcR51yl4.webp`,``+import.meta.url).href,middleeast:new URL(`/mon_accord/assets/middle_east-DMI8-64c.webp`,``+import.meta.url).href};function mt(e,n){let a=t.getShopCart(),o=[...new Set(t.consumePendingShopCart())],s=[...new Set([...a,...o])],c=s.map(e=>({id:e}));if(o.length){let e=s.length-a.length;window.showToast(`Added ${e} new item${e===1?``:`s`} from your recommended combination.`)}u();function u(){t.setShopCart(c.map(e=>e.id))}function d(){let a=t.getOwnedPerfumes().monAccord||[];ht(a,c),e.innerHTML=`
      <div class="page__container">
                  ...
                  <div class="shop-region-card__products">
                    ${t?(()=>{let n=c.find(e=>e.id===t.id),r=a.includes(t.id);return`
                        <button class="shop-bottle-btn ${n?`shop-bottle-btn--in-cart`:``} ${r?`shop-bottle-btn--owned`:``}" data-id="${t.id}" style="--region-color:${e.color};">
                          ...
                          <img src="${ft[e.id]}" class="shop-bottle-img" alt="Spray" loading="lazy" decoding="async" />
                    ...
                    ${n?(()=>{let t=c.find(e=>e.id===n.id),r=a.includes(n.id);return`
                        <button class="shop-bottle-btn ${t?`shop-bottle-btn--in-cart`:``} ${r?`shop-bottle-btn--owned`:``}" data-id="${n.id}" style="--region-color:${e.color};">
                          ...
                          ${r?`<div class="shop-bottle-owned">Owned</div>`:``}
                          <img src="${pt[e.id]}" class="shop-bottle-img" alt="Oil" loading="lazy" decoding="async" />
    ...
    `,_t(),e.querySelectorAll(`.shop-bottle-btn`).forEach(e=>{e.addEventListener(`click`,t=>{t.stopPropagation();let n=e.dataset.id,r=c.findIndex(e=>e.id===n);r>=0?c.splice(r,1):c.push({id:n}),u(),d()})}),e.querySelectorAll(`.shop-cart__item-remove`).forEach(e=>{e.addEventListener(`click`,()=>{c=c.filter(t=>t.id!==e.dataset.id),u(),d()})});let o=e.querySelector(`#btn-confirm-order`);o&&o.addEventListener(`click`,()=>{gt(c,e,n)})}d()}function ht(e,t){if(!e.length)return null;let n=t.map(e=>e.id),r=e.map(e=>l(e)).filter(Boolean),a=[...new Set(r.flatMap(e=>e.scentFamily.split(`-`)))],o=i.filter(t=>!e.includes(t.id)&&!n.includes(t.id)&&t.scentFamily.split(`-`).some(e=>a.includes(e))).slice(0,2);return o.length?`Based on your owned ${r.map(e=>e.name).join(` & `)}: try adding ${o.map(e=>e.name).join(` or `)}.`:null}function gt(e,n,r){t.clearShopCart();let i=document.createElement(`div`);i.className=`modal-overlay`,i.innerHTML=`
        ...
        <div class="order-retailers">
          <div class="order-retailers__grid">
            <div class="order-retailer-card" aria-label="Sephora">
              <img src="${ct}" alt="Sephora" class="order-retailer-card__logo" loading="lazy" decoding="async" />
            </div>
            <div class="order-retailer-card" aria-label="Boyner">
              <img src="${lt}" alt="Boyner" class="order-retailer-card__logo" loading="lazy" decoding="async" />
            </div>
            <div class="order-retailer-card" aria-label="Trendyol">
              <img src="${ut}" alt="Trendyol" class="order-retailer-card__logo" loading="lazy" decoding="async" />
            </div>
            <div class="order-retailer-card" aria-label="Hepsiburada">
              <img src="${dt}" alt="Hepsiburada" class="order-retailer-card__logo" loading="lazy" decoding="async" />
            </div>
          </div>
        </div>
  ...
  `,document.head.appendChild(e)}function N(e){return new Promise(t=>setTimeout(t,e))}function P(e,t){if(!e)return N(0);var n=t===void 0?90:t,r=e.getBoundingClientRect().top+window.scrollY-n;return window.scrollTo({top:Math.max(0,r),behavior:`smooth`}),N(700)}function vt(e,t,n){return Math.min(n,Math.max(t,e))}function yt(e){let t={};for(let n=0;n<e.length;n+=1){let r=e.key(n);r&&(t[r]=e.getItem(r))}return t}function bt(e,t){e.clear(),Object.entries(t||{}).forEach(([t,n])=>{n!=null&&e.setItem(t,n)})}async function xt(){let e=document.createElement(`div`);e.id=`dt-flash`,e.style.cssText=`
    position:fixed;inset:0;z-index:99994;
    background:rgba(200,169,126,0.07);
    pointer-events:none;opacity:0;
    transition:opacity 0.18s ease;
  `,document.body.appendChild(e),requestAnimationFrame(()=>requestAnimationFrame(()=>{e.style.opacity=`1`})),await N(210),e.style.opacity=`0`,setTimeout(()=>e.remove(),230)}async function F(e){window.location.hash!==e&&xt(),window.location.hash===e&&(window.location.hash=`#_nav`,await N(150)),window.location.hash=e,await N(450)}function I(e,t,n=2e3){return new Promise(r=>{let i=()=>document.querySelector(e)||(t?document.querySelector(t):null),a=i();if(a){r(a);return}let o=new MutationObserver(()=>{let e=i();e&&(o.disconnect(),clearTimeout(s),r(e))});o.observe(document.body,{childList:!0,subtree:!0});let s=setTimeout(()=>{o.disconnect(),r(document.getElementById(`page-content`)||document.body)},n)})}var St={username:`Alp`,archetypeName:`The Elegant Minimalist`,description:`You are drawn to understated sophistication - scents that whisper rather than shout. Your olfactory identity is defined by clean precision, refined florals, and the quiet depth of woody bases. You seek harmony, not performance.`,primaryFamilies:[`floral`,`woody`,`citrus`],sillageProfile:`Medium - intimate yet memorable, discovered only in close proximity.`,notePreferences:{loves:[`iris`,`sandalwood`,`bergamot`,`white musk`],explore:[`oud`,`rose`,`vetiver`,`ambergris`],avoid:[`heavy synthetics`,`excessive sweetness`,`dense patchouli`]},recommendedRegions:[`scandinavian`,`mediterranean`,`eastasia`],updatedAt:Date.now()},Ct=`monaccord_profile`,wt=`monaccord_quiz_state`,L=``,R=null,z=null,B=void 0,V=[{title:`Mon Accord`,run:async()=>(await F(`#landing`),window.scrollTo({top:0,behavior:`auto`}),await I(`#hero-section .hero__content`,`#hero-section`,2e3)),duration:3500},{title:`Six World Regions`,run:async()=>{let e=document.querySelector(`#regions-section`);return e&&await P(e,60),document.querySelector(`.region-card`)||e||document.querySelector(`.page__container`)},duration:4e3},{title:`Spray & Oil Notes`,run:async()=>(document.querySelector(`.notes-popup`)?.remove(),document.querySelector(`.region-format-btn[data-format="spray"]`)?.click(),await N(250),document.querySelector(`.notes-popup`)?.scrollIntoView({behavior:`smooth`,block:`nearest`}),await N(300),document.querySelector(`.notes-popup__inner`)||document.querySelector(`#regions-section`)),duration:4e3},{title:`Take the Quiz`,samePage:!0,run:async()=>{document.querySelector(`.notes-popup`)?.remove();let e=document.querySelector(`#cta-section`);e?e.scrollIntoView({behavior:`smooth`,block:`center`}):window.scrollTo({top:document.body.scrollHeight,behavior:`smooth`}),await N(700);let t=document.querySelector(`#bottom-cta`);return t&&(t.scrollIntoView({behavior:`smooth`,block:`center`}),await N(500)),t||document.querySelector(`#cta-section`)||document.querySelector(`.page__container`)},duration:3e3},{title:`Step 1 - Your Name`,run:async()=>{document.querySelector(`.notes-popup`)?.remove(),localStorage.removeItem(wt),window.__retakeQuiz=!0,await F(`#profile`);let e=await I(`.quiz-container`,`.page__container`,2500);await N(300);let t=document.querySelector(`#quiz-username`);if(t){t.focus();for(let e of`Alp`)t.value+=e,t.dispatchEvent(new Event(`input`)),await N(120)}return e},duration:2500},{title:`Step 2 - Scent Families`,run:async()=>{document.querySelector(`#quiz-next`)?.click(),await N(400);let e=[...document.querySelectorAll(`.quiz-grid--families .quiz-option`)];for(let t of e.slice(0,3))t.click(),await N(280);return document.querySelector(`.quiz-grid--families`)||document.querySelector(`.quiz-container`)},duration:2500},{title:`Step 3 - Perfumes You Love`,run:async()=>{document.querySelector(`#quiz-next`)?.click(),await N(450);let e=[...document.querySelectorAll(`.quiz-option--perfume`)];for(let t of e.slice(0,2))t.click(),await N(220);return document.querySelector(`.quiz-perfume-list`)||document.querySelector(`.quiz-container`)},duration:2500},{title:`Step 4 - Performance`,run:async()=>{document.querySelector(`#quiz-next`)?.click(),await N(400);for(let e of[{id:`slider-sillage`,valId:`sillage-val`,target:7},{id:`slider-longevity`,valId:`longevity-val`,target:10},{id:`slider-intensity`,valId:`intensity-val`,target:5}]){let t=document.querySelector(`#`+e.id);t&&(t.value=e.target,t.dispatchEvent(new Event(`input`,{bubbles:!0})),await N(350))}return document.querySelector(`.quiz-sliders`)||document.querySelector(`.quiz-container`)},duration:2500},{title:`Step 5 - Your Personality`,run:async()=>{document.querySelector(`#quiz-next`)?.click(),await N(400),(document.querySelector(`[id="ctx-elegant"]`)||document.querySelector(`.quiz-grid--context .quiz-option`))?.click(),await N(300);let e=document.querySelector(`#quiz-notes`);if(e){e.focus();for(let t of`I love the scent of fresh linen and evening air.`)e.value+=t,e.dispatchEvent(new Event(`input`)),await N(38)}return document.querySelector(`.quiz-grid--context`)||document.querySelector(`.quiz-container`)},duration:5500},{title:`Your Olfactory Profile`,run:async()=>(localStorage.setItem(Ct,JSON.stringify(St)),localStorage.setItem(`monaccord_my_perfumes`,JSON.stringify({monAccord:[`scandinavian-spray`,`eastasia-spray`,`mediterranean-oil`],loreal:[`ysl-libre`,`ysl-black-opium`]})),window.__retakeQuiz=!1,window.dispatchEvent(new Event(`hashchange`)),await N(500),window.scrollTo({top:0,behavior:`auto`}),await I(`.profile-result`,`.page__container`,3e3),document.querySelector(`.section-header`)||document.querySelector(`.profile-result`)||document.querySelector(`.page__container`)),duration:4e3},{title:`Identity & Preferences`,run:async()=>{let e=document.querySelector(`.profile-overview`),t=document.querySelector(`.profile-details`)||e;return await P(e),t||document.querySelector(`.profile-result`)},duration:4e3},{title:`Mon Accord Combinations`,run:async()=>{let e=document.querySelectorAll(`.recommendation-grid`)[0],t=e?.previousElementSibling?.previousElementSibling||e,n=e?.querySelector(`.combo-card`)||e;return await P(t||n,80),n||document.querySelector(`.profile-result`)},duration:4e3},{title:`L'Oreal Luxe Combinations`,run:async()=>{let e=document.querySelectorAll(`.recommendation-grid`),t=e.length>1?e[1]:e[0],n=t?.previousElementSibling?.previousElementSibling||t,r=t?.querySelector(`.combo-card`)||t;return await P(n||r,80),r||document.querySelector(`.profile-result`)},duration:4e3},{title:`Shop - Build Your Collection`,run:async()=>{let e=document.querySelector(`[data-buy-combo]`);return e?(e.click(),await N(600)):await F(`#shop`),await I(`.shop-region-card`,`.shop-layout`,2e3)},duration:4e3},{title:`Add to Cart`,run:async()=>{let e=[...document.querySelectorAll(`.shop-product__btn:not(.shop-product__btn--added)`)];e[0]&&(e[0].click(),await N(180)),e[1]&&(e[1].click(),await N(180));let t=document.querySelector(`.shop-cart`);return t?.scrollIntoView({behavior:`smooth`,block:`center`}),await N(300),t||document.querySelector(`.page__container`)},duration:4e3},{title:`Now Available`,run:async()=>(document.querySelector(`#btn-confirm-order`)?.click(),await N(350),document.querySelector(`.order-confirmed-modal`)||document.querySelector(`.modal-overlay`)||document.querySelector(`.page__container`)),duration:4e3},{title:`The Layering Lab`,run:async()=>{document.querySelector(`.modal-overlay`)?.remove(),await F(`#lab`),window.scrollTo({top:0,behavior:`auto`}),await N(200);let e=[...document.querySelectorAll(`.lab-add-btn:not(.lab-add-btn--added)`)];return e[0]&&(e[0].click(),await N(200)),e[2]&&(e[2].click(),await N(200)),await N(300),document.querySelector(`#lab-layers`)||document.querySelector(`#lab-add-section`)},duration:5e3},{title:`Virtual Scent Simulation`,samePage:!0,run:async()=>{let e=document.querySelector(`#btn-simulate`);e&&(await P(e),e.innerHTML=`<span class=”loading-spinner”></span> Simulating...`,e.disabled=!0,await N(1200),e.innerHTML=`✦ Simulate Scent`,e.disabled=!1);let t=document.querySelector(`#simulation-result`);if(!t){t=document.createElement(`div`),t.id=`simulation-result`,t.className=`ai-response mt-lg`;let e=document.querySelector(`#lab-actions`);e?e.insertAdjacentElement(`afterend`,t):document.querySelector(`#lab-layers`)?.appendChild(t)}return t&&(t.style.display=``,t.innerHTML=`
        ...
        `,await P(t)),t||document.querySelector(`#lab-layers`)||document.querySelector(`.page__container`)},duration:4500},{title:`Contextual Advisor`,run:async()=>{let e=document.querySelector(`.lab-advisor`);await P(e),document.querySelector(`#mood-chips .lab-chip`)?.click(),await N(100),document.querySelector(`#occasion-chips .lab-chip`)?.click(),await N(100),document.querySelector(`#season-chips .lab-chip`)?.click(),await N(250);let t=document.querySelector(`#advisor-result`);return t||(t=document.createElement(`div`),t.id=`advisor-result`,t.className=`lab-advisor__result mt-lg`,document.querySelector(`.lab-advisor__form`)?.insertAdjacentElement(`afterend`,t)),t&&(t.style.display=``,t.innerHTML=`
        ...
        `),t||document.querySelector(`.lab-advisor__form`)||e||document.querySelector(`.page__container`)},duration:5e3},{title:`Save to Vault`,samePage:!0,run:async()=>{let e=await I(`#btn-save-formula`,`.page__container`,1200);e&&e.id===`btn-save-formula`&&(await P(e),e.click(),await N(450));let t=await I(`#save-vault-overlay .modal`,`#save-vault-overlay`,2e3),n=document.querySelector(`#sv-name`);if(n){n.value=``,n.focus();for(let e of`Evening Velvet`)n.value+=e,n.dispatchEvent(new Event(`input`)),await N(55);await N(300)}let r=document.querySelector(`#sv-folder-dropdown`);if(r){let e=[...r.options].find(e=>e.value===`evening`||e.text.toLowerCase().includes(`evening`));e&&(r.value=e.value,r.dispatchEvent(new Event(`change`))),await N(400)}return t||document.querySelector(`#save-vault-overlay`)||document.querySelector(`.page__container`)},duration:4500},{title:`Your Vault`,run:async()=>(document.querySelector(`#sv-confirm`)?.click(),await N(350),await F(`#vault`),await I(`.vault-folders-col`,`.page__container`,2e3)),duration:4e3},{title:`My Perfumes`,samePage:!0,run:async()=>{let e=document.querySelector(`.vault-myperfumes-col`);return window.scrollTo({top:0,behavior:`smooth`}),await N(400),e||document.querySelector(`.vault-main-layout`)||document.querySelector(`.page__container`)},duration:4e3},{title:`Trending Combinations`,run:async()=>{await F(`#community`);let e=document.querySelector(`.duygu-card__photo`);return e&&!e.complete&&await new Promise(t=>{e.onload=t,e.onerror=t,setTimeout(t,3e3)}),await N(200),await I(`.community-trending-item`,`.community-trending-list`,2e3)},duration:4e3},{title:`Today's Selection`,samePage:!0,run:async()=>{let e=document.querySelector(`.community-panel--selection`);return await P(e),e||document.querySelector(`.community-top-grid`)||document.querySelector(`.page__container`)},duration:4e3},{title:`Duygu's Choice`,samePage:!0,run:async()=>{let e=document.querySelector(`.community-duygu-choice`);return e?(await P(e),e):document.querySelector(`.community-right-col`)||document.querySelector(`.page__container`)},duration:4e3},{title:`Community Discussion`,samePage:!0,run:async()=>{let e=document.querySelector(`.community-discussion-section`);return await P(e,60),document.querySelector(`.community-post-item`)?.click(),await N(300),document.querySelector(`.community-discussion-layout`)||e||document.querySelector(`.page__container`)},duration:4500}],Tt=[3e3,3500,3500,3e3,2500,2500,2500,2500,5e3,4500,3e3,4e3,3e3,3500,3500,3500,4500,4500,4e3,3500,3500,3500,3500,3e3,3500,3500];V.forEach((e,t)=>{e.duration=Tt[t]??e.duration});function Et(){return document.getElementById(`dt-card`)}function Dt(){return document.getElementById(`dt-prog`)}function Ot(){return document.getElementById(`dt-cd`)}function kt(){return document.getElementById(`dt-dots`)}function At(){return document.getElementById(`dt-count`)}function jt(){return document.getElementById(`dt-title`)}function Mt(){return document.getElementById(`dt-next`)}var Nt=24,H=20;function Pt(e){let t=Et();if(!t)return;let n=window.innerWidth,r=window.innerHeight,i=Nt,a=Math.min(t.offsetWidth||288,n-i*2),o=t.offsetHeight||126,s=e?e.getBoundingClientRect():null;if(!s||(s.width||0)===0||(s.height||0)===0||s.bottom<=0||s.top>=r||s.right<=0||s.left>=n){let e=Math.min(n-i*2,320),t=Math.min(r-i*2,160);s={left:(n-e)/2,top:(r-t)/2,right:(n+e)/2,bottom:(r+t)/2}}let c=s.right-s.left,l=s.bottom-s.top,u=[{placement:`bottom`,x:s.left+(c-a)/2,y:s.bottom+H},{placement:`top`,x:s.left+(c-a)/2,y:s.top-o-H},{placement:`right`,x:s.right+H,y:s.top+(l-o)/2},{placement:`left`,x:s.left-a-H,y:s.top+(l-o)/2},{placement:`bottom-right`,x:s.right-a,y:s.bottom+H},{placement:`bottom-left`,x:s.left,y:s.bottom+H},{placement:`top-right`,x:s.right-a,y:s.top-o-H},{placement:`top-left`,x:s.left,y:s.top-o-H}];function d(e){let t=vt(e.x,i,Math.max(i,n-a-i)),s=vt(e.y,i,Math.max(i,r-o-i));return{placement:e.placement,left:t,top:s,right:t+a,bottom:s+o,clampDelta:Math.abs(e.x-t)+Math.abs(e.y-s)}}function f(e){let t=(e.left+e.right)/2,n=(e.top+e.bottom)/2,r=(s.left+s.right)/2,i=(s.top+s.bottom)/2;return Math.hypot(t-r,n-i)}function p(e){let t=Math.max(e.left,s.left),n=Math.max(e.top,s.top),r=Math.min(e.right,s.right),i=Math.min(e.bottom,s.bottom);return r>t&&i>n?(r-t)*(i-n):0}let m=u.map(e=>{let t=d(e);return{rect:t,score:p(t)*4+f(t)+t.clampDelta*6}}).sort((e,t)=>e.score-t.score)[0];t.dataset.placement=m.rect.placement,t.style.transform=`translate(${Math.round(m.rect.left)}px, ${Math.round(m.rect.top)}px)`}function Ft(e){let t=V[e],n=V.length,r=At(),i=jt(),a=kt(),o=Dt(),s=Mt();if(r&&(r.textContent=`${String(e+1).padStart(2,`0`)} / ${String(n).padStart(2,`0`)}`),i&&(i.textContent=t.title),o&&(o.style.width=`${(e+1)/n*100}%`),s&&(s.textContent=e===n-1?`Finish`:`Next`),a){let t=Math.min(n,12),r=Math.max(0,e-Math.floor(t/2));a.innerHTML=Array.from({length:t},(t,n)=>{let i=n+r;return`<span class="dt-dot ${i===e?`dt-dot--active`:i<e?`dt-dot--done`:`dt-dot--pending`}"></span>`}).join(``)}}var U=null;function It(e){clearInterval(U);let t=Ot();if(!t)return;t.style.width=`0%`;let n=Date.now();U=setInterval(()=>{let r=Math.min(100,(Date.now()-n)/e*100);t.style.width=r+`%`,r>=100&&clearInterval(U)},40)}var W=!1,G=0,K=null;async function Lt(e){if(!W)return;G=e;let t=V[e];Ft(e);let n;try{n=await t.run()}catch{n=document.getElementById(`page-content`)||document.body}W&&(Pt(n),It(t.duration),clearTimeout(K),K=setTimeout(()=>Rt(),t.duration))}function Rt(){W&&(clearTimeout(K),G>=V.length-1?q():Lt(G+1))}function zt(){Bt(),Ht();let e=document.createElement(`div`);e.id=`dt-card`,e.innerHTML=`
  ...
  `,document.body.appendChild(e),document.getElementById(`dt-skip`).addEventListener(`click`,q),document.getElementById(`dt-next`).addEventListener(`click`,()=>{clearTimeout(K),clearInterval(U),Rt()})}function Bt(){document.getElementById(`dt-card`)?.remove(),document.getElementById(`dt-flash`)?.remove()}function Vt(){W&&q(),L=window.location.hash||``,R=yt(localStorage),z=yt(sessionStorage),B=window.__retakeQuiz,localStorage.clear(),sessionStorage.clear(),delete window.__retakeQuiz,localStorage.removeItem(Ct),W=!0,G=0,zt(),Lt(0)}function q(){W=!1,clearTimeout(K),clearInterval(U),bt(localStorage,R),bt(sessionStorage,z),R=null,z=null,B===void 0?delete window.__retakeQuiz:window.__retakeQuiz=B,B=void 0,(L||``)!==(window.location.hash||``)&&(window.location.hash=L||`#landing`),L=``,Bt()}function Ht(){if(document.getElementById(`dt-styles`))return;let e=document.createElement(`style`);e.id=`dt-styles`,e.textContent=`
    #dt-card {
      position: fixed;
      z-index: 99995;
      top: 0;
  ...
  `,document.head.appendChild(e)}function J(e){return new Promise(t=>setTimeout(t,e))}function Y(){if(document.getElementById(`pd-styles`))return;let e=document.createElement(`style`);e.id=`pd-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e)}function X(e){(Array.isArray(e)?e:[e]).forEach(e=>{e.classList.remove(`pd-highlight--out`),e.classList.add(`pd-highlight`)})}async function Z(e){let t=Array.isArray(e)?e:[e];t.forEach(e=>{e.classList.remove(`pd-highlight`),e.classList.add(`pd-highlight--out`)}),await J(900),t.forEach(e=>e.classList.remove(`pd-highlight--out`))}function Ut(e){e.classList.remove(`pd-highlight-inner--out`),e.classList.add(`pd-highlight-inner`)}async function Wt(e){e.classList.remove(`pd-highlight-inner`),e.classList.add(`pd-highlight-inner--out`),await J(900),e.classList.remove(`pd-highlight-inner--out`)}async function Gt(){Y();let e=document.querySelector(`.profile-hero`),t=document.querySelector(`.profile-summary-card`);if(!e||!t)return;function n(e,t=1.05){e.style.transition=`transform 0.7s cubic-bezier(0.25,0.46,0.45,0.94)`,e.style.transformOrigin=`top center`,e.style.transform=`scale(${t})`}function r(e){e.style.transform=`scale(1)`}async function i(e){await J(600),e.style.transform=``,e.style.transition=``,e.style.transformOrigin=``}window.scrollTo({top:0,behavior:`auto`}),await J(2e3),t.scrollIntoView({behavior:`smooth`,block:`nearest`}),await J(500),n(t,1.04),X(t),await J(3e3),r(t),await Z(t),await i(t);let a=document.querySelectorAll(`.recommendation-grid`),o=a[0];if(!o)return;let s=o.closest(`.mt-xl`)||o.parentElement,c=(s||o).getBoundingClientRect().top+window.scrollY-80;window.scrollTo({top:Math.max(0,c),behavior:`smooth`}),await J(700),s&&(n(s,1.03),X(s),await J(3e3),r(s),await Z(s),await i(s));let l=a[1]?.closest(`.mt-xl`)||a[1]?.parentElement;if(l){let e=l.getBoundingClientRect().top+window.scrollY-80;window.scrollTo({top:Math.max(0,e),behavior:`smooth`}),await J(500),n(l,1.03),X(l),await J(3e3),r(l),await Z(l),await i(l)}}async function Kt(){Y(),window.scrollTo({top:0,behavior:`auto`}),await J(2e3);let e=document.querySelector(`.lab-advisor`);if(e){let t=e.getBoundingClientRect().top+window.scrollY-90;window.scrollTo({top:Math.max(0,t),behavior:`smooth`})}await J(700);for(let[e,t]of[[`#mood-chips`,`romantic`],[`#occasion-chips`,`date-night`],[`#season-chips`,`autumn`],[`#time-chips`,`evening`]])document.querySelector(`${e} [data-value="${t}"]`)?.click(),await J(450);let t=document.querySelector(`#advisor-intensity`);t&&(t.value=7,t.dispatchEvent(new Event(`input`,{bubbles:!0}))),await J(500);let n=document.querySelector(`#btn-get-advice`);n&&(n.innerHTML=`<span class="loading-spinner"></span> Crafting...`,n.disabled=!0),await J(1100),window.__labSetDemoContext?.({formulaName:`Evening Accord`,reasoning:`A confident, romantic blend for a date night — woody base anchors the floral heart.`,scentPreview:`Opens with bergamot, blooms into iris, settles into warm sandalwood.`,tips:`Apply to pulse points for intimate sillage.`,layers:[{perfumeId:`scandinavian-spray`,amount:3,unit:`sprays`},{perfumeId:`mediterranean-spray`,amount:2,unit:`sprays`},{perfumeId:`eastasia-oil`,amount:2,unit:`drops`}]}),await J(150);let r=document.querySelector(`#advisor-result`);if(r){let e=r.getBoundingClientRect().top+window.scrollY-90;window.scrollTo({top:Math.max(0,e),behavior:`smooth`}),await J(700),X(r),await J(1500),await Z(r),await J(300),document.querySelector(`#btn-apply-recommendation`)?.click(),await J(500)}await J(300);let i=document.querySelector(`.lab-canvas`);i&&(i.scrollIntoView({behavior:`smooth`,block:`start`}),await J(600),X(i),await J(1e3),await Z(i));let a=document.querySelector(`#btn-simulate`);a&&(a.innerHTML=`<span class="loading-spinner"></span> Simulating...`,a.disabled=!0),await J(1e3);let o=document.querySelector(`#simulation-result`),s=document.querySelector(`#lab-actions`);!o&&s&&(o=document.createElement(`div`),o.id=`simulation-result`,o.className=`ai-response mt-lg`,o.style.margin=`0 6px`,s.insertAdjacentElement(`afterend`,o)),o&&(o.innerHTML=`
      <div class="ai-response__label">✦ Virtual Scent Simulation</div>
      <div class="ai-response__text">
    ...
    `),a&&(a.innerHTML=`✦ Simulate Scent`,a.disabled=!1),await J(200),o&&(i?i.scrollTo({top:i.scrollHeight,behavior:`smooth`}):o.scrollIntoView({behavior:`smooth`,block:`center`}),await J(700),o.style.margin=`0 6px`,Ut(o),await J(1800),await Wt(o)),await J(400),document.querySelector(`#btn-clear-layers`)?.click(),await J(500);let c=document.querySelector(`#lab-add-section`);if(c){let e=c.getBoundingClientRect().top+window.scrollY-80;window.scrollTo({top:Math.max(0,e),behavior:`smooth`}),await J(700);for(let e of[`middleeast-spray`,`southamerica-spray`,`southafrica-oil`]){let t=document.querySelector(`.lab-bottle-btn[data-id="${e}"]`);t&&(t.click(),await J(400))}}}async function qt(){Y(),window.__vaultForceReseed?.(),await J(200),window.scrollTo({top:0,behavior:`auto`}),await J(2e3);let e=document.querySelector(`.vault-folders-grid`);e&&(window.scrollTo({top:0,behavior:`smooth`}),await J(500),e.style.transition=`transform 0.7s cubic-bezier(0.25,0.46,0.45,0.94)`,e.style.transformOrigin=`top center`,e.style.transform=`scale(1.06)`,X(e),await J(3e3),e.style.transform=`scale(1)`,await Z(e),await J(600),e.style.transform=``,e.style.transition=``,e.style.transformOrigin=``);let t=document.querySelector(`.vault-myperfumes-col`);t&&(t.scrollIntoView({behavior:`smooth`,block:`center`}),await J(600),t.style.transition=`transform 0.7s cubic-bezier(0.25,0.46,0.45,0.94)`,t.style.transformOrigin=`top center`,t.style.transform=`scale(1.04)`,X(t),await J(3e3),t.style.transform=`scale(1)`,await Z(t),await J(600),t.style.transform=``,t.style.transition=``,t.style.transformOrigin=``)}async function Jt(){Y(),window.scrollTo({top:0,behavior:`auto`}),await J(2e3);function e(e,t=1.05){e.style.transition=`transform 0.7s cubic-bezier(0.25,0.46,0.45,0.94)`,e.style.transformOrigin=`top center`,e.style.transform=`scale(${t})`}function t(e){e.style.transform=`scale(1)`}async function n(e){await J(600),e.style.transform=``,e.style.transition=``,e.style.transformOrigin=``}let r=document.querySelector(`.community-panel:not(.community-panel--selection)`);r&&(r.scrollIntoView({behavior:`smooth`,block:`nearest`}),window.scrollTo({top:0,behavior:`smooth`}),await J(500),e(r,1.04),X(r),await J(3e3),t(r),await Z(r),await n(r));let i=document.querySelector(`.community-panel--selection`);i&&(i.scrollIntoView({behavior:`smooth`,block:`nearest`}),await J(500),e(i,1.04),X(i),await J(3e3),t(i),await Z(i),await n(i));let a=document.querySelector(`.community-choice-row`);a&&(a.scrollIntoView({behavior:`smooth`,block:`center`}),await J(600),e(a,1.03),X(a),await J(3e3),t(a),await Z(a),await n(a));let o=document.querySelector(`.community-discussion-section`);if(o){o.scrollIntoView({behavior:`smooth`,block:`end`}),await J(600);let e=[...document.querySelectorAll(`.community-post-item`)];e.length&&(e[Math.floor(Math.random()*Math.min(e.length,5))].click(),await J(400)),await J(4e3)}}var Yt={"#profile":Gt,"#lab":Kt,"#vault":qt,"#community":Jt};function Xt(e){return e in Yt}function Zt(e){let t=Yt[e];if(!t){window.showToast?.(`Bu sayfa için demo mevcut değil.`,`info`);return}t().catch(e=>console.error(`[page-demo]`,e))}var Qt=document.getElementById(`app`);function Q(e){document.documentElement.setAttribute(`data-theme`,e),t.set(`theme`,e)}function $t(){let e=t.get(`theme`,`dark`);document.documentElement.setAttribute(`data-theme`,e)}var en={"":f,"#landing":f,"#profile":fe,"#lab":Re,"#vault":Ye,"#shop":mt,"#community":at};function tn(){return window.location.hash||``}function nn(e){window.location.hash=e}function $(){let e=tn(),t=en[e]||f;Qt.innerHTML=``,Qt.appendChild(n(nn,e));let r=document.createElement(`div`);if(r.className=`page page-enter`,r.id=`page-content`,t(r,nn),Qt.appendChild(r),!document.querySelector(`.toast-container`)){let e=document.createElement(`div`);e.className=`toast-container`,e.id=`toast-container`,document.body.appendChild(e)}window.scrollTo({top:0,behavior:`auto`})}window.showToast=function(e,t=`success`){let n=document.getElementById(`toast-container`);if(!n)return;let r=document.createElement(`div`);r.className=`toast toast--${t}`,r.innerHTML=`<span class="toast__message">${e}</span>`,n.appendChild(r),setTimeout(()=>{r.style.animation=`fadeOut 0.3s var(--ease-out) forwards`,setTimeout(()=>r.remove(),300)},3e3)},window.showSettings=function(){let e=document.querySelector(`.modal-overlay`);e&&e.remove();let n=t.get(`theme`,`dark`),r=document.createElement(`div`);r.className=`modal-overlay`,r.id=`settings-modal`,r.innerHTML=`
    <div class="modal">
      <div class="modal__header">
      ...
      <div class="modal__body">
        ...
        <div class="input-group" style="margin-top:var(--space-lg); padding-top:var(--space-lg); border-top:1px solid var(--border);">
          ...
          <div style="display:flex; flex-direction:column; gap:var(--space-sm);">
            ...
            ${Xt(tn())?`<button class="btn btn--primary btn--sm" id="settings-page-demo-btn" style="margin-top:2px;">▶ Bulunduğun Sayfanın Demosunu Başlat</button>`:``}
          </div>
        </div>
