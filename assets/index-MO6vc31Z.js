...
Be poetic but precise. Help the reader "smell" this blend through words. Keep it under 200 words total.`)}var Sd=5;function Cd(e,t){let n=d.getProfile();if(n&&!window.__retakeQuiz){Md(e,n,t);return}window.__retakeQuiz=!1;let r=d.getQuizState(),i=r?.step||1,a=r?.answers||{username:``,scentFamilies:[],knownPerfumes:[],sillage:5,longevity:5,intensity:5,personality:``,notes:``};function o(){d.setQuizState({step:i,answers:a}),e.innerHTML=`
      ...
      <div class="profile-result">
        ...
        <div class="mt-xl">
          ...
          <div class="recommendation-grid">
            ${r.map(e=>kd(e,{showBuyButton:!0})).join(``)}
          </div>
        </div>
  ...
  `,Nd(),e.querySelector(`#go-to-shop`).addEventListener(`click`,()=>n(`#shop`)),e.querySelectorAll(`[data-buy-combo]`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.comboIds.split(`,`).map(e=>e.trim()).filter(Boolean);d.setPendingShopCart(t),n(`#shop`)})}),e.querySelector(`#go-to-lab`).addEventListener(`click`,()=>n(`#lab`)),e.querySelector(`#retake-quiz`).addEventListener(`click`,()=>{window.__retakeQuiz=!0,d.clearQuizState(),Cd(e,n)})}function kd(e,t={}){let n=(e.productIds||[]).join(`,`);return`
  ...
  `,document.head.appendChild(e)}var Pd=[{id:`confident`,name:`Confident`,icon:`💪`,description:`Bold and commanding`},{id:`romantic`,name:`Romantic`,icon:`💕`,description:`Soft and alluring`},{id:`calm`,name:`Calm`,icon:`🧘`,description:`Peaceful and centered`},{id:`energetic`,name:`Energetic`,icon:`⚡`,description:`Vibrant and lively`},{id:`mysterious`,name:`Mysterious`,icon:`🌙`,description:`Enigmatic and deep`},{id:`playful`,name:`Playful`,icon:`✨`,description:`Light and fun`}],Fd=[{id:`office`,name:`Office / Work`,icon:`💼`},{id:`date-night`,name:`Date Night`,icon:`🕯`},{id:`casual`,name:`Casual Outing`,icon:`☀️`},{id:`formal`,name:`Formal Event`,icon:`🎩`},{id:`outdoor`,name:`Outdoor Adventure`,icon:`🏔`},{id:`cozy`,name:`Cozy Night In`,icon:`🛋`}],Id=[{id:`spring`,name:`Spring`,icon:`🌸`},{id:`summer`,name:`Summer`,icon:`☀️`},{id:`autumn`,name:`Autumn`,icon:`🍂`},{id:`winter`,name:`Winter`,icon:`❄️`}],Ld=[{id:`whisper`,name:`Whisper`,value:1,description:`Barely there, intimate`},{id:`soft`,name:`Soft`,value:3,description:`Close range only`},{id:`moderate`,name:`Moderate`,value:5,description:`Noticeable arm's length`},{id:`present`,name:`Present`,value:7,description:`Fills the room gently`},{id:`bold`,name:`Bold`,value:9,description:`Leaves a trail`}];function Rd(){let e=d.getOwnedPerfumes(),t=[];if(e.monAccord?.length){let n=e.monAccord.map(e=>m.find(t=>t.id===e)?.name).filter(Boolean);n.length&&t.push(`Mon Accord owned: ${n.join(`, `)}`)}if(e.loreal?.length){let n=e.loreal.map(e=>g.find(t=>t.id===e)?.name).filter(Boolean);n.length&&t.push(`L'Oréal Luxe owned: ${n.join(`, `)}`)}return t.length?t.join(`
`):null}async function zd(e){let t=d.getProfile(),n=Rd(),r=await md(`Based on the user's context, recommend a Mon Accord layering formula.
...
}`);if(r.success)try{let t=r.text.trim();t.startsWith("```")&&(t=t.replace(/^```(?:json)?\n?/,``).replace(/\n?```$/,``));let n=JSON.parse(t);return d.addInteraction({type:`contextual-recommendation`,context:e,recommendation:n.formulaName}),{success:!0,recommendation:n}}catch(e){return console.error(`Failed to parse recommendation:`,e),{success:!0,recommendation:{formulaName:`AI Recommendation`,layers:[],reasoning:r.text,scentPreview:``,tips:``}}}return{success:!1,error:r.text}}function Bd(e,t={}){let{showNameInput:n=!0,onSaved:r}=t,i=d.get(`vault_folders`,[{id:`default`,name:`All Formulas`,icon:`📁`}]),a=`default`,o=!1;function s(){let t=document.getElementById(`save-vault-overlay`);t&&t.remove();let c=document.createElement(`div`);c.className=`modal-overlay`,c.id=`save-vault-overlay`,c.innerHTML=`
...
`).filter(e=>e.trim()).map(e=>{if(e.match(/^(OPENING|HEART|DRY DOWN|OVERALL|SILLAGE)/i)){let[t,...n]=e.split(`:`);return`<p><strong style="color: var(--accent);">${t.trim()}:</strong> ${n.join(`:`).trim()}</p>`}return`<p>${e}</p>`}).join(``):``}function Yd(){if(document.getElementById(`lab-styles`))return;let e=document.createElement(`style`);e.id=`lab-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e)}var Xd=`/mon_accord/assets/open-folder-BdwKQCux.png`;function Zd(e,t){let n=d.get(`vault_folders`,[{id:`default`,name:`All Formulas`}]).map(e=>({id:e.id,name:e.name})),r=null,i=!1,a=null,o=null;function s(){d.set(`vault_folders`,n)}function c(e){let t=d.getVault();return e===`default`?t:t.filter(t=>t.folderId===e)}function l(){let t=d.getVault();e.innerHTML=`
      <div class="page__container">
        ${r?_(r,t):u(t)}
      </div>
      ${v()}
    `,Qd(),b(t)}function u(e){return`
          ...
          <div class="vault-folders-grid">
            ${n.map(e=>{let t=c(e.id).length;return`
                <div class="vault-folder-card card card--interactive" data-folder="${e.id}">
                  <img src="${Xd}" class="vault-folder-icon" alt="folder" />
...
Respond in EXACTLY this JSON format (no markdown, no code blocks):
{
  ...
  "scentDescription": "Vivid 2-sentence sensory preview"
}`);if(i.success)try{let e=i.text.trim();e.startsWith("```")&&(e=e.replace(/^```(?:json)?\n?/,``).replace(/\n?```$/,``));let t=JSON.parse(e);return d.addInteraction({type:`remix-generated`,name:t.remixName}),{success:!0,remix:t}}catch{return{success:!0,remix:{remixName:`AI Remix`,layers:[],inspiration:i.text,newElement:``,scentDescription:``}}}return{success:!1,error:i.text}}function nf(){let e=d.getInteractions().filter(e=>e.type===`like`&&e.timestamp>Date.now()-720*60*60*1e3),t={};return e.forEach(e=>{t[e.formulaId]=(t[e.formulaId]||0)+1}),_.sort((e,n)=>n.likes+(t[n.id]||0)*10-(e.likes+(t[e.id]||0)*10)).slice(0,5)}var rf=[{id:`sp-1`,author:`ScentExplorer`,date:Date.now()-864e5*3,title:`My go-to date night combo`,topicType:`trending`,topicRef:`cf-4`,content:`Velvet Rose is absolutely stunning for evenings out. The oud-citrus balance is perfect — not too heavy, not too light. My partner always compliments this one!`,likes:18,comments:[{id:`sc-1`,author:`RoseAddict`,date:Date.now()-864e5*2,text:`Totally agree! I add an extra drop of the oil for more longevity.`,replies:[{id:`sc-1r1`,author:`ScentExplorer`,date:Date.now()-864e5,text:`Great tip, I'll try that next time!`}]},{id:`sc-2`,author:`NordicNose`,date:Date.now()-864e5,text:`Have you tried mixing it with a Scandinavian spray? The contrast is amazing.`,replies:[]}]},{id:`sp-2`,author:`MinimalistMusk`,date:Date.now()-864e5*5,title:`Best office-safe layering?`,topicType:`free`,topicLabel:`Office Fragrances`,content:`Looking for recommendations on subtle combinations that work in a professional setting. I love the Zen Garden combo but wondering if there are other options that project softly.`,likes:12,comments:[{id:`sc-3`,author:`CorporateChic`,date:Date.now()-864e5*4,text:`Azure Morning is my daily driver for work. Very clean and inoffensive.`,replies:[{id:`sc-3r1`,author:`MinimalistMusk`,date:Date.now()-864e5*3,text:`Oh that sounds perfect, adding it to my cart!`},{id:`sc-3r2`,author:`FreshFan`,date:Date.now()-864e5*2,text:`Second this. Mediterranean spray + Scandinavian oil is chef's kiss for office.`}]}]},{id:`sp-3`,author:`OudLover`,date:Date.now()-864e5*1,title:`Custom evening blend I created`,topicType:`combination`,topicCombination:[{perfumeId:`middleeast-spray`,amount:3,unit:`sprays`},{perfumeId:`southafrica-oil`,amount:2,unit:`drops`}],content:`I've been experimenting with this bold combo for a while. The saffron-oud meets vanilla-coffee and it's absolutely divine for cold winter nights. Who else loves intense combos?`,likes:24,comments:[]}];function af(){let e=d.getPosts();return e.length||(e=rf,d.setPosts(e)),e}function of(e,t){let n=nf(),r=d.getProfile(),i=`likes`,a=null,o=!1,s=null,c=`community_daily_suggestion`,l=new Date().toISOString().slice(0,10),u=null;try{u=JSON.parse(sessionStorage.getItem(c)||`null`)}catch{}let f=u?.date===l?u.remix:null,h=!f;function v(e){if(!r)return Math.floor(Math.random()*40)+30;let t=50,n=r.primaryFamilies||[];return(e.layers||[]).forEach(e=>{let i=y(e.perfumeId);i&&(i.scentFamily.split(`-`).forEach(e=>{n.includes(e)&&(t+=12)}),r.recommendedRegions?.includes(i.region)&&(t+=8))}),Math.min(t,98)}let b=n.map(e=>({...e,matchPercent:v(e)}));function x(){let e=[...b];return i===`likes`?e.sort((e,t)=>(t.likes||0)-(e.likes||0)):i===`match`?e.sort((e,t)=>t.matchPercent-e.matchPercent):i===`newest`&&e.sort((e,t)=>t.id>e.id?1:-1),e}function S(){let t=x(),n=af(),r=a?n.find(e=>e.id===a):null;e.innerHTML=`
      <div class="page__container">
        <!-- ═══ TRENDING SECTION ═══ -->
    ...
    `,cf(),M(t,n)}function C(e){let t=[...m.map(e=>({id:e.id,label:`${p.find(t=>t.id===e.region)?.icon||``} ${e.name} (${e.format})`,group:`Mon Accord`})),...g.map(e=>({id:e.id,label:`${e.brand} — ${e.name}`,group:`L'Oreal Luxe`}))];return`
  ...
  `:`<p style="font-size:var(--text-sm);color:var(--text-tertiary);padding:var(--space-lg);">No suggestion available.</p>`}function cf(){if(document.getElementById(`community-styles`))return;let e=document.createElement(`style`);e.id=`community-styles`,e.textContent=`
  ...
  `,document.body.appendChild(r),r.querySelector(`#btn-go-profile`).addEventListener(`click`,()=>{r.remove(),n(`#profile`)}),r.onclick=e=>{e.target===r&&(r.remove(),n(`#profile`))}}function ff(){if(document.getElementById(`shop-styles`))return;let e=document.createElement(`style`);e.id=`shop-styles`,e.textContent=`
    .shop-layout {
      display: grid;
      grid-template-columns: minmax(0, 1.55fr) minmax(18rem, var(--sidebar-width));
      gap: var(--space-2xl);
  ...
  `,document.head.appendChild(e)}var pf=`modulepreload`,mf=function(e){return`/mon_accord/`+e},hf={},gf=function(e,t,n){let r=Promise.resolve();if(t&&t.length>0){let e=document.getElementsByTagName(`link`),i=document.querySelector(`meta[property=csp-nonce]`),a=i?.nonce||i?.getAttribute(`nonce`);function o(e){return Promise.all(e.map(e=>Promise.resolve(e).then(e=>({status:`fulfilled`,value:e}),e=>({status:`rejected`,reason:e}))))}r=o(t.map(t=>{if(t=mf(t,n),t in hf)return;hf[t]=!0;let r=t.endsWith(`.css`),i=r?`[rel="stylesheet"]`:``;if(n)for(let n=e.length-1;n>=0;n--){let i=e[n];if(i.href===t&&(!r||i.rel===`stylesheet`))return}else if(document.querySelector(`link[href="${t}"]${i}`))return;let o=document.createElement(`link`);if(o.rel=r?`stylesheet`:pf,r||(o.as=`script`),o.crossOrigin=``,o.href=t,a&&o.setAttribute(`nonce`,a),document.head.appendChild(o),r)return new Promise((e,n)=>{o.addEventListener(`load`,e),o.addEventListener(`error`,()=>n(Error(`Unable to preload CSS for ${t}`)))})}))}function i(e){let t=new Event(`vite:preloadError`,{cancelable:!0});if(t.payload=e,window.dispatchEvent(t),!t.defaultPrevented)throw e}return r.then(t=>{for(let e of t||[])e.status===`rejected`&&i(e.reason);return e().catch(i)})},_f=document.getElementById(`app`),vf={"":b,"#landing":b,"#profile":Cd,"#lab":Gd,"#vault":Zd,"#shop":lf,"#community":of};function yf(){return window.location.hash||``}function bf(e){window.location.hash=e}function xf(){let e=yf(),t=vf[e]||b;_f.innerHTML=``,_f.appendChild(f(bf,e));let n=document.createElement(`div`);if(n.className=`page page-enter`,n.id=`page-content`,t(n,bf),_f.appendChild(n),!document.querySelector(`.toast-container`)){let e=document.createElement(`div`);e.className=`toast-container`,e.id=`toast-container`,document.body.appendChild(e)}window.scrollTo({top:0,behavior:`auto`})}window.showToast=function(e,t=`success`){let n=document.getElementById(`toast-container`);if(!n)return;let r=document.createElement(`div`);r.className=`toast toast--${t}`,r.innerHTML=`<span class="toast__message">${e}</span>`,n.appendChild(r),setTimeout(()=>{r.style.animation=`fadeOut 0.3s var(--ease-out) forwards`,setTimeout(()=>r.remove(),300)},3e3)},window.showSettings=function(){let e=document.querySelector(`.modal-overlay`);e&&e.remove();let t=d.getApiKey(),n=document.createElement(`div`);n.className=`modal-overlay`,n.id=`settings-modal`,n.innerHTML=`
    <div class="modal">
      <div class="modal__header">
  ...
  `,document.body.appendChild(n),n.querySelector(`#close-settings`).onclick=()=>n.remove(),n.querySelector(`#cancel-settings`).onclick=()=>n.remove(),n.onclick=e=>{e.target===n&&n.remove()},n.querySelector(`#save-settings`).onclick=()=>{let e=n.querySelector(`#api-key-input`).value.trim();d.setApiKey(e),gf(()=>Promise.resolve().then(()=>ud).then(e=>e.resetAI()),void 0),window.showToast(`Settings saved!`),n.remove()};let r=n.querySelector(`#reset-profile-btn`);r&&(r.onclick=()=>{d.remove(`monaccord_profile`),d.set(`profile`,null),window.showToast(`Profile reset.`),n.remove(),xf()})},window.addEventListener(`hashchange`,xf),xf();