(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var e=`monaccord_`,t={get(t,n=null){try{let r=localStorage.getItem(e+t);return r?JSON.parse(r):n}catch{return n}},set(t,n){try{localStorage.setItem(e+t,JSON.stringify(n))}catch(e){console.warn(`Storage write failed:`,e)}},remove(t){localStorage.removeItem(e+t)},getProfile(){return this.get(`profile`,null)},setProfile(e){this.set(`profile`,{...e,updatedAt:Date.now()})},getVault(){return this.get(`vault`,[])},saveFormula(e){let t=this.getVault(),n=t.findIndex(t=>t.id===e.id);return n>=0?t[n]={...e,savedAt:Date.now()}:t.push({...e,id:e.id||`f-`+Date.now(),savedAt:Date.now()}),this.set(`vault`,t),this.addInteraction({type:`save`,formulaId:e.id,timestamp:Date.now()}),t},removeFormula(e){let t=this.getVault().filter(t=>t.id!==e);return this.set(`vault`,t),t},getInteractions(){return this.get(`interactions`,[])},addInteraction(e){let t=this.getInteractions();t.push({...e,timestamp:e.timestamp||Date.now()}),t.length>500&&t.splice(0,t.length-500),this.set(`interactions`,t)},getLikes(){return this.get(`likes`,[])},toggleLike(e){let t=this.getLikes(),n=t.indexOf(e);return n>=0?(t.splice(n,1),this.addInteraction({type:`unlike`,formulaId:e,timestamp:Date.now()})):(t.push(e),this.addInteraction({type:`like`,formulaId:e,timestamp:Date.now()})),this.set(`likes`,t),t},isLiked(e){return this.getLikes().includes(e)},getOwnedPerfumes(){let e=this.get(`my_perfumes`,null);return Array.isArray(e)?{monAccord:e.map(e=>e.id),loreal:[]}:e||{monAccord:[],loreal:[]}},setOwnedPerfumes(e){this.set(`my_perfumes`,e)},getApiKey(){return this.get(`gemini_api_key`,``)},setApiKey(e){this.set(`gemini_api_key`,e)},getQuizState(){return this.get(`quiz_state`,null)},setQuizState(e){this.set(`quiz_state`,e)},clearQuizState(){this.remove(`quiz_state`)},setPendingShopCart(e){this.set(`pending_shop_cart`,e)},consumePendingShopCart(){let e=this.get(`pending_shop_cart`,[]);return this.remove(`pending_shop_cart`),Array.isArray(e)?e:[]},getShopCart(){let e=this.get(`shop_cart`,[]);return Array.isArray(e)?e:[]},setShopCart(e){this.set(`shop_cart`,e)},clearShopCart(){this.remove(`shop_cart`)},getUsername(){return this.getProfile()?.username||`Anonymous`},getPosts(){return this.get(`community_posts`,[])},setPosts(e){this.set(`community_posts`,e)},getPostLikes(){return this.get(`post_likes`,[])},togglePostLike(e){let t=this.getPostLikes(),n=t.indexOf(e);return n>=0?t.splice(n,1):t.push(e),this.set(`post_likes`,t),t},isPostLiked(e){return this.getPostLikes().includes(e)}};function n(e,n){let r=document.createElement(`nav`);return r.className=`navbar`,r.id=`main-nav`,t.getProfile(),r.innerHTML=`
  ...
  `,document.head.appendChild(e)}var m=`https://mon-accord-ai.alpgiraykacira.workers.dev`,h=`https://generativelanguage.googleapis.com/v1beta/models`,g=`gemini-2.5-flash`,_=`You are Mon Accord's AI Perfume Advisor — a world-class fragrance expert specializing in scent layering and olfactory profiling.
...
Keep responses concise but rich. Use fragrance terminology naturally.`,v=e=>({contents:[{parts:[{text:e}]}],systemInstruction:{parts:[{text:_}]}});function y(e,t){return t===429||e.includes(`429`)||e.includes(`quota`)||e.includes(`RESOURCE_EXHAUSTED`)?`rate-limit`:t===401||t===403||e.includes(`API_KEY`)||e.includes(`API key`)?`invalid-key`:e.includes(`fetch`)||e.includes(`network`)||e.includes(`Failed to fetch`)?`network`:`unknown`}function b(e,t){switch(e){case`rate-limit`:return`Rate limit reached. Please wait ${t||30} seconds and try again.`;case`invalid-key`:return`AI service key is invalid. Please contact the site owner.`;case`network`:return`Network error. Please check your connection and try again.`;default:return`AI request failed. Please try again.`}}async function x(e){let n=v(e);{let e=await fetch(m,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify(n)}),t=await e.json();if(!e.ok){let n=t?.error?.message||`HTTP ${e.status}`;throw Object.assign(Error(n),{status:e.status})}return t}let r=t.getApiKey();if(!r)throw Object.assign(Error(`no-api-key`),{status:0});let i=await fetch(`${h}/${g}:generateContent?key=${r}`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify(n)}),a=await i.json();if(!i.ok){let e=a?.error?.message||`HTTP ${i.status}`;throw Object.assign(Error(e),{status:i.status})}return a}async function S(e,t=2){try{let t=await x(e),n=t?.candidates?.[0]?.content?.parts?.[0]?.text;return n?{success:!0,text:n}:(console.error(`Empty Gemini response`,t),{success:!1,error:`empty-response`,text:`AI returned an empty response. Please try again.`})}catch(n){let r=n.message||String(n),i=n.status??0;if(console.error(`AI error:`,r),r===`no-api-key`)return{success:!1,error:`no-api-key`,text:`AI features are not available. Please contact the site owner.`};let a=y(r,i);if(a===`rate-limit`&&t>0){let n=r.match(/retry[^0-9]*(\d+)/i),i=n?parseInt(n[1]):30;return console.log(`Rate limited. Waiting ${i}s… (${t} retries left)`),await new Promise(e=>setTimeout(e,(i+2)*1e3)),S(e,t-1)}let o=r.match(/retry[^0-9]*(\d+)/i);return{success:!1,error:a,text:b(a,o?parseInt(o[1]):void 0)}}}function C(){return!0}async function ee(e){let n=await S(w(e));if(n.success){let r=te(n.text,e);return t.setProfile(r),t.addInteraction({type:`profile-created`,profile:r.archetype}),{success:!0,profile:r}}return{success:!1,error:n.text}}function w(e){let t=e.scentFamilies?.join(`, `)||`not specified`,n=(e.knownPerfumes||[]).map(e=>{let t=o.find(t=>t.id===e);return t?`${t.brand} ${t.name} (${t.family})`:e}).join(`, `)||`none selected`,r=e.interestedRegions?.join(`, `)||`all regions`;return`Analyze this user's fragrance preferences and create their olfactory profile.
...
}`}function te(e,t){try{let n=e.trim();return n.startsWith("```")&&(n=n.replace(/^```(?:json)?\n?/,``).replace(/\n?```$/,``)),{...JSON.parse(n),createdAt:Date.now(),quizAnswers:t}}catch(e){return console.error(`Failed to parse profile JSON:`,e),T(t)}}function T(e){let t=e.scentFamilies||[`woody`],n=`woody-earthy`;t.includes(`oriental`)||t.includes(`spicy`)?n=`warm-oriental`:t.includes(`fresh`)||t.includes(`citrus`)?n=`fresh-aquatic`:t.includes(`floral`)?n=`floral-romantic`:t.includes(`gourmand`)&&(n=`gourmand-cozy`);let r=c.find(e=>e.id===n)||c[0];return{archetype:n,archetypeName:r.name,description:r.description,primaryFamilies:t.slice(0,3),sillageProfile:e.sillage>7?`high`:e.sillage>4?`medium`:`low`,notePreferences:{loves:[],explore:[],avoid:[]},recommendedRegions:e.interestedRegions||[`mediterranean`,`eastasia`],signatureBlend:null,createdAt:Date.now(),quizAnswers:e}}async function ne(e){return await S(`You are simulating the scent experience of this fragrance combination. Describe what the wearer would smell in three temporal phases.
...
Be poetic but precise. Help the reader "smell" this blend through words. Keep it under 200 words total.`)}var E=Object.fromEntries([`fresh`,`floral`,`woody`,`oriental`,`citrus`,`gourmand`,`green`,`aromatic`,`spicy`,`musky`].map(e=>[e,new URL(Object.assign({})[`../assets/quiz_scent_families/${e}.png`],import.meta.url).href])),re={elegant:new URL(`/mon_accord/assets/elegant_classic-BWma7RUf.webp`,``+import.meta.url).href,adventurous:new URL(`/mon_accord/assets/adventurous_bold-B8qkq39y.webp`,``+import.meta.url).href,romantic:new URL(`/mon_accord/assets/romantic_dreamy-DfjnsaQk.webp`,``+import.meta.url).href,minimalist:new URL(`/mon_accord/assets/minimalist_clean-DxYPWAc5.webp`,``+import.meta.url).href,creative:new URL(`/mon_accord/assets/creative_expressive-KLFTMH5o.webp`,``+import.meta.url).href,confident:new URL(`/mon_accord/assets/confident_powerful-9p8Y7K6Y.webp`,``+import.meta.url).href},D=5;function O(e,n){let r=t.getProfile();if(r&&!window.__retakeQuiz){le(e,r,n);return}window.__retakeQuiz=!1;let i=t.getQuizState(),a=i?.step||1,o=i?.answers||{username:``,scentFamilies:[],knownPerfumes:[],sillage:5,longevity:5,intensity:5,personality:``,notes:``};function s(){t.setQuizState({step:a,answers:o}),e.innerHTML=`
      <div class="page__container">
        <div class="quiz-container">
          <div class="quiz-stepper">
            ${Array.from({length:D},(e,t)=>{let n=t+1,r=n<a?`completed`:n===a?`active`:``,i=n<a?`done`:``;return`<div class="quiz-stepper__item ${r}"><div class="quiz-stepper__circle">${n<a?`✓`:String(n)}</div></div>${t<D-1?`<div class="quiz-stepper__line ${i}"></div>`:``}`}).join(``)}
          </div>
          ...
          <div class="quiz-actions">
            ...
            ${a<D?`<button class="btn btn--primary" id="quiz-next">Continue →</button>`:`<button class="btn btn--primary" id="quiz-finish">
                  ${C()?`✦ Generate My Profile`:`✦ Generate Profile`}
                </button>`}
          </div>
        </div>
      </div>
    `,M(),ie(a,o,e);let r=e.querySelector(`#quiz-back`),i=e.querySelector(`#quiz-next`),c=e.querySelector(`#quiz-finish`);r&&r.addEventListener(`click`,()=>{j(a,o,e),a--,s()}),i&&i.addEventListener(`click`,()=>{j(a,o,e),a++,s()}),c&&c.addEventListener(`click`,async()=>{if(j(a,o,e),!C()){window.showToast(`Please set your Gemini API key in Settings first.`,`error`),window.showSettings();return}c.disabled=!0,c.innerHTML=`<span class="loading-spinner"></span> Generating...`;let r=e.querySelector(`#generation-status`);r||(r=document.createElement(`p`),r.id=`generation-status`,r.style.cssText=`font-size: var(--text-xs); color: var(--text-tertiary); text-align: center; margin-top: var(--space-sm);`,c.parentElement.appendChild(r)),r.textContent=`Analyzing your preferences with AI...`;let i=await ee(o);i.success?(i.profile.username=o.username||`Anonymous`,t.setProfile(i.profile),t.clearQuizState(),ae(e,i.profile,n),window.showToast(`Your olfactory profile has been created! ✦`)):(c.disabled=!1,c.innerHTML=`✦ Generate My Profile`,r.textContent=``,window.showToast(i.error||`Failed to generate profile.`,`error`))})}s()}function k(e,t){switch(e){case 1:return`
        ...
        <div class="quiz-grid quiz-grid--families">
          ${a.map(e=>`
            <div class="quiz-option quiz-option--img-card ${t.scentFamilies.includes(e.id)?`quiz-option--selected`:``}" data-value="${e.id}" id="family-${e.id}">
              <img class="quiz-option__img" src="${E[e.id]}" alt="${e.name}" loading="lazy" decoding="async" />
            </div>
          `).join(``)}
        </div>
      `;case 3:return`
        ...
        <div class="quiz-grid quiz-grid--context">
          ${[{id:`elegant`,name:`Elegant & Classic`,desc:`Timeless sophistication`},{id:`adventurous`,name:`Adventurous & Bold`,desc:`Love discovering the new`},{id:`romantic`,name:`Romantic & Dreamy`,desc:`Soft, poetic, emotional`},{id:`minimalist`,name:`Minimalist & Clean`,desc:`Less is more`},{id:`creative`,name:`Creative & Expressive`,desc:`Unique, unconventional`},{id:`confident`,name:`Confident & Powerful`,desc:`Commands attention`}].map(e=>`
            <div class="quiz-option quiz-option--img-card ${t.personality===e.id?`quiz-option--selected`:``}" data-value="${e.id}" id="ctx-${e.id}">
              <img class="quiz-option__img" src="${re[e.id]}" alt="${e.name}" loading="lazy" decoding="async" />
            </div>
          `).join(``)}
        </div>
      ...
      `}}function A(e,t){let n=t?o.filter(e=>e.name.toLowerCase().includes(t.toLowerCase())||e.brand.toLowerCase().includes(t.toLowerCase())):o,r={};return n.forEach(e=>{r[e.brand]||(r[e.brand]=[]),r[e.brand].push(e)}),Object.entries(r).map(([t,n])=>`
      ...
      <div class="profile-result">
        ...
        <div class="mt-xl">
          ...
          <div class="recommendation-grid">
            ${i.map(e=>oe(e,{showBuyButton:!0})).join(``)}
          </div>
        </div>
  ...
  `,document.head.appendChild(e)}var ue=[{id:`confident`,name:`Confident`,icon:``,description:`Bold and commanding`},{id:`romantic`,name:`Romantic`,icon:``,description:`Soft and alluring`},{id:`calm`,name:`Calm`,icon:``,description:`Peaceful and centered`},{id:`energetic`,name:`Energetic`,icon:``,description:`Vibrant and lively`},{id:`mysterious`,name:`Mysterious`,icon:``,description:`Enigmatic and deep`},{id:`playful`,name:`Playful`,icon:``,description:`Light and fun`}],de=[{id:`office`,name:`Office / Work`,icon:``},{id:`date-night`,name:`Date Night`,icon:``},{id:`casual`,name:`Casual Outing`,icon:``},{id:`formal`,name:`Formal Event`,icon:``},{id:`outdoor`,name:`Outdoor Adventure`,icon:``},{id:`cozy`,name:`Cozy Night In`,icon:``}],fe=[{id:`spring`,name:`Spring`,icon:``},{id:`summer`,name:`Summer`,icon:``},{id:`autumn`,name:`Autumn`,icon:``},{id:`winter`,name:`Winter`,icon:``}],N=[{id:`whisper`,name:`Whisper`,value:1,description:`Barely there, intimate`},{id:`soft`,name:`Soft`,value:3,description:`Close range only`},{id:`moderate`,name:`Moderate`,value:5,description:`Noticeable arm's length`},{id:`present`,name:`Present`,value:7,description:`Fills the room gently`},{id:`bold`,name:`Bold`,value:9,description:`Leaves a trail`}];function pe(){let e=t.getOwnedPerfumes(),n=[];if(e.monAccord?.length){let t=e.monAccord.map(e=>i.find(t=>t.id===e)?.name).filter(Boolean);t.length&&n.push(`Mon Accord owned: ${t.join(`, `)}`)}if(e.loreal?.length){let t=e.loreal.map(e=>o.find(t=>t.id===e)?.name).filter(Boolean);t.length&&n.push(`L'Oréal Luxe owned: ${t.join(`, `)}`)}return n.length?n.join(`
`):null}async function me(e){let n=t.getProfile(),r=pe(),a=await S(`Based on the user's context, recommend a Mon Accord layering formula.
...
Respond in EXACTLY this JSON format (no markdown, no code blocks):
{
...
}`);if(a.success)try{let n=a.text.trim();n.startsWith("```")&&(n=n.replace(/^```(?:json)?\n?/,``).replace(/\n?```$/,``));let r=JSON.parse(n);return t.addInteraction({type:`contextual-recommendation`,context:e,recommendation:r.formulaName}),{success:!0,recommendation:r}}catch(e){return console.error(`Failed to parse recommendation:`,e),{success:!0,recommendation:{formulaName:`AI Recommendation`,layers:[],reasoning:a.text,scentPreview:``,tips:``}}}return{success:!1,error:a.text}}function he(e,n={}){let{showNameInput:r=!0,onSaved:i}=n,a=t.get(`vault_folders`,[{id:`default`,name:`All Formulas`,icon:``}]),o=`default`,s=!1;function c(){let n=document.getElementById(`save-vault-overlay`);n&&n.remove();let l=document.createElement(`div`);l.className=`modal-overlay`,l.id=`save-vault-overlay`,l.innerHTML=`
  ...
  `,document.head.appendChild(e)}var _e={scandinavian:new URL(`/mon_accord/assets/scandinavian-BN4U9Git.webp`,``+import.meta.url).href,eastasia:new URL(`/mon_accord/assets/east_asia-CIRqjWhL.webp`,``+import.meta.url).href,southafrica:new URL(`/mon_accord/assets/south_africa-BeRUDnKj.webp`,``+import.meta.url).href,mediterranean:new URL(`/mon_accord/assets/mediterranean-v1NGrAPJ.webp`,``+import.meta.url).href,southamerica:new URL(`/mon_accord/assets/south_america-DEpvED0s.webp`,``+import.meta.url).href,middleeast:new URL(`/mon_accord/assets/middle_east-DE6yO7Qz.webp`,``+import.meta.url).href},ve={scandinavian:new URL(`/mon_accord/assets/scandinavian-DvtLirp4.webp`,``+import.meta.url).href,eastasia:new URL(`/mon_accord/assets/east_asia-UzXNmAB4.webp`,``+import.meta.url).href,southafrica:new URL(`/mon_accord/assets/south_africa-PFpjigF9.webp`,``+import.meta.url).href,mediterranean:new URL(`/mon_accord/assets/mediterranean-CV0fuGZk.webp`,``+import.meta.url).href,southamerica:new URL(`/mon_accord/assets/south_america-DyzbduD3.webp`,``+import.meta.url).href,middleeast:new URL(`/mon_accord/assets/middle_east-Bmlf8LlT.webp`,``+import.meta.url).href},ye={scandinavian:new URL(`/mon_accord/assets/scandinavian-BBoj4W5D.webp`,``+import.meta.url).href,eastasia:new URL(`/mon_accord/assets/east_asia-D93RVLet.webp`,``+import.meta.url).href,southafrica:new URL(`/mon_accord/assets/south_africa-hdHFYwXs.webp`,``+import.meta.url).href,mediterranean:new URL(`/mon_accord/assets/mediterranean-B_fMAvaC.webp`,``+import.meta.url).href,southamerica:new URL(`/mon_accord/assets/south_america-DcR51yl4.webp`,``+import.meta.url).href,middleeast:new URL(`/mon_accord/assets/middle_east-DMI8-64c.webp`,``+import.meta.url).href},be=`lab_session_state`;function xe(e){try{sessionStorage.setItem(be,JSON.stringify(e))}catch{}}function Se(){try{return JSON.parse(sessionStorage.getItem(be)||`null`)}catch{return null}}function Ce(e,t){let n=Se(),a=n?.layers||[],o=n?.scentSimulation||null,s=!1,c=n?.contextResult||null,u=!1,d=n?.selectedMood||null,f=n?.selectedOccasion||null,p=n?.selectedSeason||null,m=n?.selectedTime||null,h=n?.selectedIntensity||5;function g(){xe({layers:a,scentSimulation:o,contextResult:c,selectedMood:d,selectedOccasion:f,selectedSeason:p,selectedTime:m,selectedIntensity:h})}let _=JSON.parse(sessionStorage.getItem(`labPending`)||`[]`);_.length>0&&(_.forEach(e=>{let t=l(e);t&&!a.find(t=>t.perfumeId===e)&&a.push({perfumeId:e,amount:t.format===`spray`?2:3,unit:t.format===`spray`?`sprays`:`drops`})}),sessionStorage.removeItem(`labPending`),g());function v(){e.innerHTML=`
      <div class="page__container">
                      ...
                      <div class="lab-bottle-row">
                        ${t?`
                          <button class="lab-bottle-btn ${r?`lab-bottle-btn--added`:``}" data-id="${t.id}" style="--region-color: ${e.color};" title="${e.name} Spray">
                            <img src="${ve[e.id]}" alt="${e.name} Spray" class="lab-bottle-img" loading="lazy" decoding="async" />
                        ...
                        ${n?`
                          <button class="lab-bottle-btn ${o?`lab-bottle-btn--added`:``}" data-id="${n.id}" style="--region-color: ${e.color};" title="${e.name} Oil">
                            <img src="${ye[e.id]}" alt="${e.name} Oil" class="lab-bottle-img" loading="lazy" decoding="async" />
              ...
              <div class="lab-advisor__form">
                <div class="input-group">
                  ...
                  <div class="lab-advisor__chips" id="mood-chips">
                    ${ue.map(e=>`<button class="lab-chip ${d===e.id?`lab-chip--active`:``}" data-value="${e.id}">${e.icon} ${e.name}</button>`).join(``)}
                  </div>
                </div>
                ...
                <div class="input-group">
                  ...
                  <div class="lab-advisor__chips" id="occasion-chips">
                    ${de.map(e=>`<button class="lab-chip ${f===e.id?`lab-chip--active`:``}" data-value="${e.id}">${e.icon} ${e.name}</button>`).join(``)}
                  </div>
                </div>
                ...
                <div class="input-group">
                  ...
                  <div class="lab-advisor__chips" id="season-chips">
                    ${fe.map(e=>`<button class="lab-chip ${p===e.id?`lab-chip--active`:``}" data-value="${e.id}">${e.icon} ${e.name}</button>`).join(``)}
                  </div>
                </div>
                ...
                <div class="slider-container">
                  <div class="slider-header">
                    <span class="slider-label">Intensity</span>
                    <span class="slider-value" id="advisor-intensity-val">${(N.find(e=>e.value===h)||{}).name||`Moderate`}</span>
                  </div>
                ...
                <div class="lab-advisor__result mt-lg" id="advisor-result">
                  <div class="ai-response">
                    ...
                    <div class="ai-response__text ai-response__text--compact">
                      ${c.reasoning?`<p>${P(c.reasoning,150)}</p>`:``}
                      ${c.scentPreview?`<p><em>${P(c.scentPreview,100)}</em></p>`:``}
                      ${c.tips?`<p style="color: var(--accent); font-size: var(--text-xs);">◈ ${P(c.tips,100)}</p>`:``}
                    </div>
                  </div>
                  ${c.layers?.length>0?`
            ...
            <div class="lab-layers" id="lab-layers">
              ...
              `:a.map((e,t)=>{let n=l(e.perfumeId),i=r.find(e=>e.id===n.region);return`
                  <div class="lab-layer" data-idx="${t}" style="--region-color: ${i.color};">
                    <div class="lab-layer__bg" style="background-image: url('${_e[i.id]}')"></div>
...
`).filter(e=>e.trim()).map(e=>{if(e.match(/^(OPENING|HEART|DRY DOWN|OVERALL|SILLAGE)/i)){let[t,...n]=e.split(`:`);return`<p><strong style="color: var(--accent);">${t.trim()}:</strong> ${n.join(`:`).trim()}</p>`}return`<p>${e}</p>`}).join(``):``}function Te(){if(document.getElementById(`lab-styles`))return;let e=document.createElement(`style`);e.id=`lab-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e)}var Ee=`/mon_accord/assets/folder-DAYqs9Dj.webp`,De={scandinavian:new URL(`/mon_accord/assets/scandinavian-DvtLirp4.webp`,``+import.meta.url).href,eastasia:new URL(`/mon_accord/assets/east_asia-UzXNmAB4.webp`,``+import.meta.url).href,southafrica:new URL(`/mon_accord/assets/south_africa-PFpjigF9.webp`,``+import.meta.url).href,mediterranean:new URL(`/mon_accord/assets/mediterranean-CV0fuGZk.webp`,``+import.meta.url).href,southamerica:new URL(`/mon_accord/assets/south_america-DyzbduD3.webp`,``+import.meta.url).href,middleeast:new URL(`/mon_accord/assets/middle_east-Bmlf8LlT.webp`,``+import.meta.url).href},Oe={scandinavian:new URL(`/mon_accord/assets/scandinavian-BBoj4W5D.webp`,``+import.meta.url).href,eastasia:new URL(`/mon_accord/assets/east_asia-D93RVLet.webp`,``+import.meta.url).href,southafrica:new URL(`/mon_accord/assets/south_africa-hdHFYwXs.webp`,``+import.meta.url).href,mediterranean:new URL(`/mon_accord/assets/mediterranean-B_fMAvaC.webp`,``+import.meta.url).href,southamerica:new URL(`/mon_accord/assets/south_america-DcR51yl4.webp`,``+import.meta.url).href,middleeast:new URL(`/mon_accord/assets/middle_east-DMI8-64c.webp`,``+import.meta.url).href},ke={"mugler-angel":new URL(`/mon_accord/assets/angel-JaKC4iyN.webp`,``+import.meta.url).href,"ysl-black-opium":new URL(`/mon_accord/assets/black_opium-CwBrp2qb.webp`,``+import.meta.url).href,"lancome-la-vie-est-belle":new URL(`/mon_accord/assets/la_vie_est_belle-CHGqLg72.webp`,``+import.meta.url).href},Ae=[`mugler-angel`,`ysl-black-opium`,`lancome-la-vie-est-belle`],je=[{id:`evening`,name:`Evening Wear`},{id:`daytime`,name:`Daytime`},{id:`office`,name:`Office`},{id:`weekend`,name:`Weekend`},{id:`seasonal`,name:`Seasonal`}];function Me(e,n){let a=t.getOwnedPerfumes(),s=Ae.filter(e=>!a.loreal.includes(e));s.length&&(a.loreal=[...a.loreal,...s],t.setOwnedPerfumes(a));let c=t.get(`vault_folders`,null),u;if(!c)u=je.map(e=>({...e})),t.set(`vault_folders`,u);else{let e=c.filter(e=>e.id!==`default`),n=new Set(e.map(e=>e.id)),r=je.filter(e=>!n.has(e.id));u=[...e,...r].map(e=>({id:e.id,name:e.name})),t.set(`vault_folders`,u)}let d=null,f=!1,p=null,m=null;function h(){t.set(`vault_folders`,u)}function g(e){return t.getVault().filter(t=>t.folderId===e)}function _(){let n=t.getVault();e.innerHTML=`
      <div class="page__container">
        ${d?b(d,n):v(n)}
      </div>
      ${x()}
    `,Ne(),S(n)}function v(e){return`
          ...
          <div class="vault-folders-grid">
            ${u.map(e=>{let t=g(e.id).length;return`
                <div class="vault-folder-card card card--interactive" data-folder="${e.id}">
                  <img src="${Ee}" class="vault-folder-icon" alt="folder" loading="lazy" decoding="async" />
    ...
    `}function y(){let e=t.getOwnedPerfumes();function n(e){return e.length?`<div class="vault-bottle-grid">
        ${e.map(e=>{let t=l(e),n=t?r.find(e=>e.id===t.region):null,i=t?.format===`spray`?De[t.region]:Oe[t.region],a=t?.name||e;return`
        ...
        ${e.map(e=>{let t=o.find(t=>t.id===e),n=ke[e],r=t?t.name:e,i=t?.brand||``;return`
                      ...
                      <div class="vault-modal-bottle-row">
                        ${n?`
                          <button class="vault-modal-bottle-btn vault-owned-add-btn ${a?`vault-modal-bottle-btn--owned`:``}"
                            ...
                            ${a?`disabled`:``}>
                            <img src="${De[t.id]}" alt="Spray" class="vault-modal-bottle-img" loading="lazy" decoding="async" />
                        ...
                        ${r?`
                          <button class="vault-modal-bottle-btn vault-owned-add-btn ${o?`vault-modal-bottle-btn--owned`:``}"
                            ...
                            ${o?`disabled`:``}>
                            <img src="${Oe[t.id]}" alt="Oil" class="vault-modal-bottle-img" loading="lazy" decoding="async" />
...
Respond in EXACTLY this JSON format (no markdown, no code blocks):
{
  ...
  "scentDescription": "Vivid 2-sentence sensory preview"
        ...
        <div class="community-choice-row">
          <div class="community-choice-card">
            <div class="community-choice-card__photo-wrap">
              <img src="${Re}" alt="Duygu Özaslan" class="community-choice-card__photo" loading="lazy" decoding="async" />
            </div>
              ...
              ${i.length===0?`<p style="color:var(--text-tertiary);font-size:var(--text-sm);padding:var(--space-lg);">No posts yet. Be the first to share!</p>`:ee(i).map(e=>`
                  <div class="community-post-item ${f===e.id?`community-post-item--active`:``}" data-post-id="${e.id}">
                    <div class="community-post-item__top">
                      <span class="community-post-item__date">${k(e.date)}</span>
                    </div>
                    ...
                    <div class="community-post-item__bottom">
                      ...
                      <span class="community-post-item__stats">${e.likes||0} · ${O(e)} comments</span>
                    </div>
                  </div>
                `).join(``)}
            </div>
    ...
    `,He(),ie(n,i)}function te(e){let t=[...i.map(e=>({id:e.id,label:`${r.find(t=>t.id===e.region)?.icon||``} ${e.name} (${e.format})`,group:`Mon Accord`})),...o.map(e=>({id:e.id,label:`${e.brand} — ${e.name}`,group:`L'Oreal Luxe`}))];return`
    ...
    `}let T=[];function ne(e){let n=t.isPostLiked(e.id),r=D(e);return`
      <div class="community-post-detail__inner">
        ...
        <div class="community-post-detail__actions">
          ...
          <span style="font-size:var(--text-xs);color:var(--text-tertiary);">${O(e)} comments</span>
        </div>
    ...
    `}function E(e,t,n){return!e||!e.length?``:e.map(e=>`
      <div class="community-comment" style="margin-left:${n*20}px;">
        <div class="community-comment__meta">
          ...
          <span class="community-comment__date">${k(e.date)}</span>
        </div>
    ...
    `).join(``)}function re(e){if(e.topicType===`trending`){let t=s.find(t=>t.id===e.topicRef);return t?t.name:`Trending`}return e.topicType===`combination`?`Custom Combo`:e.topicLabel||`General`}function D(e){if(e.topicType===`trending`){let t=s.find(t=>t.id===e.topicRef);return t?`<div class="community-post-topic-badge">Trending: ${t.name}</div>`:``}return e.topicType===`combination`&&e.topicCombination?.length?`<div class="community-post-topic-badge">${e.topicCombination.map(e=>`${l(e.perfumeId)?.name||e.perfumeId} (${e.amount} ${e.unit})`).join(` + `)}</div>`:e.topicType===`free`&&e.topicLabel?`<div class="community-post-topic-badge">${e.topicLabel}</div>`:``}function O(e){let t=0;function n(e){(e||[]).forEach(e=>{t++,n(e.replies)})}return n(e.comments),t}function k(e){let t=Date.now()-e,n=Math.floor(t/6e4);if(n<60)return`${n}m ago`;let r=Math.floor(n/60);return r<24?`${r}h ago`:`${Math.floor(r/24)}d ago`}function A(e,t){for(let n of e||[]){if(n.id===t)return n;let e=A(n.replies,t);if(e)return e}return null}function ie(i,a){e.querySelectorAll(`.community-sort-btn[data-sort]`).forEach(e=>{e.addEventListener(`click`,()=>{u=e.dataset.sort,w()})}),e.querySelectorAll(`.community-sort-btn[data-post-sort]`).forEach(e=>{e.addEventListener(`click`,()=>{d=e.dataset.postSort,w()})}),e.querySelectorAll(`[data-like-formula]`).forEach(e=>{e.addEventListener(`click`,n=>{n.stopPropagation(),t.toggleLike(e.dataset.likeFormula),w()})}),e.querySelectorAll(`[data-vault-formula]`).forEach(e=>{e.addEventListener(`click`,t=>{t.stopPropagation();let n=i.find(t=>t.id===e.dataset.vaultFormula);if(!n)return;let a=(n.layers||[]).map(e=>{let t=l(e.perfumeId);return t&&r.find(e=>e.id===t.region),{...e,name:t?.name||e.perfumeId}});he({...n,id:`saved-`+n.id,layers:a},{showNameInput:!1})})});let s=e.querySelector(`#btn-load-suggestion`);s&&s.addEventListener(`click`,()=>{v?.layers?.length&&(sessionStorage.setItem(`labPending`,JSON.stringify(v.layers.map(e=>e.perfumeId).filter(e=>l(e)))),n(`#lab`))}),e.querySelectorAll(`[data-post-id]`).forEach(e=>{e.classList.contains(`community-post-item`)&&e.addEventListener(`click`,()=>{f=e.dataset.postId,w()})});let c=e.querySelector(`#btn-new-post`);c&&c.addEventListener(`click`,()=>{p=!0,m=null,T=[],w()}),e.querySelectorAll(`.community-topic-btn`).forEach(e=>{e.addEventListener(`click`,()=>{m=e.dataset.topic,T=[],w()})});let h=e.querySelector(`#combo-add-layer`);h&&h.addEventListener(`click`,()=>{let t=e.querySelector(`#combo-perfume-select`),n=e.querySelector(`#combo-amount`),r=e.querySelector(`#combo-unit`);t.value&&(T.push({perfumeId:t.value,amount:parseInt(n.value)||2,unit:r.value}),g())});function g(){let t=e.querySelector(`#combo-layers`);t&&(t.innerHTML=T.map((e,t)=>{let n=l(e.perfumeId),r=o.find(t=>t.id===e.perfumeId);return`<div class="community-combo-layer"><span>${n?.name||(r?`${r.brand} — ${r.name}`:e.perfumeId)} (${e.amount} ${e.unit})</span><button class="community-combo-remove" data-idx="${t}">✕</button></div>`}).join(``),t.querySelectorAll(`.community-combo-remove`).forEach(e=>{e.addEventListener(`click`,()=>{T.splice(parseInt(e.dataset.idx),1),g()})}))}let _=e.querySelector(`#btn-cancel-post`);_&&_.addEventListener(`click`,()=>{p=!1,m=null,w()});let y=e.querySelector(`#btn-submit-post`);y&&y.addEventListener(`click`,()=>{let n=e.querySelector(`#new-post-title`)?.value.trim(),r=e.querySelector(`#new-post-content`)?.value.trim();if(!n||!r||!m){window.showToast(`Please fill in all fields.`,`error`);return}let i={id:`p-`+Date.now(),author:t.getUsername(),date:Date.now(),title:n,content:r,topicType:m,likes:0,comments:[]};m===`combination`&&(i.topicCombination=[...T]),m===`trending`&&(i.topicRef=e.querySelector(`#trending-select`)?.value||``),m===`free`&&(i.topicLabel=e.querySelector(`#free-topic-label`)?.value.trim()||`General`);let a=F();a.unshift(i),t.setPosts(a),p=!1,m=null,f=i.id,w(),window.showToast(`Post published!`)}),e.querySelectorAll(`[data-like-post]`).forEach(e=>{e.addEventListener(`click`,()=>{let n=e.dataset.likePost,r=t.isPostLiked(n);t.togglePostLike(n);let i=F(),a=i.find(e=>e.id===n);a&&(a.likes=(a.likes||0)+(r?-1:1),t.setPosts(i)),w()})});let b=e.querySelector(`#btn-add-comment`);b&&b.addEventListener(`click`,()=>{let n=e.querySelector(`#new-comment-input`)?.value.trim();if(!n)return;let r=F(),i=r.find(e=>e.id===b.dataset.postId);i&&(i.comments||=[],i.comments.push({id:`c-`+Date.now(),author:t.getUsername(),date:Date.now(),text:n,replies:[]}),t.setPosts(r),w())}),e.querySelectorAll(`.community-reply-toggle`).forEach(t=>{t.addEventListener(`click`,()=>{let n=e.querySelector(`#reply-${t.dataset.commentId}`);n&&(n.style.display=n.style.display===`none`?`flex`:`none`)})}),e.querySelectorAll(`.community-reply-submit`).forEach(n=>{n.addEventListener(`click`,()=>{let r=(e.querySelector(`#reply-${n.dataset.commentId}`)?.querySelector(`input`))?.value.trim();if(!r)return;let i=F(),a=i.find(e=>e.id===n.dataset.postId);if(a){let e=A(a.comments,n.dataset.commentId);e&&(e.replies||=[],e.replies.push({id:`r-`+Date.now(),author:t.getUsername(),date:Date.now(),text:r,replies:[]}),t.setPosts(i),w())}})})}w(),v||(async()=>{let e;if(C()){let t=await Ie();e=t.success&&t.remix?t.remix:j()}else e=j();v=e,y=!1,sessionStorage.setItem(h,JSON.stringify({date:g,remix:e})),w()})();function j(){let e=x.slice().sort((e,t)=>t.matchPercent-e.matchPercent)[0]||a[0];return{remixName:e?.name||`Daily Selection`,layers:e?.layers||[],inspiration:e?.description||`A community-loved formula selected for your profile.`,scentDescription:`Balanced for versatility across day and evening wear.`}}}function Ve(e,t){return t?`<div style="padding:var(--space-xl);text-align:center;"><span class="loading-spinner"></span><p style="margin-top:var(--space-md);color:var(--text-tertiary);">Preparing today's suggestion...</p></div>`:e?`
  ...
  `:`<p style="font-size:var(--text-sm);color:var(--text-tertiary);padding:var(--space-lg);">No suggestion available.</p>`}function He(){if(document.getElementById(`community-styles`))return;let e=document.createElement(`style`);e.id=`community-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e)}var Ue=`/mon_accord/assets/Sephora-Logo-0PIDe1Np.png`,We=`/mon_accord/assets/Boyner_Logo-CbMLtvDJ.webp`,Ge=`/mon_accord/assets/Trendyol_logo-CfMQtlYm.png`,Ke=`/mon_accord/assets/hepsiburada-logo-I_Ul_zcU.png`,qe={scandinavian:new URL(`/mon_accord/assets/scandinavian-DvtLirp4.webp`,``+import.meta.url).href,eastasia:new URL(`/mon_accord/assets/east_asia-UzXNmAB4.webp`,``+import.meta.url).href,southafrica:new URL(`/mon_accord/assets/south_africa-PFpjigF9.webp`,``+import.meta.url).href,mediterranean:new URL(`/mon_accord/assets/mediterranean-CV0fuGZk.webp`,``+import.meta.url).href,southamerica:new URL(`/mon_accord/assets/south_america-DyzbduD3.webp`,``+import.meta.url).href,middleeast:new URL(`/mon_accord/assets/middle_east-Bmlf8LlT.webp`,``+import.meta.url).href},Je={scandinavian:new URL(`/mon_accord/assets/scandinavian-BBoj4W5D.webp`,``+import.meta.url).href,eastasia:new URL(`/mon_accord/assets/east_asia-D93RVLet.webp`,``+import.meta.url).href,southafrica:new URL(`/mon_accord/assets/south_africa-hdHFYwXs.webp`,``+import.meta.url).href,mediterranean:new URL(`/mon_accord/assets/mediterranean-B_fMAvaC.webp`,``+import.meta.url).href,southamerica:new URL(`/mon_accord/assets/south_america-DcR51yl4.webp`,``+import.meta.url).href,middleeast:new URL(`/mon_accord/assets/middle_east-DMI8-64c.webp`,``+import.meta.url).href};function Ye(e,n){let a=t.getShopCart(),o=[...new Set(t.consumePendingShopCart())],s=[...new Set([...a,...o])],c=s.map(e=>({id:e}));if(o.length){let e=s.length-a.length;window.showToast(`Added ${e} new item${e===1?``:`s`} from your recommended combination.`)}u();function u(){t.setShopCart(c.map(e=>e.id))}function d(){let a=t.getOwnedPerfumes().monAccord||[];Xe(a,c),e.innerHTML=`
      <div class="page__container">
                  ...
                  <div class="shop-region-card__products">
                    ${t?(()=>{let n=c.find(e=>e.id===t.id),r=a.includes(t.id);return`
                        <button class="shop-bottle-btn ${n?`shop-bottle-btn--in-cart`:``} ${r?`shop-bottle-btn--owned`:``}" data-id="${t.id}" style="--region-color:${e.color};">
                          ...
                          <img src="${qe[e.id]}" class="shop-bottle-img" alt="Spray" loading="lazy" decoding="async" />
                    ...
                    ${n?(()=>{let t=c.find(e=>e.id===n.id),r=a.includes(n.id);return`
                        <button class="shop-bottle-btn ${t?`shop-bottle-btn--in-cart`:``} ${r?`shop-bottle-btn--owned`:``}" data-id="${n.id}" style="--region-color:${e.color};">
                          ...
                          ${r?`<div class="shop-bottle-owned">Owned</div>`:``}
                          <img src="${Je[e.id]}" class="shop-bottle-img" alt="Oil" loading="lazy" decoding="async" />
    ...
    `,Qe(),e.querySelectorAll(`.shop-bottle-btn`).forEach(e=>{e.addEventListener(`click`,t=>{t.stopPropagation();let n=e.dataset.id,r=c.findIndex(e=>e.id===n);r>=0?c.splice(r,1):c.push({id:n}),u(),d()})}),e.querySelectorAll(`.shop-cart__item-remove`).forEach(e=>{e.addEventListener(`click`,()=>{c=c.filter(t=>t.id!==e.dataset.id),u(),d()})});let o=e.querySelector(`#btn-confirm-order`);o&&o.addEventListener(`click`,()=>{Ze(c,e,n)})}d()}function Xe(e,t){if(!e.length)return null;let n=t.map(e=>e.id),r=e.map(e=>l(e)).filter(Boolean),a=[...new Set(r.flatMap(e=>e.scentFamily.split(`-`)))],o=i.filter(t=>!e.includes(t.id)&&!n.includes(t.id)&&t.scentFamily.split(`-`).some(e=>a.includes(e))).slice(0,2);return o.length?`Based on your owned ${r.map(e=>e.name).join(` & `)}: try adding ${o.map(e=>e.name).join(` or `)}.`:null}function Ze(e,n,r){t.clearShopCart();let i=document.createElement(`div`);i.className=`modal-overlay`,i.innerHTML=`
        ...
        <div class="order-retailers">
          <div class="order-retailers__grid">
            <div class="order-retailer-card" aria-label="Sephora">
              <img src="${Ue}" alt="Sephora" class="order-retailer-card__logo" loading="lazy" decoding="async" />
            </div>
            <div class="order-retailer-card" aria-label="Boyner">
              <img src="${We}" alt="Boyner" class="order-retailer-card__logo" loading="lazy" decoding="async" />
            </div>
            <div class="order-retailer-card" aria-label="Trendyol">
              <img src="${Ge}" alt="Trendyol" class="order-retailer-card__logo" loading="lazy" decoding="async" />
            </div>
            <div class="order-retailer-card" aria-label="Hepsiburada">
              <img src="${Ke}" alt="Hepsiburada" class="order-retailer-card__logo" loading="lazy" decoding="async" />
            </div>
          </div>
        </div>
  ...
  `,document.head.appendChild(e)}function I(e){return new Promise(t=>setTimeout(t,e))}function L(e,t){if(!e)return I(0);var n=t===void 0?90:t,r=e.getBoundingClientRect().top+window.scrollY-n;return window.scrollTo({top:Math.max(0,r),behavior:`smooth`}),I(700)}function $e(e,t,n){return Math.min(n,Math.max(t,e))}function et(e){let t={};for(let n=0;n<e.length;n+=1){let r=e.key(n);r&&(t[r]=e.getItem(r))}return t}function tt(e,t){e.clear(),Object.entries(t||{}).forEach(([t,n])=>{n!=null&&e.setItem(t,n)})}async function nt(){let e=document.createElement(`div`);e.id=`dt-flash`,e.style.cssText=`
    position:fixed;inset:0;z-index:99994;
    background:rgba(200,169,126,0.07);
    pointer-events:none;opacity:0;
    transition:opacity 0.18s ease;
  `,document.body.appendChild(e),requestAnimationFrame(()=>requestAnimationFrame(()=>{e.style.opacity=`1`})),await I(210),e.style.opacity=`0`,setTimeout(()=>e.remove(),230)}async function R(e){window.location.hash!==e&&nt(),window.location.hash===e&&(window.location.hash=`#_nav`,await I(150)),window.location.hash=e,await I(450)}function z(e,t,n=2e3){return new Promise(r=>{let i=()=>document.querySelector(e)||(t?document.querySelector(t):null),a=i();if(a){r(a);return}let o=new MutationObserver(()=>{let e=i();e&&(o.disconnect(),clearTimeout(s),r(e))});o.observe(document.body,{childList:!0,subtree:!0});let s=setTimeout(()=>{o.disconnect(),r(document.getElementById(`page-content`)||document.body)},n)})}var rt={username:`Alp`,archetypeName:`The Elegant Minimalist`,description:`You are drawn to understated sophistication - scents that whisper rather than shout. Your olfactory identity is defined by clean precision, refined florals, and the quiet depth of woody bases. You seek harmony, not performance.`,primaryFamilies:[`floral`,`woody`,`citrus`],sillageProfile:`Medium - intimate yet memorable, discovered only in close proximity.`,notePreferences:{loves:[`iris`,`sandalwood`,`bergamot`,`white musk`],explore:[`oud`,`rose`,`vetiver`,`ambergris`],avoid:[`heavy synthetics`,`excessive sweetness`,`dense patchouli`]},recommendedRegions:[`scandinavian`,`mediterranean`,`eastasia`],updatedAt:Date.now()},it=`monaccord_profile`,at=`monaccord_quiz_state`,B=``,V=null,H=null,U=void 0,W=[{title:`Mon Accord`,run:async()=>(await R(`#landing`),window.scrollTo({top:0,behavior:`auto`}),await z(`#hero-section .hero__content`,`#hero-section`,2e3)),duration:3500},{title:`Six World Regions`,run:async()=>{let e=document.querySelector(`#regions-section`);return e&&await L(e,60),document.querySelector(`.region-card`)||e||document.querySelector(`.page__container`)},duration:4e3},{title:`Spray & Oil Notes`,run:async()=>(document.querySelector(`.notes-popup`)?.remove(),document.querySelector(`.region-format-btn[data-format="spray"]`)?.click(),await I(250),document.querySelector(`.notes-popup`)?.scrollIntoView({behavior:`smooth`,block:`nearest`}),await I(300),document.querySelector(`.notes-popup__inner`)||document.querySelector(`#regions-section`)),duration:4e3},{title:`Take the Quiz`,samePage:!0,run:async()=>{document.querySelector(`.notes-popup`)?.remove();let e=document.querySelector(`#cta-section`);e?e.scrollIntoView({behavior:`smooth`,block:`center`}):window.scrollTo({top:document.body.scrollHeight,behavior:`smooth`}),await I(700);let t=document.querySelector(`#bottom-cta`);return t&&(t.scrollIntoView({behavior:`smooth`,block:`center`}),await I(500)),t||document.querySelector(`#cta-section`)||document.querySelector(`.page__container`)},duration:3e3},{title:`Step 1 - Your Name`,run:async()=>{document.querySelector(`.notes-popup`)?.remove(),localStorage.removeItem(at),window.__retakeQuiz=!0,await R(`#profile`);let e=await z(`.quiz-container`,`.page__container`,2500);await I(300);let t=document.querySelector(`#quiz-username`);if(t){t.focus();for(let e of`Alp`)t.value+=e,t.dispatchEvent(new Event(`input`)),await I(120)}return e},duration:2500},{title:`Step 2 - Scent Families`,run:async()=>{document.querySelector(`#quiz-next`)?.click(),await I(400);let e=[...document.querySelectorAll(`.quiz-grid--families .quiz-option`)];for(let t of e.slice(0,3))t.click(),await I(280);return document.querySelector(`.quiz-grid--families`)||document.querySelector(`.quiz-container`)},duration:2500},{title:`Step 3 - Perfumes You Love`,run:async()=>{document.querySelector(`#quiz-next`)?.click(),await I(450);let e=[...document.querySelectorAll(`.quiz-option--perfume`)];for(let t of e.slice(0,2))t.click(),await I(220);return document.querySelector(`.quiz-perfume-list`)||document.querySelector(`.quiz-container`)},duration:2500},{title:`Step 4 - Performance`,run:async()=>{document.querySelector(`#quiz-next`)?.click(),await I(400);for(let e of[{id:`slider-sillage`,valId:`sillage-val`,target:7},{id:`slider-longevity`,valId:`longevity-val`,target:10},{id:`slider-intensity`,valId:`intensity-val`,target:5}]){let t=document.querySelector(`#`+e.id);t&&(t.value=e.target,t.dispatchEvent(new Event(`input`,{bubbles:!0})),await I(350))}return document.querySelector(`.quiz-sliders`)||document.querySelector(`.quiz-container`)},duration:2500},{title:`Step 5 - Your Personality`,run:async()=>{document.querySelector(`#quiz-next`)?.click(),await I(400),(document.querySelector(`[id="ctx-elegant"]`)||document.querySelector(`.quiz-grid--context .quiz-option`))?.click(),await I(300);let e=document.querySelector(`#quiz-notes`);if(e){e.focus();for(let t of`I love the scent of fresh linen and evening air.`)e.value+=t,e.dispatchEvent(new Event(`input`)),await I(38)}return document.querySelector(`.quiz-grid--context`)||document.querySelector(`.quiz-container`)},duration:5500},{title:`Your Olfactory Profile`,run:async()=>(localStorage.setItem(it,JSON.stringify(rt)),localStorage.setItem(`monaccord_my_perfumes`,JSON.stringify({monAccord:[`scandinavian-spray`,`eastasia-spray`,`mediterranean-oil`],loreal:[`ysl-libre`,`ysl-black-opium`]})),window.__retakeQuiz=!1,window.dispatchEvent(new Event(`hashchange`)),await I(500),window.scrollTo({top:0,behavior:`auto`}),await z(`.profile-result`,`.page__container`,3e3),document.querySelector(`.section-header`)||document.querySelector(`.profile-result`)||document.querySelector(`.page__container`)),duration:4e3},{title:`Identity & Preferences`,run:async()=>{let e=document.querySelector(`.profile-overview`),t=document.querySelector(`.profile-details`)||e;return await L(e),t||document.querySelector(`.profile-result`)},duration:4e3},{title:`Mon Accord Combinations`,run:async()=>{let e=document.querySelectorAll(`.recommendation-grid`)[0],t=e?.previousElementSibling?.previousElementSibling||e,n=e?.querySelector(`.combo-card`)||e;return await L(t||n,80),n||document.querySelector(`.profile-result`)},duration:4e3},{title:`L'Oreal Luxe Combinations`,run:async()=>{let e=document.querySelectorAll(`.recommendation-grid`),t=e.length>1?e[1]:e[0],n=t?.previousElementSibling?.previousElementSibling||t,r=t?.querySelector(`.combo-card`)||t;return await L(n||r,80),r||document.querySelector(`.profile-result`)},duration:4e3},{title:`Shop - Build Your Collection`,run:async()=>{let e=document.querySelector(`[data-buy-combo]`);return e?(e.click(),await I(600)):await R(`#shop`),await z(`.shop-region-card`,`.shop-layout`,2e3)},duration:4e3},{title:`Add to Cart`,run:async()=>{let e=[...document.querySelectorAll(`.shop-product__btn:not(.shop-product__btn--added)`)];e[0]&&(e[0].click(),await I(180)),e[1]&&(e[1].click(),await I(180));let t=document.querySelector(`.shop-cart`);return t?.scrollIntoView({behavior:`smooth`,block:`center`}),await I(300),t||document.querySelector(`.page__container`)},duration:4e3},{title:`Now Available`,run:async()=>(document.querySelector(`#btn-confirm-order`)?.click(),await I(350),document.querySelector(`.order-confirmed-modal`)||document.querySelector(`.modal-overlay`)||document.querySelector(`.page__container`)),duration:4e3},{title:`The Layering Lab`,run:async()=>{document.querySelector(`.modal-overlay`)?.remove(),await R(`#lab`),window.scrollTo({top:0,behavior:`auto`}),await I(200);let e=[...document.querySelectorAll(`.lab-add-btn:not(.lab-add-btn--added)`)];return e[0]&&(e[0].click(),await I(200)),e[2]&&(e[2].click(),await I(200)),await I(300),document.querySelector(`#lab-layers`)||document.querySelector(`#lab-add-section`)},duration:5e3},{title:`Virtual Scent Simulation`,samePage:!0,run:async()=>{let e=document.querySelector(`#btn-simulate`);e&&(await L(e),e.innerHTML=`<span class=”loading-spinner”></span> Simulating...`,e.disabled=!0,await I(1200),e.innerHTML=`✦ Simulate Scent`,e.disabled=!1);let t=document.querySelector(`#simulation-result`);if(!t){t=document.createElement(`div`),t.id=`simulation-result`,t.className=`ai-response mt-lg`;let e=document.querySelector(`#lab-actions`);e?e.insertAdjacentElement(`afterend`,t):document.querySelector(`#lab-layers`)?.appendChild(t)}return t&&(t.style.display=``,t.innerHTML=`
        ...
        `,await L(t)),t||document.querySelector(`#lab-layers`)||document.querySelector(`.page__container`)},duration:4500},{title:`AI Contextual Advisor`,run:async()=>{let e=document.querySelector(`.lab-advisor`);await L(e),document.querySelector(`#mood-chips .lab-chip`)?.click(),await I(100),document.querySelector(`#occasion-chips .lab-chip`)?.click(),await I(100),document.querySelector(`#season-chips .lab-chip`)?.click(),await I(250);let t=document.querySelector(`#advisor-result`);return t||(t=document.createElement(`div`),t.id=`advisor-result`,t.className=`lab-advisor__result mt-lg`,document.querySelector(`.lab-advisor__form`)?.insertAdjacentElement(`afterend`,t)),t&&(t.style.display=``,t.innerHTML=`
        ...
        `),t||document.querySelector(`.lab-advisor__form`)||e||document.querySelector(`.page__container`)},duration:5e3},{title:`Save to Vault`,samePage:!0,run:async()=>{let e=await z(`#btn-save-formula`,`.page__container`,1200);e&&e.id===`btn-save-formula`&&(await L(e),e.click(),await I(450));let t=await z(`#save-vault-overlay .modal`,`#save-vault-overlay`,2e3),n=document.querySelector(`#sv-name`);if(n){n.value=``,n.focus();for(let e of`Evening Velvet`)n.value+=e,n.dispatchEvent(new Event(`input`)),await I(55);await I(300)}let r=document.querySelector(`#sv-folder-dropdown`);if(r){let e=[...r.options].find(e=>e.value===`evening`||e.text.toLowerCase().includes(`evening`));e&&(r.value=e.value,r.dispatchEvent(new Event(`change`))),await I(400)}return t||document.querySelector(`#save-vault-overlay`)||document.querySelector(`.page__container`)},duration:4500},{title:`Your Vault`,run:async()=>(document.querySelector(`#sv-confirm`)?.click(),await I(350),await R(`#vault`),await z(`.vault-folders-col`,`.page__container`,2e3)),duration:4e3},{title:`My Perfumes`,samePage:!0,run:async()=>{let e=document.querySelector(`.vault-myperfumes-col`);return window.scrollTo({top:0,behavior:`smooth`}),await I(400),e||document.querySelector(`.vault-main-layout`)||document.querySelector(`.page__container`)},duration:4e3},{title:`Trending Combinations`,run:async()=>{await R(`#community`);let e=document.querySelector(`.duygu-card__photo`);return e&&!e.complete&&await new Promise(t=>{e.onload=t,e.onerror=t,setTimeout(t,3e3)}),await I(200),await z(`.community-trending-item`,`.community-trending-list`,2e3)},duration:4e3},{title:`Today's Selection`,samePage:!0,run:async()=>{let e=document.querySelector(`.community-panel--selection`);return await L(e),e||document.querySelector(`.community-top-grid`)||document.querySelector(`.page__container`)},duration:4e3},{title:`Duygu's Choice`,samePage:!0,run:async()=>{let e=document.querySelector(`.community-duygu-choice`);return e?(await L(e),e):document.querySelector(`.community-right-col`)||document.querySelector(`.page__container`)},duration:4e3},{title:`Community Discussion`,samePage:!0,run:async()=>{let e=document.querySelector(`.community-discussion-section`);return await L(e,60),document.querySelector(`.community-post-item`)?.click(),await I(300),document.querySelector(`.community-discussion-layout`)||e||document.querySelector(`.page__container`)},duration:4500}],ot=[3e3,3500,3500,3e3,2500,2500,2500,2500,5e3,4500,3e3,4e3,3e3,3500,3500,3500,4500,4500,4e3,3500,3500,3500,3500,3e3,3500,3500];W.forEach((e,t)=>{e.duration=ot[t]??e.duration});function st(){return document.getElementById(`dt-card`)}function ct(){return document.getElementById(`dt-prog`)}function lt(){return document.getElementById(`dt-cd`)}function ut(){return document.getElementById(`dt-dots`)}function dt(){return document.getElementById(`dt-count`)}function ft(){return document.getElementById(`dt-title`)}function pt(){return document.getElementById(`dt-next`)}var mt=24,G=20;function ht(e){let t=st();if(!t)return;let n=window.innerWidth,r=window.innerHeight,i=mt,a=Math.min(t.offsetWidth||288,n-i*2),o=t.offsetHeight||126,s=e?e.getBoundingClientRect():null;if(!s||(s.width||0)===0||(s.height||0)===0||s.bottom<=0||s.top>=r||s.right<=0||s.left>=n){let e=Math.min(n-i*2,320),t=Math.min(r-i*2,160);s={left:(n-e)/2,top:(r-t)/2,right:(n+e)/2,bottom:(r+t)/2}}let c=s.right-s.left,l=s.bottom-s.top,u=[{placement:`bottom`,x:s.left+(c-a)/2,y:s.bottom+G},{placement:`top`,x:s.left+(c-a)/2,y:s.top-o-G},{placement:`right`,x:s.right+G,y:s.top+(l-o)/2},{placement:`left`,x:s.left-a-G,y:s.top+(l-o)/2},{placement:`bottom-right`,x:s.right-a,y:s.bottom+G},{placement:`bottom-left`,x:s.left,y:s.bottom+G},{placement:`top-right`,x:s.right-a,y:s.top-o-G},{placement:`top-left`,x:s.left,y:s.top-o-G}];function d(e){let t=$e(e.x,i,Math.max(i,n-a-i)),s=$e(e.y,i,Math.max(i,r-o-i));return{placement:e.placement,left:t,top:s,right:t+a,bottom:s+o,clampDelta:Math.abs(e.x-t)+Math.abs(e.y-s)}}function f(e){let t=(e.left+e.right)/2,n=(e.top+e.bottom)/2,r=(s.left+s.right)/2,i=(s.top+s.bottom)/2;return Math.hypot(t-r,n-i)}function p(e){let t=Math.max(e.left,s.left),n=Math.max(e.top,s.top),r=Math.min(e.right,s.right),i=Math.min(e.bottom,s.bottom);return r>t&&i>n?(r-t)*(i-n):0}let m=u.map(e=>{let t=d(e);return{rect:t,score:p(t)*4+f(t)+t.clampDelta*6}}).sort((e,t)=>e.score-t.score)[0];t.dataset.placement=m.rect.placement,t.style.transform=`translate(${Math.round(m.rect.left)}px, ${Math.round(m.rect.top)}px)`}function gt(e){let t=W[e],n=W.length,r=dt(),i=ft(),a=ut(),o=ct(),s=pt();if(r&&(r.textContent=`${String(e+1).padStart(2,`0`)} / ${String(n).padStart(2,`0`)}`),i&&(i.textContent=t.title),o&&(o.style.width=`${(e+1)/n*100}%`),s&&(s.textContent=e===n-1?`Finish`:`Next`),a){let t=Math.min(n,12),r=Math.max(0,e-Math.floor(t/2));a.innerHTML=Array.from({length:t},(t,n)=>{let i=n+r;return`<span class="dt-dot ${i===e?`dt-dot--active`:i<e?`dt-dot--done`:`dt-dot--pending`}"></span>`}).join(``)}}var K=null;function _t(e){clearInterval(K);let t=lt();if(!t)return;t.style.width=`0%`;let n=Date.now();K=setInterval(()=>{let r=Math.min(100,(Date.now()-n)/e*100);t.style.width=r+`%`,r>=100&&clearInterval(K)},40)}var q=!1,J=0,Y=null;async function vt(e){if(!q)return;J=e;let t=W[e];gt(e);let n;try{n=await t.run()}catch{n=document.getElementById(`page-content`)||document.body}q&&(ht(n),_t(t.duration),clearTimeout(Y),Y=setTimeout(()=>yt(),t.duration))}function yt(){q&&(clearTimeout(Y),J>=W.length-1?X():vt(J+1))}function bt(){xt(),Ct();let e=document.createElement(`div`);e.id=`dt-card`,e.innerHTML=`
  ...
  `,document.body.appendChild(e),document.getElementById(`dt-skip`).addEventListener(`click`,X),document.getElementById(`dt-next`).addEventListener(`click`,()=>{clearTimeout(Y),clearInterval(K),yt()})}function xt(){document.getElementById(`dt-card`)?.remove(),document.getElementById(`dt-flash`)?.remove()}function St(){q&&X(),B=window.location.hash||``,V=et(localStorage),H=et(sessionStorage),U=window.__retakeQuiz,localStorage.clear(),sessionStorage.clear(),delete window.__retakeQuiz,localStorage.removeItem(it),q=!0,J=0,bt(),vt(0)}function X(){q=!1,clearTimeout(Y),clearInterval(K),tt(localStorage,V),tt(sessionStorage,H),V=null,H=null,U===void 0?delete window.__retakeQuiz:window.__retakeQuiz=U,U=void 0,(B||``)!==(window.location.hash||``)&&(window.location.hash=B||`#landing`),B=``,xt()}function Ct(){if(document.getElementById(`dt-styles`))return;let e=document.createElement(`style`);e.id=`dt-styles`,e.textContent=`
    #dt-card {
      position: fixed;
      z-index: 99995;
      top: 0;
  ...
  `,document.head.appendChild(e)}var Z=document.getElementById(`app`);function Q(e){document.documentElement.setAttribute(`data-theme`,e),t.set(`theme`,e)}function wt(){let e=t.get(`theme`,`dark`);document.documentElement.setAttribute(`data-theme`,e)}var Tt={"":f,"#landing":f,"#profile":O,"#lab":Ce,"#vault":Me,"#shop":Ye,"#community":Be};function Et(){return window.location.hash||``}function Dt(e){window.location.hash=e}function $(){let e=Et(),t=Tt[e]||f;Z.innerHTML=``,Z.appendChild(n(Dt,e));let r=document.createElement(`div`);if(r.className=`page page-enter`,r.id=`page-content`,t(r,Dt),Z.appendChild(r),!document.querySelector(`.toast-container`)){let e=document.createElement(`div`);e.className=`toast-container`,e.id=`toast-container`,document.body.appendChild(e)}window.scrollTo({top:0,behavior:`auto`})}window.showToast=function(e,t=`success`){let n=document.getElementById(`toast-container`);if(!n)return;let r=document.createElement(`div`);r.className=`toast toast--${t}`,r.innerHTML=`<span class="toast__message">${e}</span>`,n.appendChild(r),setTimeout(()=>{r.style.animation=`fadeOut 0.3s var(--ease-out) forwards`,setTimeout(()=>r.remove(),300)},3e3)},window.showSettings=function(){let e=document.querySelector(`.modal-overlay`);e&&e.remove();let n=t.get(`theme`,`dark`),r=document.createElement(`div`);r.className=`modal-overlay`,r.id=`settings-modal`,r.innerHTML=`
    <div class="modal">
      <div class="modal__header">
