...
Keep responses concise but rich. Use fragrance terminology naturally.`,re=e=>({contents:[{parts:[{text:e}]}],systemInstruction:{parts:[{text:T}]}});function ie(e,t){return t===429||e.includes(`429`)||e.includes(`quota`)||e.includes(`RESOURCE_EXHAUSTED`)?`rate-limit`:t===401||t===403||e.includes(`API_KEY`)||e.includes(`API key`)?`invalid-key`:e.includes(`fetch`)||e.includes(`network`)||e.includes(`Failed to fetch`)?`network`:`unknown`}function E(e,t){switch(e){case`rate-limit`:return`Rate limit reached. Please wait ${t||30} seconds and try again.`;case`invalid-key`:return`AI service key is invalid. Please contact the site owner.`;case`network`:return`Network error. Please check your connection and try again.`;default:return`AI request failed. Please try again.`}}async function D(e){let n=re(e);{let e={"Content-Type":`application/json`};e[`X-Worker-Token`]=te;let t=await fetch(C,{method:`POST`,headers:e,body:JSON.stringify(n)}),r=await t.json();if(!t.ok){let e=r?.error?.message||`HTTP ${t.status}`;throw Object.assign(Error(e),{status:t.status})}return r}let r=t.getApiKey();if(!r)throw Object.assign(Error(`no-api-key`),{status:0});let i=await fetch(`${w}/${ne}:generateContent?key=${r}`,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify(n)}),a=await i.json();if(!i.ok){let e=a?.error?.message||`HTTP ${i.status}`;throw Object.assign(Error(e),{status:i.status})}return a}async function O(e,t=2){try{let t=await D(e),n=t?.candidates?.[0]?.content?.parts?.[0]?.text;return n?{success:!0,text:n}:(console.error(`Empty Gemini response`,t),{success:!1,error:`empty-response`,text:`AI returned an empty response. Please try again.`})}catch(n){let r=n.message||String(n),i=n.status??0;if(console.error(`AI error:`,r),r===`no-api-key`)return{success:!1,error:`no-api-key`,text:`AI features are not available. Please contact the site owner.`};let a=ie(r,i);if(a===`rate-limit`&&t>0){let n=r.match(/retry[^0-9]*(\d+)/i),i=n?parseInt(n[1]):30;return console.log(`Rate limited. Waiting ${i}s… (${t} retries left)`),await new Promise(e=>setTimeout(e,(i+2)*1e3)),O(e,t-1)}let o=r.match(/retry[^0-9]*(\d+)/i);return{success:!1,error:a,text:E(a,o?parseInt(o[1]):void 0)}}}function k(){return!0}async function ae(e){let n=await O(A(e));if(n.success){let r=oe(n.text,e);return t.setProfile(r),t.addInteraction({type:`profile-created`,profile:r.archetype}),{success:!0,profile:r}}return{success:!1,error:n.text}}function A(e){let t=e.scentFamilies?.join(`, `)||`not specified`,n=(e.knownPerfumes||[]).map(e=>{let t=o.find(t=>t.id===e);return t?`${t.brand} ${t.name} (${t.family})`:e}).join(`, `)||`none selected`,r=e.interestedRegions?.join(`, `)||`all regions`;return`Analyze this user's fragrance preferences and create their olfactory profile.
        ...
        <div class="quiz-container">
          ...
          <div class="quiz-actions">
            ...
            ${a<j?`<button class="btn btn--primary" id="quiz-next">Continue →</button>`:`<button class="btn btn--primary" id="quiz-finish">
                  ${k()?`✦ Generate My Profile`:`✦ Generate Profile`}
                </button>`}
          </div>
        </div>
      </div>
    `,be(),me(a,o,e);let r=e.querySelector(`#quiz-back`),i=e.querySelector(`#quiz-next`),c=e.querySelector(`#quiz-finish`);r&&r.addEventListener(`click`,()=>{M(a,o,e),a--,s()}),i&&i.addEventListener(`click`,()=>{M(a,o,e),a++,s()}),c&&c.addEventListener(`click`,async()=>{if(M(a,o,e),!k()){window.showToast(`Please set your Gemini API key in Settings first.`,`error`),window.showSettings();return}c.disabled=!0,c.innerHTML=`<span class="loading-spinner"></span> Generating...`;let r=e.querySelector(`#generation-status`);r||(r=document.createElement(`p`),r.id=`generation-status`,r.style.cssText=`font-size: var(--text-xs); color: var(--text-tertiary); text-align: center; margin-top: var(--space-sm);`,c.parentElement.appendChild(r)),r.textContent=`Analyzing your preferences with AI...`;let i=await ae(o);i.success?(i.profile.username=o.username||`Anonymous`,t.setProfile(i.profile),t.clearQuizState(),he(e,i.profile,n),window.showToast(`Your olfactory profile has been created! ✦`)):(c.disabled=!1,c.innerHTML=`✦ Generate My Profile`,r.textContent=``,window.showToast(i.error||`Failed to generate profile.`,`error`))})}s()}function fe(e,t){switch(e){case 1:return`
...
`),o=await O(`Based on this user's fragrance journey, suggest a fresh remix.
...
Respond in EXACTLY this JSON format (no markdown, no code blocks):
{
  ...
  "scentDescription": "Vivid 2-sentence sensory preview"
    ...
    `,$e(),ae(n,i)}function te(e){let t=[...i.map(e=>({id:e.id,label:`${r.find(t=>t.id===e.region)?.icon||``} ${e.name} (${e.format})`,group:`Mon Accord`})),...o.map(e=>({id:e.id,label:`${e.brand} — ${e.name}`,group:`L'Oreal Luxe`}))];return`
  ...
  `,document.body.appendChild(e),document.getElementById(`dt-skip`).addEventListener(`click`,X),document.getElementById(`dt-next`).addEventListener(`click`,()=>{clearTimeout(Y),clearInterval(K),At()})}function Mt(){document.getElementById(`dt-card`)?.remove(),document.getElementById(`dt-flash`)?.remove()}function Nt(){q&&X(),B=window.location.hash||``,V=dt(localStorage),H=dt(sessionStorage),U=window.__retakeQuiz,localStorage.clear(),sessionStorage.clear(),delete window.__retakeQuiz,localStorage.removeItem(ht),q=!0,J=0,jt(),kt(0)}function X(){q=!1,clearTimeout(Y),clearInterval(K),ft(localStorage,V),ft(sessionStorage,H),V=null,H=null,U===void 0?delete window.__retakeQuiz:window.__retakeQuiz=U,U=void 0,(B||``)!==(window.location.hash||``)&&(window.location.hash=B||`#landing`),B=``,Mt()}function Pt(){if(document.getElementById(`dt-styles`))return;let e=document.createElement(`style`);e.id=`dt-styles`,e.textContent=`
    #dt-card {
      position: fixed;
      z-index: 99995;
      top: 0;
