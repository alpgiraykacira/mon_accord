(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var e=`monaccord_`,t={get(t,n=null){try{let r=localStorage.getItem(e+t);return r?JSON.parse(r):n}catch{return n}},set(t,n){try{localStorage.setItem(e+t,JSON.stringify(n))}catch(e){console.warn(`Storage write failed:`,e)}},remove(t){localStorage.removeItem(e+t)},getProfile(){return this.get(`profile`,null)},setProfile(e){this.set(`profile`,{...e,updatedAt:Date.now()})},getVault(){return this.get(`vault`,[])},saveFormula(e){let t=this.getVault(),n=t.findIndex(t=>t.id===e.id);return n>=0?t[n]={...e,savedAt:Date.now()}:t.push({...e,id:e.id||`f-`+Date.now(),savedAt:Date.now()}),this.set(`vault`,t),this.addInteraction({type:`save`,formulaId:e.id,timestamp:Date.now()}),t},removeFormula(e){let t=this.getVault().filter(t=>t.id!==e);return this.set(`vault`,t),t},getInteractions(){return this.get(`interactions`,[])},addInteraction(e){let t=this.getInteractions();t.push({...e,timestamp:e.timestamp||Date.now()}),t.length>500&&t.splice(0,t.length-500),this.set(`interactions`,t)},getLikes(){return this.get(`likes`,[])},toggleLike(e){let t=this.getLikes(),n=t.indexOf(e);return n>=0?(t.splice(n,1),this.addInteraction({type:`unlike`,formulaId:e,timestamp:Date.now()})):(t.push(e),this.addInteraction({type:`like`,formulaId:e,timestamp:Date.now()})),this.set(`likes`,t),t},isLiked(e){return this.getLikes().includes(e)},getOwnedPerfumes(){let e=this.get(`my_perfumes`,null);return Array.isArray(e)?{monAccord:e.map(e=>e.id),loreal:[]}:e||{monAccord:[],loreal:[]}},setOwnedPerfumes(e){this.set(`my_perfumes`,e)},getApiKey(){return this.get(`gemini_api_key`,``)},setApiKey(e){this.set(`gemini_api_key`,e)},getQuizState(){return this.get(`quiz_state`,null)},setQuizState(e){this.set(`quiz_state`,e)},clearQuizState(){this.remove(`quiz_state`)},setPendingShopCart(e){this.set(`pending_shop_cart`,e)},consumePendingShopCart(){let e=this.get(`pending_shop_cart`,[]);return this.remove(`pending_shop_cart`),Array.isArray(e)?e:[]},getShopCart(){let e=this.get(`shop_cart`,[]);return Array.isArray(e)?e:[]},setShopCart(e){this.set(`shop_cart`,e)},clearShopCart(){this.remove(`shop_cart`)},getUsername(){return this.getProfile()?.username||`Anonymous`},getPosts(){return this.get(`community_posts`,[])},setPosts(e){this.set(`community_posts`,e)},getPostLikes(){return this.get(`post_likes`,[])},togglePostLike(e){let t=this.getPostLikes(),n=t.indexOf(e);return n>=0?t.splice(n,1):t.push(e),this.set(`post_likes`,t),t},isPostLiked(e){return this.getPostLikes().includes(e)}};function n(e,n){let r=document.createElement(`nav`);return r.className=`navbar`,r.id=`main-nav`,t.getProfile(),r.innerHTML=`
  ...
  `,document.head.appendChild(e)}var m=`/mon_accord/assets/aromatic-DWjP6BVr.png`,h=`/mon_accord/assets/citrus-D8TDl6xe.png`,g=`/mon_accord/assets/floral-glVrpiWz.png`,_=`/mon_accord/assets/fresh-BFmGFrMP.png`,v=`/mon_accord/assets/gourmand-CGXhpT3R.png`,y=`/mon_accord/assets/green-WRJ2vWnP.png`,b=`/mon_accord/assets/musky-DamQXXMh.png`,x=`/mon_accord/assets/oriental-FtX0uIsC.png`,S=`/mon_accord/assets/spicy-CPptZyIq.png`,ee=`/mon_accord/assets/woody-B8ZJqfkR.png`,C=`https://mon-accord-ai.alpgiraykacira.workers.dev`,te=`https://generativelanguage.googleapis.com/v1beta/models`,w=`gemini-2.5-flash`,ne=`You are Mon Accord's AI Perfume Advisor — a world-class fragrance expert specializing in scent layering and olfactory profiling.
...
}`}function A(e,t){try{let n=e.trim();return n.startsWith("```")&&(n=n.replace(/^```(?:json)?\n?/,``).replace(/\n?```$/,``)),{...JSON.parse(n),createdAt:Date.now(),quizAnswers:t}}catch(e){return console.error(`Failed to parse profile JSON:`,e),oe(t)}}function oe(e){let t=e.scentFamilies||[`woody`],n=`woody-earthy`;t.includes(`oriental`)||t.includes(`spicy`)?n=`warm-oriental`:t.includes(`fresh`)||t.includes(`citrus`)?n=`fresh-aquatic`:t.includes(`floral`)?n=`floral-romantic`:t.includes(`gourmand`)&&(n=`gourmand-cozy`);let r=c.find(e=>e.id===n)||c[0];return{archetype:n,archetypeName:r.name,description:r.description,primaryFamilies:t.slice(0,3),sillageProfile:e.sillage>7?`high`:e.sillage>4?`medium`:`low`,notePreferences:{loves:[],explore:[],avoid:[]},recommendedRegions:e.interestedRegions||[`mediterranean`,`eastasia`],signatureBlend:null,createdAt:Date.now(),quizAnswers:e}}async function se(e){return await D(`You are simulating the scent experience of this fragrance combination. Describe what the wearer would smell in three temporal phases.
...
Be poetic but precise. Help the reader "smell" this blend through words. Keep it under 200 words total.`)}var ce=Object.fromEntries([`fresh`,`floral`,`woody`,`oriental`,`citrus`,`gourmand`,`green`,`aromatic`,`spicy`,`musky`].map(e=>[e,new URL(Object.assign({"../assets/quiz_scent_families/aromatic.png":m,"../assets/quiz_scent_families/citrus.png":h,"../assets/quiz_scent_families/floral.png":g,"../assets/quiz_scent_families/fresh.png":_,"../assets/quiz_scent_families/gourmand.png":v,"../assets/quiz_scent_families/green.png":y,"../assets/quiz_scent_families/musky.png":b,"../assets/quiz_scent_families/oriental.png":x,"../assets/quiz_scent_families/spicy.png":S,"../assets/quiz_scent_families/woody.png":ee})[`../assets/quiz_scent_families/${e}.png`],import.meta.url).href])),le={elegant:new URL(`/mon_accord/assets/elegant_classic-DRER5jBh.png`,``+import.meta.url).href,adventurous:new URL(`/mon_accord/assets/adventurous_bold-SjNP7L-r.png`,``+import.meta.url).href,romantic:new URL(`/mon_accord/assets/romantic_dreamy-CLUpTJmt.png`,``+import.meta.url).href,minimalist:new URL(`/mon_accord/assets/minimalist_clean-BrPXV53H.png`,``+import.meta.url).href,creative:new URL(`/mon_accord/assets/creative_expressive-Cd5TtKJD.png`,``+import.meta.url).href,confident:new URL(`/mon_accord/assets/confident_powerful-w73bkLNN.png`,``+import.meta.url).href},j=5;function ue(e,n){let r=t.getProfile();if(r&&!window.__retakeQuiz){ve(e,r,n);return}window.__retakeQuiz=!1;let i=t.getQuizState(),a=i?.step||1,o=i?.answers||{username:``,scentFamilies:[],knownPerfumes:[],sillage:5,longevity:5,intensity:5,personality:``,notes:``};function s(){t.setQuizState({step:a,answers:o}),e.innerHTML=`
      <div class="page__container">
        <div class="quiz-container">
          <div class="quiz-stepper">
            ${Array.from({length:j},(e,t)=>{let n=t+1,r=n<a?`completed`:n===a?`active`:``,i=n<a?`done`:``;return`<div class="quiz-stepper__item ${r}"><div class="quiz-stepper__circle">${n<a?`✓`:String(n)}</div></div>${t<j-1?`<div class="quiz-stepper__line ${i}"></div>`:``}`}).join(``)}
          </div>
          ...
          <div class="quiz-actions">
            ...
            ${a<j?`<button class="btn btn--primary" id="quiz-next">Continue →</button>`:`<button class="btn btn--primary" id="quiz-finish">
                  ${O()?`✦ Generate My Profile`:`✦ Generate Profile`}
                </button>`}
          </div>
        </div>
      </div>
    `,ye(),pe(a,o,e);let r=e.querySelector(`#quiz-back`),i=e.querySelector(`#quiz-next`),c=e.querySelector(`#quiz-finish`);r&&r.addEventListener(`click`,()=>{M(a,o,e),a--,s()}),i&&i.addEventListener(`click`,()=>{M(a,o,e),a++,s()}),c&&c.addEventListener(`click`,async()=>{if(M(a,o,e),!O()){window.showToast(`Please set your Gemini API key in Settings first.`,`error`),window.showSettings();return}c.disabled=!0,c.innerHTML=`<span class="loading-spinner"></span> Generating...`;let r=e.querySelector(`#generation-status`);r||(r=document.createElement(`p`),r.id=`generation-status`,r.style.cssText=`font-size: var(--text-xs); color: var(--text-tertiary); text-align: center; margin-top: var(--space-sm);`,c.parentElement.appendChild(r)),r.textContent=`Analyzing your preferences with AI...`;let i=await k(o);i.success?(i.profile.username=o.username||`Anonymous`,t.setProfile(i.profile),t.clearQuizState(),me(e,i.profile,n),window.showToast(`Your olfactory profile has been created! ✦`)):(c.disabled=!1,c.innerHTML=`✦ Generate My Profile`,r.textContent=``,window.showToast(i.error||`Failed to generate profile.`,`error`))})}s()}function de(e,t){switch(e){case 1:return`
        ...
        <div class="quiz-grid quiz-grid--families">
          ${a.map(e=>`
            <div class="quiz-option quiz-option--img-card ${t.scentFamilies.includes(e.id)?`quiz-option--selected`:``}" data-value="${e.id}" id="family-${e.id}">
              <img class="quiz-option__img" src="${ce[e.id]}" alt="${e.name}" loading="lazy" decoding="async" />
            </div>
          `).join(``)}
        </div>
      `;case 3:return`
        ...
        <div class="quiz-grid quiz-grid--context">
          ${[{id:`elegant`,name:`Elegant & Classic`,desc:`Timeless sophistication`},{id:`adventurous`,name:`Adventurous & Bold`,desc:`Love discovering the new`},{id:`romantic`,name:`Romantic & Dreamy`,desc:`Soft, poetic, emotional`},{id:`minimalist`,name:`Minimalist & Clean`,desc:`Less is more`},{id:`creative`,name:`Creative & Expressive`,desc:`Unique, unconventional`},{id:`confident`,name:`Confident & Powerful`,desc:`Commands attention`}].map(e=>`
            <div class="quiz-option quiz-option--img-card ${t.personality===e.id?`quiz-option--selected`:``}" data-value="${e.id}" id="ctx-${e.id}">
              <img class="quiz-option__img" src="${le[e.id]}" alt="${e.name}" loading="lazy" decoding="async" />
            </div>
          `).join(``)}
        </div>
      ...
      `}}function fe(e,t){let n=t?o.filter(e=>e.name.toLowerCase().includes(t.toLowerCase())||e.brand.toLowerCase().includes(t.toLowerCase())):o,r={};return n.forEach(e=>{r[e.brand]||(r[e.brand]=[]),r[e.brand].push(e)}),Object.entries(r).map(([t,n])=>`
      ...
      <div class="profile-result">
        ...
        <div class="mt-xl">
          ...
          <div class="recommendation-grid">
            ${i.map(e=>he(e,{showBuyButton:!0})).join(``)}
          </div>
        </div>
  ...
  `,document.head.appendChild(e)}var be=[{id:`confident`,name:`Confident`,icon:``,description:`Bold and commanding`},{id:`romantic`,name:`Romantic`,icon:``,description:`Soft and alluring`},{id:`calm`,name:`Calm`,icon:``,description:`Peaceful and centered`},{id:`energetic`,name:`Energetic`,icon:``,description:`Vibrant and lively`},{id:`mysterious`,name:`Mysterious`,icon:``,description:`Enigmatic and deep`},{id:`playful`,name:`Playful`,icon:``,description:`Light and fun`}],xe=[{id:`office`,name:`Office / Work`,icon:``},{id:`date-night`,name:`Date Night`,icon:``},{id:`casual`,name:`Casual Outing`,icon:``},{id:`formal`,name:`Formal Event`,icon:``},{id:`outdoor`,name:`Outdoor Adventure`,icon:``},{id:`cozy`,name:`Cozy Night In`,icon:``}],Se=[{id:`spring`,name:`Spring`,icon:``},{id:`summer`,name:`Summer`,icon:``},{id:`autumn`,name:`Autumn`,icon:``},{id:`winter`,name:`Winter`,icon:``}],N=[{id:`whisper`,name:`Whisper`,value:1,description:`Barely there, intimate`},{id:`soft`,name:`Soft`,value:3,description:`Close range only`},{id:`moderate`,name:`Moderate`,value:5,description:`Noticeable arm's length`},{id:`present`,name:`Present`,value:7,description:`Fills the room gently`},{id:`bold`,name:`Bold`,value:9,description:`Leaves a trail`}];function Ce(){let e=t.getOwnedPerfumes(),n=[];if(e.monAccord?.length){let t=e.monAccord.map(e=>i.find(t=>t.id===e)?.name).filter(Boolean);t.length&&n.push(`Mon Accord owned: ${t.join(`, `)}`)}if(e.loreal?.length){let t=e.loreal.map(e=>o.find(t=>t.id===e)?.name).filter(Boolean);t.length&&n.push(`L'Oréal Luxe owned: ${t.join(`, `)}`)}return n.length?n.join(`
`):null}async function we(e){let n=t.getProfile(),r=Ce(),a=await D(`Based on the user's context, recommend a Mon Accord layering formula.
...
Respond in EXACTLY this JSON format (no markdown, no code blocks):
{
  ...
  `,document.head.appendChild(e)}var De={scandinavian:new URL(`/mon_accord/assets/scandinavian-Ufy3hcXT.png`,``+import.meta.url).href,eastasia:new URL(`/mon_accord/assets/east_asia-ZOLilsJx.png`,``+import.meta.url).href,southafrica:new URL(`/mon_accord/assets/south_africa-DiWwpsLm.png`,``+import.meta.url).href,mediterranean:new URL(`/mon_accord/assets/mediterranean-BMwBLMbR.png`,``+import.meta.url).href,southamerica:new URL(`/mon_accord/assets/south_america-0d3PjZaO.png`,``+import.meta.url).href,middleeast:new URL(`/mon_accord/assets/middle_east-D6fedABT.png`,``+import.meta.url).href},Oe={scandinavian:new URL(`/mon_accord/assets/scandinavian-5ibAHdAH.png`,``+import.meta.url).href,eastasia:new URL(`/mon_accord/assets/east_asia-DZK4DeZe.png`,``+import.meta.url).href,southafrica:new URL(`/mon_accord/assets/south_africa-DwLt2ni5.png`,``+import.meta.url).href,mediterranean:new URL(`/mon_accord/assets/mediterranean-fZMNFcGl.png`,``+import.meta.url).href,southamerica:new URL(`/mon_accord/assets/south_america-PQykoay7.png`,``+import.meta.url).href,middleeast:new URL(`/mon_accord/assets/middle_east-DEEY7Abs.png`,``+import.meta.url).href},ke={scandinavian:new URL(`/mon_accord/assets/scandinavian-T91uXqs1.png`,``+import.meta.url).href,eastasia:new URL(`/mon_accord/assets/east_asia-C6p3o17-.png`,``+import.meta.url).href,southafrica:new URL(`/mon_accord/assets/south_africa-DqiVXp2O.png`,``+import.meta.url).href,mediterranean:new URL(`/mon_accord/assets/mediterranean-zaCtkOEY.png`,``+import.meta.url).href,southamerica:new URL(`/mon_accord/assets/south_america-BEKaFRIH.png`,``+import.meta.url).href,middleeast:new URL(`/mon_accord/assets/middle_east-Df_0XU0d.png`,``+import.meta.url).href},Ae=`lab_session_state`;function je(e){try{sessionStorage.setItem(Ae,JSON.stringify(e))}catch{}}function Me(){try{return JSON.parse(sessionStorage.getItem(Ae)||`null`)}catch{return null}}function Ne(e,t){let n=Me(),a=n?.layers||[],o=n?.scentSimulation||null,s=!1,c=n?.contextResult||null,u=!1,d=n?.selectedMood||null,f=n?.selectedOccasion||null,p=n?.selectedSeason||null,m=n?.selectedTime||null,h=n?.selectedIntensity||5;function g(){je({layers:a,scentSimulation:o,contextResult:c,selectedMood:d,selectedOccasion:f,selectedSeason:p,selectedTime:m,selectedIntensity:h})}let _=JSON.parse(sessionStorage.getItem(`labPending`)||`[]`);_.length>0&&(_.forEach(e=>{let t=l(e);t&&!a.find(t=>t.perfumeId===e)&&a.push({perfumeId:e,amount:t.format===`spray`?2:3,unit:t.format===`spray`?`sprays`:`drops`})}),sessionStorage.removeItem(`labPending`),g());function v(){e.innerHTML=`
      <div class="page__container">
                      ...
                      <div class="lab-bottle-row">
                        ${t?`
                          <button class="lab-bottle-btn ${r?`lab-bottle-btn--added`:``}" data-id="${t.id}" style="--region-color: ${e.color};" title="${e.name} Spray">
                            <img src="${Oe[e.id]}" alt="${e.name} Spray" class="lab-bottle-img" loading="lazy" decoding="async" />
                        ...
                        ${n?`
                          <button class="lab-bottle-btn ${o?`lab-bottle-btn--added`:``}" data-id="${n.id}" style="--region-color: ${e.color};" title="${e.name} Oil">
                            <img src="${ke[e.id]}" alt="${e.name} Oil" class="lab-bottle-img" loading="lazy" decoding="async" />
              ...
              <div class="lab-advisor__form">
                <div class="input-group">
                  ...
                  <div class="lab-advisor__chips" id="mood-chips">
                    ${be.map(e=>`<button class="lab-chip ${d===e.id?`lab-chip--active`:``}" data-value="${e.id}">${e.icon} ${e.name}</button>`).join(``)}
                  </div>
                </div>
                ...
                <div class="input-group">
                  ...
                  <div class="lab-advisor__chips" id="occasion-chips">
                    ${xe.map(e=>`<button class="lab-chip ${f===e.id?`lab-chip--active`:``}" data-value="${e.id}">${e.icon} ${e.name}</button>`).join(``)}
                  </div>
                </div>
                ...
                <div class="input-group">
                  ...
                  <div class="lab-advisor__chips" id="season-chips">
                    ${Se.map(e=>`<button class="lab-chip ${p===e.id?`lab-chip--active`:``}" data-value="${e.id}">${e.icon} ${e.name}</button>`).join(``)}
                  </div>
                </div>
            ...
            <div class="lab-layers" id="lab-layers">
              ...
              `:a.map((e,t)=>{let n=l(e.perfumeId),i=r.find(e=>e.id===n.region);return`
                  <div class="lab-layer" data-idx="${t}" style="--region-color: ${i.color};">
                    <div class="lab-layer__bg" style="background-image: url('${De[i.id]}')"></div>
...
`).filter(e=>e.trim()).map(e=>{if(e.match(/^(OPENING|HEART|DRY DOWN|OVERALL|SILLAGE)/i)){let[t,...n]=e.split(`:`);return`<p><strong style="color: var(--accent);">${t.trim()}:</strong> ${n.join(`:`).trim()}</p>`}return`<p>${e}</p>`}).join(``):``}function Fe(){if(document.getElementById(`lab-styles`))return;let e=document.createElement(`style`);e.id=`lab-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e)}var Ie=`/mon_accord/assets/folder-BRsxHk1w.png`,Le={scandinavian:new URL(`/mon_accord/assets/scandinavian-5ibAHdAH.png`,``+import.meta.url).href,eastasia:new URL(`/mon_accord/assets/east_asia-DZK4DeZe.png`,``+import.meta.url).href,southafrica:new URL(`/mon_accord/assets/south_africa-DwLt2ni5.png`,``+import.meta.url).href,mediterranean:new URL(`/mon_accord/assets/mediterranean-fZMNFcGl.png`,``+import.meta.url).href,southamerica:new URL(`/mon_accord/assets/south_america-PQykoay7.png`,``+import.meta.url).href,middleeast:new URL(`/mon_accord/assets/middle_east-DEEY7Abs.png`,``+import.meta.url).href},Re={scandinavian:new URL(`/mon_accord/assets/scandinavian-T91uXqs1.png`,``+import.meta.url).href,eastasia:new URL(`/mon_accord/assets/east_asia-C6p3o17-.png`,``+import.meta.url).href,southafrica:new URL(`/mon_accord/assets/south_africa-DqiVXp2O.png`,``+import.meta.url).href,mediterranean:new URL(`/mon_accord/assets/mediterranean-zaCtkOEY.png`,``+import.meta.url).href,southamerica:new URL(`/mon_accord/assets/south_america-BEKaFRIH.png`,``+import.meta.url).href,middleeast:new URL(`/mon_accord/assets/middle_east-Df_0XU0d.png`,``+import.meta.url).href},ze={"mugler-angel":new URL(`/mon_accord/assets/angel-D_kA6fPK.png`,``+import.meta.url).href,"ysl-black-opium":new URL(`/mon_accord/assets/black_opium-DMAdw-Xv.png`,``+import.meta.url).href,"lancome-la-vie-est-belle":new URL(`/mon_accord/assets/la_vie_est_belle-Cgv61fN4.png`,``+import.meta.url).href},Be=[`mugler-angel`,`ysl-black-opium`,`lancome-la-vie-est-belle`],Ve=[{id:`evening`,name:`Evening Wear`},{id:`daytime`,name:`Daytime`},{id:`office`,name:`Office`},{id:`weekend`,name:`Weekend`},{id:`seasonal`,name:`Seasonal`}];function He(e,n){let a=t.getOwnedPerfumes(),s=Be.filter(e=>!a.loreal.includes(e));s.length&&(a.loreal=[...a.loreal,...s],t.setOwnedPerfumes(a));let c=t.get(`vault_folders`,null),u;if(!c)u=Ve.map(e=>({...e})),t.set(`vault_folders`,u);else{let e=c.filter(e=>e.id!==`default`),n=new Set(e.map(e=>e.id)),r=Ve.filter(e=>!n.has(e.id));u=[...e,...r].map(e=>({id:e.id,name:e.name})),t.set(`vault_folders`,u)}let d=null,f=!1,p=null,m=null;function h(){t.set(`vault_folders`,u)}function g(e){return t.getVault().filter(t=>t.folderId===e)}function _(){let n=t.getVault();e.innerHTML=`
      <div class="page__container">
        ${d?b(d,n):v(n)}
      </div>
      ${x()}
    `,Ue(),S(n)}function v(e){return`
          ...
          <div class="vault-folders-grid">
            ${u.map(e=>{let t=g(e.id).length;return`
                <div class="vault-folder-card card card--interactive" data-folder="${e.id}">
                  <img src="${Ie}" class="vault-folder-icon" alt="folder" loading="lazy" decoding="async" />
    ...
    `}function y(){let e=t.getOwnedPerfumes();function n(e){return e.length?`<div class="vault-bottle-grid">
        ${e.map(e=>{let t=l(e),n=t?r.find(e=>e.id===t.region):null,i=t?.format===`spray`?Le[t.region]:Re[t.region],a=t?.name||e;return`
        ...
        ${e.map(e=>{let t=o.find(t=>t.id===e),n=ze[e],r=t?t.name:e,i=t?.brand||``;return`
                      ...
                      <div class="vault-modal-bottle-row">
                        ${n?`
                          <button class="vault-modal-bottle-btn vault-owned-add-btn ${a?`vault-modal-bottle-btn--owned`:``}"
                            ...
                            ${a?`disabled`:``}>
                            <img src="${Le[t.id]}" alt="Spray" class="vault-modal-bottle-img" loading="lazy" decoding="async" />
                        ...
                        ${r?`
                          <button class="vault-modal-bottle-btn vault-owned-add-btn ${o?`vault-modal-bottle-btn--owned`:``}"
                            ...
                            ${o?`disabled`:``}>
                            <img src="${Re[t.id]}" alt="Oil" class="vault-modal-bottle-img" loading="lazy" decoding="async" />
...
Respond in EXACTLY this JSON format (no markdown, no code blocks):
{
  ...
  "scentDescription": "Vivid 2-sentence sensory preview"
        ...
        <div class="community-choice-row">
          <div class="community-choice-card">
            <div class="community-choice-card__photo-wrap">
              <img src="${Je}" alt="Duygu Özaslan" class="community-choice-card__photo" loading="lazy" decoding="async" />
            </div>
    ...
    `,Qe(),ae(n,i)}function te(e){let t=[...i.map(e=>({id:e.id,label:`${r.find(t=>t.id===e.region)?.icon||``} ${e.name} (${e.format})`,group:`Mon Accord`})),...o.map(e=>({id:e.id,label:`${e.brand} — ${e.name}`,group:`L'Oreal Luxe`}))];return`
  ...
  `:`<p style="font-size:var(--text-sm);color:var(--text-tertiary);padding:var(--space-lg);">No suggestion available.</p>`}function Qe(){if(document.getElementById(`community-styles`))return;let e=document.createElement(`style`);e.id=`community-styles`,e.textContent=`
  ...
  `,document.head.appendChild(e)}var $e=`/mon_accord/assets/Sephora-Logo-0PIDe1Np.png`,et=`/mon_accord/assets/Boyner_Logo-CHQxYmdT.jpg`,tt=`/mon_accord/assets/Trendyol_logo-CfMQtlYm.png`,nt=`/mon_accord/assets/hepsiburada-logo-I_Ul_zcU.png`,rt={scandinavian:new URL(`/mon_accord/assets/scandinavian-5ibAHdAH.png`,``+import.meta.url).href,eastasia:new URL(`/mon_accord/assets/east_asia-DZK4DeZe.png`,``+import.meta.url).href,southafrica:new URL(`/mon_accord/assets/south_africa-DwLt2ni5.png`,``+import.meta.url).href,mediterranean:new URL(`/mon_accord/assets/mediterranean-fZMNFcGl.png`,``+import.meta.url).href,southamerica:new URL(`/mon_accord/assets/south_america-PQykoay7.png`,``+import.meta.url).href,middleeast:new URL(`/mon_accord/assets/middle_east-DEEY7Abs.png`,``+import.meta.url).href},it={scandinavian:new URL(`/mon_accord/assets/scandinavian-T91uXqs1.png`,``+import.meta.url).href,eastasia:new URL(`/mon_accord/assets/east_asia-C6p3o17-.png`,``+import.meta.url).href,southafrica:new URL(`/mon_accord/assets/south_africa-DqiVXp2O.png`,``+import.meta.url).href,mediterranean:new URL(`/mon_accord/assets/mediterranean-zaCtkOEY.png`,``+import.meta.url).href,southamerica:new URL(`/mon_accord/assets/south_america-BEKaFRIH.png`,``+import.meta.url).href,middleeast:new URL(`/mon_accord/assets/middle_east-Df_0XU0d.png`,``+import.meta.url).href};function at(e,n){let a=t.getShopCart(),o=[...new Set(t.consumePendingShopCart())],s=[...new Set([...a,...o])],c=s.map(e=>({id:e}));if(o.length){let e=s.length-a.length;window.showToast(`Added ${e} new item${e===1?``:`s`} from your recommended combination.`)}u();function u(){t.setShopCart(c.map(e=>e.id))}function d(){let a=t.getOwnedPerfumes().monAccord||[];ot(a,c),e.innerHTML=`
      <div class="page__container">
                  ...
                  <div class="shop-region-card__products">
                    ${t?(()=>{let n=c.find(e=>e.id===t.id),r=a.includes(t.id);return`
                        <button class="shop-bottle-btn ${n?`shop-bottle-btn--in-cart`:``} ${r?`shop-bottle-btn--owned`:``}" data-id="${t.id}" style="--region-color:${e.color};">
                          ...
                          <img src="${rt[e.id]}" class="shop-bottle-img" alt="Spray" loading="lazy" decoding="async" />
                    ...
                    ${n?(()=>{let t=c.find(e=>e.id===n.id),r=a.includes(n.id);return`
                        <button class="shop-bottle-btn ${t?`shop-bottle-btn--in-cart`:``} ${r?`shop-bottle-btn--owned`:``}" data-id="${n.id}" style="--region-color:${e.color};">
                          ...
                          ${r?`<div class="shop-bottle-owned">Owned</div>`:``}
                          <img src="${it[e.id]}" class="shop-bottle-img" alt="Oil" loading="lazy" decoding="async" />
    ...
    `,ct(),e.querySelectorAll(`.shop-bottle-btn`).forEach(e=>{e.addEventListener(`click`,t=>{t.stopPropagation();let n=e.dataset.id,r=c.findIndex(e=>e.id===n);r>=0?c.splice(r,1):c.push({id:n}),u(),d()})}),e.querySelectorAll(`.shop-cart__item-remove`).forEach(e=>{e.addEventListener(`click`,()=>{c=c.filter(t=>t.id!==e.dataset.id),u(),d()})});let o=e.querySelector(`#btn-confirm-order`);o&&o.addEventListener(`click`,()=>{st(c,e,n)})}d()}function ot(e,t){if(!e.length)return null;let n=t.map(e=>e.id),r=e.map(e=>l(e)).filter(Boolean),a=[...new Set(r.flatMap(e=>e.scentFamily.split(`-`)))],o=i.filter(t=>!e.includes(t.id)&&!n.includes(t.id)&&t.scentFamily.split(`-`).some(e=>a.includes(e))).slice(0,2);return o.length?`Based on your owned ${r.map(e=>e.name).join(` & `)}: try adding ${o.map(e=>e.name).join(` or `)}.`:null}function st(e,n,r){t.clearShopCart();let i=document.createElement(`div`);i.className=`modal-overlay`,i.innerHTML=`
        ...
        <div class="order-retailers">
          <div class="order-retailers__grid">
            <div class="order-retailer-card" aria-label="Sephora">
              <img src="${$e}" alt="Sephora" class="order-retailer-card__logo" loading="lazy" decoding="async" />
            </div>
            <div class="order-retailer-card" aria-label="Boyner">
              <img src="${et}" alt="Boyner" class="order-retailer-card__logo" loading="lazy" decoding="async" />
            </div>
            <div class="order-retailer-card" aria-label="Trendyol">
              <img src="${tt}" alt="Trendyol" class="order-retailer-card__logo" loading="lazy" decoding="async" />
            </div>
            <div class="order-retailer-card" aria-label="Hepsiburada">
              <img src="${nt}" alt="Hepsiburada" class="order-retailer-card__logo" loading="lazy" decoding="async" />
            </div>
          </div>
        </div>
  ...
  `,document.body.appendChild(e),document.getElementById(`dt-skip`).addEventListener(`click`,X),document.getElementById(`dt-next`).addEventListener(`click`,()=>{clearTimeout(Y),clearInterval(K),kt()})}function jt(){document.getElementById(`dt-card`)?.remove(),document.getElementById(`dt-flash`)?.remove()}function Mt(){q&&X(),B=window.location.hash||``,V=ut(localStorage),H=ut(sessionStorage),U=window.__retakeQuiz,localStorage.clear(),sessionStorage.clear(),delete window.__retakeQuiz,localStorage.removeItem(mt),q=!0,J=0,At(),Ot(0)}function X(){q=!1,clearTimeout(Y),clearInterval(K),dt(localStorage,V),dt(sessionStorage,H),V=null,H=null,U===void 0?delete window.__retakeQuiz:window.__retakeQuiz=U,U=void 0,(B||``)!==(window.location.hash||``)&&(window.location.hash=B||`#landing`),B=``,jt()}function Nt(){if(document.getElementById(`dt-styles`))return;let e=document.createElement(`style`);e.id=`dt-styles`,e.textContent=`
    #dt-card {
      position: fixed;
      z-index: 99995;
      top: 0;
  ...
  `,document.head.appendChild(e)}var Z=document.getElementById(`app`);function Q(e){document.documentElement.setAttribute(`data-theme`,e),t.set(`theme`,e)}function Pt(){let e=t.get(`theme`,`dark`);document.documentElement.setAttribute(`data-theme`,e)}var Ft={"":f,"#landing":f,"#profile":ue,"#lab":Ne,"#vault":He,"#shop":at,"#community":Xe};function It(){return window.location.hash||``}function Lt(e){window.location.hash=e}function $(){let e=It(),t=Ft[e]||f;Z.innerHTML=``,Z.appendChild(n(Lt,e));let r=document.createElement(`div`);if(r.className=`page page-enter`,r.id=`page-content`,t(r,Lt),Z.appendChild(r),!document.querySelector(`.toast-container`)){let e=document.createElement(`div`);e.className=`toast-container`,e.id=`toast-container`,document.body.appendChild(e)}window.scrollTo({top:0,behavior:`auto`})}window.showToast=function(e,t=`success`){let n=document.getElementById(`toast-container`);if(!n)return;let r=document.createElement(`div`);r.className=`toast toast--${t}`,r.innerHTML=`<span class="toast__message">${e}</span>`,n.appendChild(r),setTimeout(()=>{r.style.animation=`fadeOut 0.3s var(--ease-out) forwards`,setTimeout(()=>r.remove(),300)},3e3)},window.showSettings=function(){let e=document.querySelector(`.modal-overlay`);e&&e.remove();let n=t.get(`theme`,`dark`),r=document.createElement(`div`);r.className=`modal-overlay`,r.id=`settings-modal`,r.innerHTML=`
    <div class="modal">
      <div class="modal__header">
